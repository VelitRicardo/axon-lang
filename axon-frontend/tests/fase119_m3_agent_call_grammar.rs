//! §Fase 119.m.3 — `<Agent>(arg, …)` is a step-body statement.
//!
//! The last piece that makes §119.m.1's bounded control loop reachable from
...
//! from nothing. §119.m.1 was, briefly and by design, the very shape this fase
...
fn m3_2_arguments_are_dotted_subjects() {
    // README block 9: `TrendAnalyzer(Gather.output)`. Without §119.f.10's
...
"#;
    ...
    assert_eq!(
        ...
        vec![
            ...
            // §60's classification survives: the quotes distinguish a literal
...
// ── §3 — the call must not shadow the step's own fields ────────────────────

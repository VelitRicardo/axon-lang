//! v2.89.0 — **`axon-T1213`: a streaming tool cannot be memoised.**
//!
//! # Why this law exists
//!
//! v2.89.0 cabled the `cache` primitive into the dispatch path. Measuring the
//! consumers first — rather than trusting v2.89.0's note that this was *"the
...
//! neither, silently, is precisely the defect v2.89.0 exists to remove.
//!
//! So the combination is refused. This follows **the design decision** — the ruling that
...
//! drifts from the law, the v2.87.0 defect this project has already paid for
//! twice.
//!
...
///
/// v2.87.0 found `check` and `check_with_warnings` silently divergent for entire
/// cycles, with the CLI taking the one that had lost two laws. They are
...
fn errors_through_both_doors(src: &str) -> Vec<String> {
    ...
    assert_eq!(
        ...
        "the two checker entry points disagree about this program — v2.87.0's defect, returned"
    );
    via_check
}
...
fn a_streaming_tool_may_not_name_a_cache() {
    ...
    // Sanity: this is the shape that compiled CLEAN before v2.89.0. `axon-T865`
...
/// not an error. This pins that v2.89.0 narrowed the language by exactly one
/// shape and left the design decision's widening story intact.
#[test]
...
";
    ...
    assert!(
        ...
        "v2.89.0 must not have turned every widening into an error: {errs:?}"
    );
}

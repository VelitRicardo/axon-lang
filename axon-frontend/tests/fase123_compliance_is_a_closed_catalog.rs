//! §Fase 123 — **`compliance:` is a closed catalogue, as the paper always said.**
//!
//! # The promise, and where it went
//!
//! The ESK paper §6.1 (*Regulatory Type Theory*) states the rule as a
//! compile-time law:
//!
...
//! this fase existed: `compliance: [NOT_A_FRAMEWORK]` on an `axonendpoint`
...
fn errors(src: &str) -> Vec<String> {
    ...
    // Through BOTH doors — §120.e found these two silently divergent for entire
    // fases, with the CLI taking the one that had lost two laws.
...
/// a good law. It compares raw strings, so before §123 a `type` labelled
...
fn a_symmetric_typo_can_no_longer_satisfy_the_coverage_law() {
    ...
    assert!(
        ...
        "spelled correctly, this is exactly the shape §6.1 exists to bless: {:?}",
        errors(&fixed)
    );
}

...
//!
//! §119.h landed this ratchet on `axon-lang` and measured the other two crates
...
//! (§111.i's dead cable, §117's gate that stayed red, §118's port gate that
//! measured itself) — and here the gate is not even ours, it ships with the
//! toolchain.
//!
//! # Why the baseline is 17 and not §119.h's 16
//!
//! Measured, not copied. `cargo doc --no-deps` reports **17** today. The
//! difference is NOT this fase's doing, and that was checked rather than
//! assumed: of the links reported in files §119.n touched, all are pre-existing
//! mathematical notation on lines this fase did not edit — `[0,1]` as an
//! interval, `derivatives[i]` as an index — which rustdoc reads as link syntax.
//!
//! §119.n did introduce exactly one, and the measurement caught it before the
//! ratchet went in: `[`fase_23_algebraic_effects_runtime.md`]` in `ast.rs`,
...
//!
//! Same reason as §119.h: `#![deny]` turns 17 warnings into 17 build errors, so
...
#[test]
fn fase119h2_frontend_broken_intra_doc_links_only_ever_shrink() {

//! §Fase 121.b — **the confidence guard: grammar → AST → IR, and every closed
...
// ── §1 — the published surface parses and the guard rides its validation ────
...
// ── §2 — every closed catalog REFUSES, naming itself ────────────────────────
...
// ── §3 — the structural law: no validation, no guard ────────────────────────
...
// ── §5 — nothing moved for programs without guards ──────────────────────────
...
fn b5_a_guardless_program_adds_nothing_to_the_ir_json() {
    ...
    assert!(
        ...
        "the guard field must be ELIDED when absent so no pre-§121 IR-SHA moves"
    );
}

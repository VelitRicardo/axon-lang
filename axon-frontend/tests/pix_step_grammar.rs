//! v2.83.0 — the pix/corpus family: PIX verbs as step-body statements.
//!
//! v2.81.0 built the README ledger and predicted this exact family: *"ONE
//! `pix`/`corpus` header-grammar decision clears fifteen blocks."* The
//! decision turned out to be the the design decision shape a second time — the README
...
//! generates, exactly like v2.83.0's `lambda`/`ots` guards. Reusing the
//! flow-level node types (no new AST shapes) is the the design decision doctrine — one
...
fn the_depth_override_reaches_the_ir_so_something_can_consume_it() {
    // A declared field consumed by nothing is the v2.67.0 defect. `depth:`

...
#[test]
fn every_manifest_agrees_on_the_msrv() {
...
#[test]
fn readme_states_the_real_msrv() {
    ...
    assert!(
        ...
        "README.md must state the real MSRV (`**{msrv}+**`, as declared by the manifests). \
         Before v2.81.0 it claimed 1.75+ against a real floor of 1.95 — a reader on an older \
...
#[test]
fn readme_documents_the_c_toolchain_prerequisite() {
    ...
    //
    // v2.83.0 — the REASON changed even though the requirement did not, and
...
const CLASSIFIED_HOSTS: &[(&str, Host)] = &[
    ...
    ("axon-lang.io", DoesNotResolve(
        "HTTP 000 / no DNS answer. v2.83.0 REMOVED every USER-FACING use of it. the design decision \
...
#[test]
fn every_project_url_host_is_classified() {
...
#[test]
fn dead_hosts_are_declared_not_discovered() {

//! §Fase 119.f.10 — **a SUBJECT is a dotted reference.**
//!
//! Every statement in the language has two kinds of operand, and until this
//! fase one function parsed both:
//!
...
//! (`parse_dotted_identifier`) had existed since §62.
//!
...
//! ambiguity §60's literal/reference classification exists to remove.
...
fn f10_2_a_string_literal_argument_keeps_its_quotes() {
    ...
    // The quotes are the §60 classification made durable: without them dispatch
...
// ── §3 — the NAME position is NOT dotted ─────────────────────────────────────
...
"#;
    let program = parse(src).expect("the pre-§119.f.10 bare forms must still parse");

//! v2.83.0 — `fabric` governs: the compile-time half.
//!
//! v2.67.0's finding: *"`provider`/`region`/`zones` are consumed by NOTHING at
//! runtime … Still governs nothing that runs."* `fabric` has no paper; its
//! specification is its knowledge doc, which draws the line this step
//! works inside:
//!

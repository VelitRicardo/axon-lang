//! v2.83.0 — the grammar that let README's lambda/ots blocks compile.
//!
...
//! v2.0.0's own comments name `List<T>` as the canonical carrier, so the
...
//! position) — the the design decision statement position extended to the fourth
...
/// v2.83.0 — README III writes `hibernate until "quarterly_data_available"`;

//! v2.83.0 — **`stream<T> { on_chunk: … on_complete: … }` is REAL.**
//!
...
//! `Stream.output` the next step then reasoned over. The block had left the v2.81.0
//! ledger on the strength of compiling.
//!
//! At FLOW level the same body was a hard parse error
//! (`Unexpected token in flow body: 'on_chunk'`), because v2.67.0 had given
//! `StreamBlock` a `body: Vec<FlowStep>` — a shape **no published block and no
//! paper writes**. `algebraic_effects_runtime.md` section 3.7 specifies
//! `stream<τ> { on_chunk: B₁ on_complete: B₂ }`, and its D8 promises *"backward
//! compat 100%, cero cambios en `.axon` source files de adopters"*.
//!
//! So `stream`'s `advertised.rs` attestation was the v2.83.0 `reason` defect
...
//! - v2.67.0's `stream { <steps> }` body form still parses — this is additive;
...
/// v2.67.0's form still parses. This landing is additive, not a replacement.
#[test]
...
"#;
    let program = parse(src).expect("the v2.67.0 body form must keep parsing");
    for d in &program.declarations {
        if let Declaration::Flow(f) = d {
            for s in &f.body {
                if let FlowStep::Stream(sb) = s {
                    assert_eq!(sb.body.len(), 1, "the v2.67.0 body still lowers");
...
/// The v2.22.0/v2.46.0 discipline: a program that uses none of the new grammar must
...
/// The handlers must reach the IR — an AST field nothing lowers is the v2.67.0
...
// ── v2.83.0 — the third arm, `on_error` ─────────────────────────────────
...
/// v2.83.0 — ask which direction the silence fails in. A skipped handler

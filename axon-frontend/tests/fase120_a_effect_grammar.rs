//! §Fase 120 — **the four algebraic-effect constructs reach the IR, from
...
//!
//! This is not §111's *motor real, cable muerto* (a published grammar reaching
...
//!     existed since §23 and had never held an entry**;
//!   - `handle E { … } in { … }` lowers with clause bodies that are ORDINARY
//!     flow nodes (D120.1 — the property that makes a handler able to `emit`);
//!   - `perform` in a step body lands on `IRStep::performs` and NOT on
//!     `pix_ops` (the §119.n lesson: an elevation runs BEFORE generation);
//!   - **D120.2** — the bare form `perform Emit(x)` RESOLVES, and a colliding
...
//!   - backward compatibility: a pre-§120 program's IR JSON is byte-identical.
...
// ── §1 — the declaration reaches the IR field that never held one ───────────
...
fn a1_effect_declaration_lands_in_the_ir_effects_field() {
    ...
    assert_eq!(
        ...
        "`effect SSE {{ … }}` must lower into IRProgram::effects — the field \
         §23 created as a Python-parity mirror and that NOTHING had ever \
    ...
    assert_eq!(
        ...
        "`Never` is the bottom sentinel fase_23 §3.1 writes — an operation \
...
// ── §2 — `handle … in …` lowers, and its bodies are REAL nodes ──────────────

/// **D120.1.** The clause body must lower to ordinary `IRFlowNode`s. This is
/// the assertion that separates this fase from the plan's original step 2:
...
// ── §3 — `perform` in a step body is NOT an elevation ───────────────────────

/// The §119.n lesson, applied a second time. A `pix_ops` entry runs BEFORE the
...
// ── §4 — D120.2, the resolution rule ────────────────────────────────────────

/// The BARE form is what `fase_23` §3.1 publishes. It must resolve.
#[test]
...
// ── §5 — D9, and it is INTERPROCEDURAL ──────────────────────────────────────
...
fn a5_the_canonical_composition_compiles_clean() {
    ...
    assert!(
        errs.is_empty(),
        "fase_23 §3.1's own program must compile — an outer handler intercepting \
...
fn a5b_the_same_flow_run_bare_is_refused_at_the_run_site() {
    ...
    assert!(
        ...
        "an undischarged effect reaching a top-level run is a COMPILE error \
         (fase_23 D9 — there is no runtime fallback). Got:\n{errs}"
    );
...
// ── §6 — the clause-scope law and D10 ───────────────────────────────────────
...
// ── §7 — backward compatibility, measured ───────────────────────────────────
...
/// before §120 — every new IR field is `skip_serializing_if`, so no pre-§120
...
fn a7b_a_program_without_effects_has_no_effect_fields_in_its_ir_json() {
    ...
    assert!(
        ...
        "an effect-free step must not emit `performs` — the field is elided \
         when empty so no pre-§120 IR-SHA moves"
    );
    assert!(
    ...
    assert_eq!(
        ...
        1,
        "IRProgram::effects keeps its pre-§120 serialization (an empty array, \
...
// ── §8 — the AST position, pinned ───────────────────────────────────────────

/// `handle` nests: a handler inside a handler's `in` body must parse, or the
/// decorator shape of §5g could not be written.
#[test]
fn a8_handlers_nest() {
    let prog = parse(
        "effect A { Op() -> Unit }\n\

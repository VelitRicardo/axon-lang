//! §Fase 119.f.11 — `retrieve from <Store> where "…"` as a step-body statement.
//!
...
//! in the system (§35–38 typed store, `wire_integrations::run_retrieve`, the
//! `*_pg_integration` suites). Only the POSITION was missing — which is what
//! the D119.4 doctrine is for: one concept, two positions, one implementation.
//!
...
//! hand the model an empty binding, which is §112's defect.
...
fn f11_5_the_braced_form_still_parses() {
    // Back-compat: the only form that parsed before §119.f.11.

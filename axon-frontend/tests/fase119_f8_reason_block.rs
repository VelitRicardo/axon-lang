//! §Fase 119.f.8 — **`reason { given: … ask: "…" depth: N }` is REAL.**
//!
...
//! reach. §111's own phrase for this: *motor real, cable muerto*. §111 built
...
//! direction §119.h.2 named: a `reason` that silently loses its `ask:` still
...
//! (the D119.4 doctrine), so the two cannot drift.
...
fn f8_2_the_prompt_survives_to_the_ir() {
    ...
    assert!(
        ...
        "the step's only prompt must reach the IR. Before §119.f.8 this program \
...
// ── §2 — the same node at flow level ─────────────────────────────────────────

#[test]
fn f8_5_the_block_form_also_parses_at_flow_level() {
    // Pre-§119.f.8 this was a hard parse error: the two positions were served
...
// ── §3 — the catalog is CLOSED ───────────────────────────────────────────────

#[test]
fn f8_7_an_unknown_field_is_refused_and_named() {
    // §119.h.2's rule: ask which direction the silence fails in. A skipped
...
/// `reason: "…"` written as a step FIELD. §119.f gave `navigate` the same dual
...
fn f8_11_the_field_form_survives_to_the_ir() {
    ...
    assert!(
        ...
        "before §119.f.8 this value was discarded at PARSE time and never \
         reached the IR.\n{json}"
    );
}
...
// ── §4 — no IR drift for programs that do not use it ─────────────────────────

#[test]
fn f8_9_a_program_without_the_block_form_has_byte_identical_ir() {
    // The §68.b/§91.a discipline: new IR fields are elided when empty, so a
    // pre-§119.f.8 program's IR JSON — and therefore its IR-SHA — is unchanged.

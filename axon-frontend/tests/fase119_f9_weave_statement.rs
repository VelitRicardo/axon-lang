//! §Fase 119.f.9 — **`weave [a, b] format: T include: […]` is REAL.**
//!
...
//!
//! **What this pins.**
//!   - the three published surfaces parse into ONE node (D119.4);
...
//!     nothing is the §111 defect this fase exists to end;
...
// ── §1 — the published statement reaches the AST ─────────────────────────────
...
fn f9_1_the_published_step_statement_parses() {
    ...
    assert_eq!(
        ...
        "`include:` had no slot at any position before §119.f.9"
    );
}
...
fn f9_2_it_survives_to_the_ir() {
    ...
    for needle in ["Extract.output", "recommendations"] {
        assert!(
            ...
            "`{needle}` must reach the IR — before §119.f.9 the whole statement \
             was discarded at PARSE time.\n{json}"
        );
    }
}

// ── §2 — the terminator bug, from the side that caused it ────────────────────

/// The regression that named this sub-fase: `output:` after a weave belongs to
...
// ── §3 — the flow-level surface, same node ───────────────────────────────────
...
fn f9_5_the_braced_field_form_still_parses() {
    // Pre-§119.f.9 back-compat: the only form the parser used to accept.
    let src = r#"
flow F(a: String) -> R {
...
///
/// It is also a compatibility surface. §54.a's suite pins this shape, because
/// before §119.f.9 it disappeared into `skip_flow_step_structural` — parsing it
...
fn f9_6_a_weave_without_include_has_byte_identical_ir() {
    ...
    assert!(
        ...
        "`include` must be ELIDED when empty, or every pre-§119.f.9 IR-SHA \
         moves.\n{json}"
    );
}

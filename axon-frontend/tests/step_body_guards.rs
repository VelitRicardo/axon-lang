//! v2.83.0 — `mandate X on Y` / `shield X on Y -> b` / `ots X on Y`
...
//! blocks 40–42 never compiled: not `pid` (fixed in v2.83.0), and not mandate
...
//! cycle's standing rule — and the family shares one AST shape with the
...
//! enforcement of a step-scoped mandate is v2.83.0 Tier 1/Tier 2, which
//! consumes `IRStep.guards` together with the `(D, L)` obligations of
//! v2.83.0. Until that lands, `mandate` stays in `KNOWN_DEBT`: a parsed cage
...
fn a_stepless_program_serialises_without_a_guards_key() {
    // The v2.22.0/v2.46.0 discipline: pre-v2.83.0 programs' IR JSON stays
...
"#,
    ...
    assert!(
        ...
        "an empty guards key would shift every pre-v2.83.0 IR-SHA"
    );
}

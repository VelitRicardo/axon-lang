//! §Fase 117.a — **every published example compiles**. The ratchet that makes
//! the shop window a build input — `docs/fase/fase_117_the_public_artifact.md`,
...
//! government, on §113's closed `resource` catalog) and two that did not even
...
#[test]
fn fase117a_every_example_compiles() {
    let examples = every_example();

    // A directory walk that finds nothing would "pass" vacuously. Pin the
    // floor: this is the count at §117.a, and it may only grow.
    assert!(
        examples.len() >= 12,
        "expected at least the 12 examples present at §117.a, found {}: {examples:#?}",
        examples.len()
    );

//! v2.83.0 — Vía C: the compiler verifies the stability BAND, not a sign.
//!
//! # What this gate protects
//!
//! README XV promises compile-time rejection of unstable gains, and until this
//! step the checker implemented exactly the published sign tests:
//! `Kp > 0 ∧ Ki ≥ 0 ∧ Kd ≥ 0`. The two papers show that to be **necessary but
//! not sufficient**, in opposite directions:
//!
//! - `paper_mandate.md` section 3: the Lyapunov argument is conditional on the control
//!   effort **exceeding the drift bound `D`** — too small and the cage is open;
//! - `paper_mathematical_prompt_optimization.md` section 6.3: convergence needs
...
//!
//! This is the same construction as v2.4.0's proof-carrying code and v2.46.0's `now:`
...
fn gains_that_pass_every_sign_test_are_refused_below_the_declared_drift() {
    // THE test of the step. Kp, Ki, Kd all positive — every published
...
fn without_a_stability_block_the_sign_checks_still_apply_and_nothing_more() {
    ...
    // every mandate written before v2.83.0 declares no bounds and must keep
...
fn an_empty_stability_block_is_a_parse_error_not_a_silent_no_op() {
    // The v2.83.0 lesson applied forward: a block the parser walks past
...
// ══════════════════════════════════════════════════════════════════════
// v2.83.0 — a misspelled bound is an ERROR, not a dropped hypothesis
// ══════════════════════════════════════════════════════════════════════
//
// Found by smoke-testing the published 2.84.0 binary rather than by reading
// the code, which matters: the source said, in a comment I wrote in v2.83.0,
...
// thing it checks was never there — and it is exactly what v2.83.0 was built
...
#[test]
fn an_unrecognised_stability_key_is_refused_not_skipped() {
    for (typo, block) in DROPPED_BOUNDS {
        let err = parse_result(&mandate_with(block)).expect_err(
            "a stability block naming a key that is not D or L must be a parse error. \
             Before v2.83.0 it compiled clean and the bound simply vanished, which is worse \
...
#[test]
fn an_unrecognised_pid_key_is_refused_not_skipped() {
...
#[test]
fn the_accepted_spellings_all_still_parse() {

...
pub const PRIMITIVE_REGISTRY: &[PrimitiveInfo] = &[
    ...
    PrimitiveInfo {
        ...
        top_level: true,
        since: "v1.27.2",
        summary: "A declared analytical data container — the deterministic data plane's store. Typed columnar schema (`column <name>: <Type>` over the closed 6-type catalog, axon-T928), instantiated in the columnar engine at deploy; governed `ingest` and the relational query verbs since v2.63.0.",
        doc_status: DocStatus::Documented,
        is_advertised: true,
    },
    PrimitiveInfo {
    ...
    PrimitiveInfo {
        ...
        top_level: true,
        since: "v2.3.0",
        summary: "Declares the typed bidirectional dialogue protocol a socket carries — the session-type algebra (send/receive/select/branch/loop/end).",
        doc_status: DocStatus::Documented,
        is_advertised: true,
    },
    PrimitiveInfo {
    ...
    PrimitiveInfo {
        ...
        top_level: true,
        since: "v2.60.0",
        summary: "Governed CRM Delivery — the egress-dual of acquisition. A declarative, compile-time-validated write of assertions into a system of record (a CRM): idempotent operations (`upsert_contact`/`create_deal`/`add_note`), per-tenant credentials via secret custody, `web` effect. The provenance-stripping barrier (T920) refuses a `provenance: cleared` delivery of an unshielded flow value — a vendor guess must arrive in the CRM labeled as a guess, not laundered into a fact.",
        doc_status: DocStatus::Documented,
        is_advertised: true,
    },
    PrimitiveInfo {
        ...
        // anyone opened `papers/paper_mandate.md`. The registry is the
    ...
    PrimitiveInfo {
        ...
        category: "operators",
        top_level: false,
        since: "v1.12.0 (stub); real since v2.41.0",
    ...
    PrimitiveInfo {
        ...
        category: "operators",
        top_level: true,
        since: "v2.28.0 (daemon-attached); top-level since v2.69.0",
        summary: "A declared spend ceiling over cognition and tool use — enforced on the canonical `use Tool(…)` path; attachable to a daemon or declared top-level.",
        doc_status: DocStatus::Documented,
        is_advertised: true,
    },
    PrimitiveInfo {
        ...
        top_level: true,
        since: "v2.66.0",
        summary: "Governed human notification — the third egress dual (deliver · document · notify): spends HUMAN ATTENTION, carries lineage or refuses, at-most-once-per-window across replicas.",
        doc_status: DocStatus::Documented,
        is_advertised: true,
    },
    PrimitiveInfo {
    ...
    PrimitiveInfo {
        ...
        top_level: true,
        since: "v1.3.1",
        summary: "A compliance-bound UI component — the compile-time shield-coverage law over regulated κ IS enforced (a real set difference); rendering is deferred.",
        doc_status: DocStatus::Documented,
        is_advertised: true,
    },
    PrimitiveInfo {
        ...
        top_level: true,
        since: "v1.3.1",
        summary: "A UI view declaration — referential integrity is checked; route dispatch and session-typed reactivity are deferred.",
        doc_status: DocStatus::Documented,
        is_advertised: true,
    },
    PrimitiveInfo {
    ...
    PrimitiveInfo {
        ...
        top_level: true,
        since: "v0.1.0 (epistemic lattice)",
        summary: "Epistemic scope — stamps its block's derivations with the `believe` level of the uncertainty lattice (the egress floor for assertive slots).",
        doc_status: DocStatus::Documented,
        is_advertised: true,
    },
    PrimitiveInfo {
    ...
    PrimitiveInfo {
        ...
        category: "cognition",
        top_level: false,
        since: "predates v2.15.0; real fan-out since v2.15.0",
        summary: "Concurrent fan-out — executes its branches in parallel (a real `join_all`) and joins their results; branch tool calls ride the channel semaphore.",
        doc_status: DocStatus::Documented,
        is_advertised: true,
    },
    PrimitiveInfo {
        ...
        category: "cognition",
        top_level: false,
        since: "unrecorded (predates v2.67.0)",
    ...
    PrimitiveInfo {
        ...
        top_level: false,
        since: "unrecorded (predates v2.67.0)",
        summary: "Budget-bounded deliberation — FAILS CLOSED (axon-T939): refused at check time rather than silently discarded; no budget was ever controlled by the old placeholder.",
        doc_status: DocStatus::Documented,
        is_advertised: true,
    },
    PrimitiveInfo {
        ...
        top_level: false,
        since: "unrecorded (predates v2.67.0)",
        summary: "Multi-candidate consensus — FAILS CLOSED (axon-T940): refused at check time; no votes, no aggregation, no candidates in the old placeholder.",
        doc_status: DocStatus::Documented,
        is_advertised: true,
    },
    PrimitiveInfo {
        ...
        category: "operators",
        top_level: false,
        since: "predates v2.67.0; body executed since v2.67.0",
        summary: "Algebraic-effects stream block — the body is parsed, lowered and EXECUTED over the Free-Monad CPS handler runtime (since v2.67.0; it used to be discarded at parse time).",
        doc_status: DocStatus::Documented,
        is_advertised: true,
    },
    ...
    PrimitiveInfo {
        ...
        top_level: false,
        since: "v2.87.0",
        summary: "Delimited handler scope — `handle E { Op(p) -> { … } } in { … }`. The clause body is ORDINARY flow steps, so a handler composes with every primitive. Discharged inside a clause by `resume(v)` (one-shot), `abort(v)`, `forward E.Op(args)`, or by running off the end (an implicit abort).",
        doc_status: DocStatus::Documented,
        is_advertised: true,
    },
    PrimitiveInfo {
        ...
        top_level: false,
        since: "v2.87.0",
        summary: "Raise an algebraic effect — `perform Emit(x)` or `perform SSE.Emit(x)`. Legal at flow level and in a step body (where it runs AFTER generation, so it can perform the step's own output). An undischarged perform is a COMPILE error (axon-T966): there is no runtime fallback.",
        doc_status: DocStatus::Documented,
        is_advertised: true,
    },
    PrimitiveInfo {
        ...
        top_level: false,
        since: "v2.65.0",
        summary: "The proof-carrying derivative — SYMBOLIC differentiation over the expression language at compile time; the gradient IS IR (PCC GradientSoundness) and the runtime only evaluates.",
        doc_status: DocStatus::Documented,
        is_advertised: true,
    },
    PrimitiveInfo {
        ...
        category: "data_plane",
        top_level: false,
        since: "v2.12.0–v2.15.0",
        summary: "Knowledge navigation — three REAL deterministic engines (MDN store-sourced, MDN in-memory, PIX); with no indexable source in scope it falls back to an LLM prompt (the named gap).",
        doc_status: DocStatus::Documented,
        is_advertised: true,
    },
    PrimitiveInfo {
        ...
        top_level: false,
        since: "v2.12.0–v2.15.0",
        summary: "Subtree descent under a prior navigate — real navigation when a source is in scope; degrades to a placeholder otherwise.",
        doc_status: DocStatus::Documented,
        is_advertised: true,
    },
    PrimitiveInfo {
        ...
        top_level: false,
        since: "v2.12.0–v2.15.0",
        summary: "Reads the breadcrumb a navigate seeded — a real provenance trail when navigation was deterministic; inherits the same named gap on the LLM fallback path.",
        doc_status: DocStatus::Documented,
        is_advertised: true,
    },
    PrimitiveInfo {
        ...
        category: "data_plane",
        top_level: false,
        since: "predates v2.63.0; made real in v2.63.0",
        summary: "Governed ingestion into a dataspace — bounds-BEFORE-parse, sha256 provenance, born-Untrusted taint (since v2.63.0; the earlier placeholder hallucinated success).",
        doc_status: DocStatus::Documented,
        is_advertised: true,
    },
    PrimitiveInfo {
        ...
        top_level: false,
        since: "v2.63.0",
        summary: "The dataspace selection-projection verb (σ∘π) over the first-party columnar engine.",
        doc_status: DocStatus::Documented,
        is_advertised: true,
    },
    PrimitiveInfo {
        ...
        top_level: false,
        since: "v2.63.0",
        summary: "The dataspace join verb (⋈) — a hash equi-join over the columnar engine that REFUSES a keyless join.",
        doc_status: DocStatus::Documented,
        is_advertised: true,
    },
    PrimitiveInfo {
        ...
        top_level: false,
        since: "v2.63.0",
        summary: "The dataspace aggregation verb (γ) over the first-party columnar engine.",
        doc_status: DocStatus::Documented,
        is_advertised: true,
    },
    PrimitiveInfo {
        ...
        top_level: false,
        since: "v2.63.0",
        summary: "The dataspace profiling verb — zone-map statistics over declared columns.",
        doc_status: DocStatus::Documented,
        is_advertised: true,
    },
];
...
mod tests {
    ...
    fn documented_tier_matches_phase_6d_baseline_full_coverage() {
        ...
        assert_eq!(
            documented, expected,
            "Documented set drift — baseline + quant/observable + window (full coverage)"
        );
    }

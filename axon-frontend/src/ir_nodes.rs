...
pub struct IRProgram {
    ...
    /// v2.12.0 — audit-chain (`ledger`) declarations. Distinct from
    ...
    /// v2.5.0 — closed-catalog extension declarations (compiled).
    ...
    /// v2.63.0 — the compiled dataspace schemas, SERIALIZED into the
    ...
    /// v2.4.0 — Pauli-sum observable declarations (compiled). Each carries
    ...
    /// v2.23.0 — Advantage-Witness declarations. `skip_serializing_if = empty`
    ...
    /// v2.37.0 — outbound vendor connections (compiled). Each carries its
    ...
    /// v2.38.0 — named, referenced browser-origin policies. `skip_serializing_if
    ...
    /// v2.40.0 — named, referenced result-memoization policies. Same
    /// `skip_serializing_if = empty` IR-SHA discipline as `cors_policies`.
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub caches: Vec<IRCache>,
    /// v2.46.0 — ephemeral-credential contracts (`credential { ttl:
    ...
    /// v2.42.0 — long-horizon autonomous research primitives (compiled). Each
    ...
    /// v2.53.0 — compiled document declarations. Same
    ...
    /// v2.60.0 — compiled CRM delivery declarations. Same
    ...
    /// v2.66.0 — governed human notifications (the third egress dual).
    pub notifications: Vec<IRNotify>,
    /// v2.42.0 — dynamic tool-synthesis policies (compiled). Same
    /// `skip_serializing_if = empty` IR-SHA discipline as `savants`.
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub synths: Vec<IRSynth>,
    /// v2.43.0 — authorization-scope policies (compiled). Same
    /// `skip_serializing_if = empty` IR-SHA discipline as `synths`.
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub scopes: Vec<IRScope>,
    /// v1.17.0 — algebraic effect declarations (compiled).
    ...
    /// v2.76.0 — per-module provenance of a LINKED program: for every
...
/// v2.76.0 — one linked module's provenance record: the audit chain
...
/// v2.4.0 — IR for the `yield <expr>` measurement point.
#[derive(Debug, Clone, Serialize)]
pub struct IRYield {
    pub node_type: &'static str,
    pub source_line: u32,
...
/// v2.4.0 — one term `cₖ · Pₖ` of a Pauli-sum observable (compiled).
#[derive(Debug, Clone, Serialize)]
...
/// v2.4.0 — IR for a Pauli-sum observable `M = Σ cₖ Pₖ`.
#[derive(Debug, Clone, Serialize)]
...
/// v2.23.0 — IR for an Advantage Witness. The deploy/runtime evaluator reads
...
// ── v1.17.0 / v2.87.0 — Algebraic effect declarations ────────────────────
//
// v1.17.0 declared these as a MIRROR of the Python frontend's dataclasses,
// emitted by nobody, so a structural-parity gate stayed green against an empty
// `effects: []`. v2.87.0 makes `axon-frontend` emit them from `.axon` source:
...
pub struct IRResource {
    ...
    pub kind: String,
    pub endpoint: String,
    /// v2.67.0 — the pool size. Until v2.67.0 this was **read by nothing**: every
    ...
    ///
    /// Before v2.67.0, two stores shared a connection pool by **accidental DSN
    ...
    /// v2.67.0 — the `fabric` this resource lives in (`within: Prod`).
    ///
    ...
    ///
    /// Empty ⇒ no fabric declared. Skip-if-empty ⇒ every pre-v2.67.0 program
    /// serializes byte-identically (IR-SHA stability, the v2.48.0 `class`
    /// precedent).
...
/// v1.1.0; nested continuations arrive with handlers + reconcile loops.
#[derive(Debug, Clone, Serialize)]
...
// ── v1.1.0 — IRReconcile / IRLease / IREnsemble ──────────────────────
...
// ── v1.1.0 — IRSession / IRTopology ──────────────────────────────────

/// One operation in a compiled session protocol
/// (send / receive / loop / end / select / branch — v2.3.0 adds the choices).
#[derive(Debug, Clone, Serialize)]
pub struct IRSessionStep {
    ...
    /// v2.3.0 — labelled branches (only for `op == "select" | "branch"`;
    /// v2.36.0 reuses them for `op == "interrupt"`: `body` + `handler` arms).
    #[serde(skip_serializing_if = "Vec::is_empty", default)]
    pub branches: Vec<IRSessionBranch>,
    /// v2.36.0 — `op == "interrupt"` only: the handler's signal binder
    /// (`... as <sig> ...`). Skip-if-empty ⇒ zero IR-SHA drift for every
    /// non-interrupt step (the v2.33.0/v2.34.0 additive-only discipline).
    #[serde(skip_serializing_if = "String::is_empty", default)]
    pub binder: String,
    /// v2.36.0 — `op == "interrupt"` only: the block declares a `resumable`
    /// handler. Skip-if-false ⇒ byte-identical IR for every other op.
    #[serde(skip_serializing_if = "std::ops::Not::not", default)]
    pub resumable: bool,
}

/// v2.3.0 — one labelled arm of a compiled `select`/`branch` choice.
#[derive(Debug, Clone, Serialize)]
...
// ── v1.1.0 — IRImmune / IRReflex / IRHeal ────────────────────────────
...
pub struct IRImport {
    ...
    /// v2.76.0 — `true` iff the EMS resolved this import against a
    /// module in the compilation (the fields this paper-era struct always
    /// promised). Skipped when `false` so every pre-v2.76.0 program's IR
    /// JSON stays byte-identical (zero IR-SHA drift).
    #[serde(default, skip_serializing_if = "ir_import_unresolved")]
    pub resolved: bool,
    /// v2.76.0 — the resolved module's `.axi` interface hash.
...
pub struct IRContext {
    ...
    pub cite_sources: Option<bool>,
    /// v2.46.0 — the frame's declared cognitive timezone (IANA name).
    /// Elided when absent → pre-v2.46.0 context IR JSON stays byte-identical.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
/// v2.8.0 — one typed parameter of a tool's input schema (the IR mirror of
/// the AST `Parameter`). `type_name` is the flattened BASE type string
/// (`String`, `List<String>`); optionality (`T?`) is carried in `optional`, so
/// `required` is derivable with no parallel bool (v2.8.0 D1, single source of
/// truth). Lossless round-trip is gated in v2.8.0.
...
/// v2.8.0 — one bound keyword argument of a `use Tool(k = v, …)` call (the
/// IR mirror of `UseArgs::Named`). `value` is an expression string (the
/// frontend has no structured `Expr`). The runtime (v2.8.0) assembles these into
/// the structured JSON request body.
#[derive(Debug, Clone, Serialize, PartialEq)]
pub struct IRNamedArg {
    pub name: String,
    pub value: String,
    /// v2.10.0 — `"literal"` or `"reference"` (classified by `parse_let_atom`).
...
pub struct IRToolSpec {
    ...
    pub timeout: String,
    pub runtime: String,
    /// v2.69.0 — the `resource` this tool's channel runs on. Empty ⇒ legacy
    /// form. Skip-if-empty ⇒ every pre-v2.69.0 tool serializes byte-identically.
    #[serde(default, skip_serializing_if = "String::is_empty")]
    pub resource_ref: String,
    pub sandbox: Option<bool>,
    pub input_schema: Vec<String>,
    pub output_schema: String,
    /// v2.8.0 — the tool's typed INPUT SCHEMA (D1). Distinct from the v1.23.0
    ...
    /// contract the type-checker enforces (v2.8.0) and the runtime binds
    /// structured args against (v2.8.0). Empty for a schema-less tool (D5).
    pub parameters: Vec<IRToolParam>,
    /// v2.8.0 — the tool's declared OUTPUT type (D8), so `${Step.output}`
    ...
    /// requires: flat capability atoms, the `credential.grants` (v2.46.0) /
    /// `requires_capabilities` (v2.4.0) vocabulary. `axon-T956` enforces subset
    /// coverage at compile; the PCC `ScopeCoverage` witness rides the linked
    /// IR. Elided when empty — every pre-v2.77.0 tool serializes byte-identically
    /// (IR-SHA stability), and stored FlowIr hydrates via `default` (v2.4.0
    /// mirror discipline).
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    ...
    /// Elided when empty — every pre-v2.48.0 tool serializes byte-identically
    /// (IR-SHA stability).
    #[serde(default, skip_serializing_if = "String::is_empty")]
    pub secret: String,
    /// v2.49.0 — the `secret_partition:` parameter name whose runtime
    /// value is appended as a single segment to `secret` at dispatch
    /// (`selection_without_revelation`). Elided when empty, so every v2.48.0 and
    /// pre-v2.48.0 tool serializes byte-identically (IR-SHA stability). The
    ...
    /// v2.39.0 — Remote Hands. All three fields are `skip_serializing_if`
    /// so a program using none of them serialises **byte-identically** to the
    /// pre-v2.39.0 IR (the v2.33.0 IR-SHA / additive-only gate — no drift for the
    ...
    pub argv: Vec<String>,
    /// v2.40.0 — the cache-policy reference (a declared `cache` name, or the
    /// `none` opt-out sentinel). Empty ⇒ module-default-governed. Elided when
    /// empty (IR-SHA stable for cache-less programs).
    #[serde(default, skip_serializing_if = "String::is_empty")]
    pub cache: String,
    /// v2.52.0 — the closed-catalog web-acquisition config. `None` for
    /// every non-scrape tool, and `skip_serializing_if` so the entire
    /// pre-v2.52.0 corpus serialises byte-identically (the v2.33.0 IR-SHA /
    /// additive-only gate). Present ⇒ this tool acquires open-web content
    /// (born Untrusted, the design decision) and its `effect_row` carries `web`.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub scrape: Option<IRScrapeSpec>,
}

/// v2.52.0 — the IR mirror of `ast::ScrapeSpec`. Every field is
...
pub struct IRStep {
    ...
    /// v2.22.0 — the step's model-capability requirement (context window in
    /// tokens). `skip_serializing_if = Option::is_none` keeps every pre-v2.22.0 step's
    /// IR JSON byte-identical (no IR-SHA drift, the design decision); a legacy IR deserialises
    /// to `None` → the v2.22.0 resolver picks the backend default exactly as today.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub requires_context: Option<u32>,
    /// v2.46.0 — the step's declared cognitive timezone (IANA name). The
    /// runtime renders the run's captured instant in this zone into the step's
    /// cognitive context. Elided when absent → every pre-v2.46.0 step's IR JSON is
    /// byte-identical (no IR-SHA drift); legacy IR → `None` → no injection.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub now_tz: Option<String>,
    /// v2.83.0 — governance applications scoped to this step.
    /// Elided when empty so every pre-v2.83.0 program's IR JSON stays
    /// byte-identical (no IR-SHA drift — the v2.22.0/v2.46.0 discipline).
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub guards: Vec<IRStepGuard>,
    /// v2.83.0 — the PIX verbs written as statements in this step's body
    ...
    /// every pre-v2.83.0 program's IR JSON is byte-identical.
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub pix_ops: Vec<IRFlowNode>,
    /// v2.83.0 — a `stream<T> { … }` written in this step's body. NOT a
    ...
    /// Elided when absent, so every pre-v2.83.0 program's IR JSON is
    ...
    /// `the design plan` section 3.1's `perform Emit(response.token)` performs the step's own
    /// result. Elided when empty, so every pre-v2.87.0 program's IR JSON stays
    /// byte-identical (the v2.22.0/v2.46.0/v2.83.0 discipline).
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub performs: Vec<IREffectPerform>,
    pub body: Vec<serde_json::Value>,
}

/// v2.83.0 — one step-scoped governance application. For a
/// `mandate` guard, dispatch must run the v2.83.0 control loop over THIS
...
pub enum IRFlowNode {
    ...
    /// v2.87.0 — `perform E.Op(args)`.
    Perform(IREffectPerform),
    /// v2.87.0 — `resume(v)`, the one-shot continuation invocation (D2).
    Resume(IREffectResume),
    /// v2.87.0 — `abort(v)`.
    Abort(IREffectAbort),
    /// v2.87.0 — `forward E.Op(args)` (D12).
    ...
    /// v2.83.0 — `<Agent>(arg, …)`. The 46th variant, and the one that
    /// makes v2.83.0's bounded control loop reachable from source.
    AgentCall(IRAgentCall),
    Listen(IRListenStep),
    DaemonStep(IRDaemonStepNode),
    /// v1.6.0 — π-calc output prefix (Chan-Output / Chan-Mobility).
    Emit(IREmit),
    /// v2.46.0 — ephemeral-credential minting (attenuated, TTL-bounded).
    Mint(IRMintStep),
    /// v2.48.0 — mediated secret renewal (`rotation_without_revelation`).
    Rotate(IRRotateStep),
    /// v1.6.0 — capability extrusion (Publish-Ext).
    Publish(IRPublish),
    /// v1.6.0 — dual of publish (typed handle import).
    Discover(IRDiscover),
    ...
    /// v2.43.0 — the `warden` adversarial security-analysis block.
    Warden(IRWarden),
    /// v2.4.0 — the `quant` cognitive block (Hilbert-space projection).
    Quant(IRQuant),
    /// v2.4.0 — the `yield` measurement point inside a `quant` block.
    Yield(IRYield),
...
pub struct IRReasonStep {
    ...
    pub strategy: String,
    pub target: String,
    /// v2.83.0 — the evidence the deliberation reasons OVER, resolved
    /// against the flow bindings at dispatch. Elided when empty so every
    /// pre-v2.83.0 program's IR JSON is byte-identical (no IR-SHA drift).
    #[serde(default, skip_serializing_if = "String::is_empty")]
    pub given: String,
    /// v2.83.0 — the deliberation's question. The README writes it in
    /// every one of its sixteen `reason { … }` blocks and, before this cycle, it
    /// reached the model in none of them.
    #[serde(default, skip_serializing_if = "String::is_empty")]
    pub ask: String,
    /// v2.83.0 — the declared deliberation depth. Consumed by the
...
pub struct IRValidateStep {
    ...
    pub target: String,
    pub rule: String,
    /// v2.88.0 — the `type` that `against: <Schema>` names, RESOLVED at
    ...
    /// `mandate_specs` / `compute_specs` / `agent_specs` use. v2.83.0 records
    ...
    ///
    /// Elided when absent, so every pre-v2.88.0 program's IR JSON is
    /// byte-identical (the v2.22.0/v2.46.0/v2.83.0/v2.87.0 discipline).
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub resolved_schema: Option<Box<IRType>>,
    /// v2.88.0 — `if confidence < θ -> refine(max_attempts: N)`, attached to
...
/// v2.88.0 — the lowered confidence guard: a floor over the validation's CSR
...
pub struct IRWeaveStep {
    ...
    pub style: String,
    /// v2.83.0 — the parts the synthesis must contain
    /// (`include: [summary, risks, recommendations]`). Elided when empty so
    /// every pre-v2.83.0 program's IR JSON is byte-identical (no IR-SHA drift).
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
...
pub struct IRUseToolStep {
    ...
    pub argument: String,
    /// v2.8.0 — the bound keyword args of `use Tool(k = v, …)` (W1: the
...
/// v2.26.0 — the lowered form of a pure expression (`Expr`). Carried in the
...
/// the runtime evaluator (v2.26.0) matches on them. Externally-tagged by `kind`.
#[derive(Debug, Clone, Serialize)]
...
pub enum IRExpr {
    ...
    /// v2.26.0 — field access on a non-reference base (the JSONB seam).
    Field {
        base: Box<IRExpr>,
        field: String,
    },
    ...
    /// v2.83.0 — `let <name> = <value>` scoped over `<body>`. The lowered
...
/// v2.26.0 — a literal inside an [`IRExpr`].
#[derive(Debug, Clone, Serialize)]
...
pub struct IRConditional {
    ...
    /// v2.26.0 — the lowered expression form, present only for conditions
    /// the legacy triple cannot express. `skip_serializing_if` keeps the IR
    /// JSON (and its SHA) byte-identical for every pre-v2.26.0 program.
    #[serde(skip_serializing_if = "Option::is_none")]
...
pub struct IRLetBinding {
    ...
    pub target: String,
    pub value: String,
    /// v1.12.0 — preserves parser tokenization intent.
    /// One of "literal" | "reference" | "expression".
    pub value_kind: String,
    /// v2.26.0 — the lowered expression form of the value, present only for
...
/// v1.14.0 — `break` keyword IR node. Payload-free (the runner
...
/// v1.14.0 — `continue` keyword IR node. Same shape as
...
pub struct IRParallelBlock {
    ...
    pub source_line: u32,
    pub source_column: u32,
    /// v2.15.0 — the concurrent branches lowered from the AST `par { … }`.
    ...
    /// payload-free / empty `par` serializes byte-identically to the pre-v2.15.0
...
/// v2.41.0 — the compiled Directed Creative Synthesis block. This IS the
/// "structured IR metadata that the runtime executes as an orchestrated
/// pipeline" the README always claimed — pre-v2.41.0 it carried only a source
...
/// v2.65.0 — the proof-carrying derivative. `original` is the
/// differentiated `let`'s expression; `derivatives[i]` = ∂original/∂wrt[i],
/// SIMPLIFIED — computed at compile time by the symbolic
...
pub struct IRFocusStep {
    ...
    pub source_column: u32,
    pub expression: String,
    /// v2.63.0 — the data-plane `where:` (the design decision; empty ⇒ no filter).
    pub where_expr: String,
    /// v2.63.0 — π: projected columns (empty ⇒ all).
    pub select: Vec<String>,
    /// v2.63.0 — result binding (`as:`; empty ⇒ the dataspace name).
    pub output: String,
}
...
pub struct IRAssociateStep {
    ...
    /// v2.63.0 — result binding (`as:`; empty ⇒ `<L>_<R>`).
    pub output: String,
}
...
pub struct IRAggregateStep {
    ...
    pub alias: String,
    /// v2.63.0 — the closed aggregate catalog entries, raw
...
pub struct IRExploreStep {
    ...
    /// v2.63.0 — result binding (`as:`; empty ⇒ the target).
    pub output: String,
}
...
pub struct IRIngestStep {
    ...
    pub source: String,
    pub target: String,
    /// v2.63.0 — the declared wire format (`csv` | `json`, validated
    /// by axon-T929). Empty only in a pre-108.c artifact — the runtime
    /// handler fails CLOSED on it.
    pub format: String,
    /// v2.63.0 — bounds enforced on the raw byte stream BEFORE
    /// parsing (v2.54.0). `None` ⇒ the engine's conservative defaults.
    pub max_bytes: Option<u64>,
    pub max_rows: Option<u64>,
}

/// v2.69.0 — a shield's compiled BREACH POLICY, resolved onto the nodes
...
/// site could miss). Before v2.69.0 the whole `on_breach:` catalog
...
pub struct IRShieldApplyStep {
    ...
    pub target: String,
    pub output_type: String,
    /// v2.69.0 — the named shield's breach policy, resolved at lowering.
    /// `None` ⇒ the shield declares no `on_breach:` (halt, the fail-closed
    /// default) — and every pre-v2.69.0 program serializes byte-identically.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub breach_policy: Option<IRBreachPolicy>,
    /// v2.89.0 — the named shield's declared `scan:` list, resolved at
    /// lowering beside `breach_policy` (same Phase 0 pre-pass).
    ///
    /// It is the SUBJECT of the v2.89.0 refusal. A shield that declares
    ...
    /// the `warden` shape (v2.67.0 F12): a clean-looking result for something
    /// never examined. The runtime now refuses instead.
    ///
    /// Empty ⇒ a shield with no declared scan, where v2.83.0's argument still
...
pub struct IRStreamBlock {
    ...
    pub source_line: u32,
    pub source_column: u32,
    /// v2.67.0 — the block's lowered body. ADDITIVE: `skip_serializing_if`
    ...
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub body: Vec<IRFlowNode>,
    /// v2.83.0 — the `<T>` in `stream<T>`: the CHUNK type. Elided when the
    /// block was written without one.
    #[serde(default, skip_serializing_if = "String::is_empty")]
    pub chunk_type: String,
    /// v2.83.0 — `on_chunk: { … }`, lowered. Run once per chunk with the
    /// chunk bound as `chunk`.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub on_chunk: Option<Box<IRStep>>,
    /// v2.83.0 — `on_complete: { … }`, lowered. Run once, after the source
    /// closes, with the accumulated stream bound as `complete`.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub on_complete: Option<Box<IRStep>>,
    /// v2.83.0 — `on_error: { … }`, lowered. Run when the SOURCE fails,
...
// ── v2.87.0 — algebraic effects, lowered ──────────────────────────────────
//
...
// nothing. the design decision puts the effect machine in the DISPATCHER, whose alphabet is
...
pub struct IREffectHandle {
    ...
    /// The `in { … }` block — ORDINARY flow nodes. This field is the design decision.
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub body: Vec<IRFlowNode>,
...
/// form — the IR generator resolves it against the declared catalog.
...
pub struct IRNavigateStep {
    /// v2.83.0 — per-navigation depth override → `NavConfig.d_max`.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    /// v2.13.0 — MDN corpus-graph navigation: the seed document (`from:`).
    #[serde(default, skip_serializing_if = "String::is_empty")]
    pub seed: String,
    /// v2.13.0 — MDN navigation budget (`budget:` = max documents).
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub budget: Option<i64>,
    /// v2.17.0 (Q2) — column-scope filter for a `corpus from axonstore`. A raw
    ...
    /// v1.32.0 filter compiler resolves `${name}` → `$N` bind params (injection-
...
pub struct IRAgentCall {
    ...
    /// (the v2.67.0 compute doctrine — an empty catalog runs nothing).
    pub agent_name: String,
    /// Positional v2.83.0 subjects. Resolved against the flow bindings at
...
pub struct IRListenStep {
    ...
    pub source_column: u32,
    pub channel: String,
    /// v1.6.0 D4 — true ⇒ `channel` is a declared
    /// `IRChannel` ref; false ⇒ legacy string topic.
    pub channel_is_ref: bool,
    pub event_alias: String,
    /// v2.4.0 — the handler body's lowered flow-steps, executed per event /
    /// scheduled tick by the v2.4.0 runtime. `skip_serializing_if` keeps a
    /// bodyless `listen`'s JSON byte-identical to the pre-v2.4.0 shape (D8).
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub body: Vec<IRFlowNode>,
}
...
pub struct IRPersistStep {
    ...
    /// v1.30.0 — declared `{ col: value }` field block (value
...
pub struct IRRetrieveStep {
    ...
    pub where_expr: String,
    pub alias: String,
    /// v2.21.0 — `order_by:` clause (raw `"col [asc|desc], …"`).
    /// `skip_serializing_if` empty so a store that doesn't order never
    /// perturbs the serialized IR bytes (the v2.4.0 brief-#33 /
    /// [[feedback-boot-hydrate-self-heal]] no-drift discipline).
    #[serde(skip_serializing_if = "String::is_empty", default)]
    pub order_by: String,
    /// v2.21.0 — `limit:` clause (raw `"100"` or `"${max}"`).
    #[serde(skip_serializing_if = "String::is_empty", default)]
    pub limit_expr: String,
    /// v2.33.0 — `aggregate:` clause (raw, closed catalog: `count` /
    /// `sum(col)` / `avg(col)` / `min(col)` / `max(col)`).
    /// `skip_serializing_if` empty so a non-aggregating retrieve never
    /// perturbs the serialized IR bytes (the same v2.21.0 no-drift
    /// discipline — zero IR-SHA drift for existing programs).
    #[serde(skip_serializing_if = "String::is_empty", default)]
    pub aggregate: String,
    /// v2.33.0 — `group_by:` clause (raw `"col, col2"`).
    #[serde(skip_serializing_if = "String::is_empty", default)]
    pub group_by: String,
    /// v2.40.0 — `cache:` reference (a declared `cache` name). Empty ⇒
...
pub struct IRMutateStep {
    ...
    pub store_name: String,
    pub where_expr: String,
    /// v1.30.0 — declared `{ col: value }` SET assignments (value
    /// expressions kept raw; interpolated at runtime). Empty ⇒ the
...
/// v2.43.0 — IR for the `warden` adversarial-analysis block. Carries the
/// target, the mandatory `scope_ref`, and the recursively-lowered body so the
/// enterprise engine (v2.43.0) can drive the analysis and v2.43.0 can enforce the
...
/// v2.43.0 — IR for a `scope` authorization-policy declaration.
#[derive(Debug, Clone, Serialize)]
pub struct IRScope {
    pub node_type: &'static str,
    pub source_line: u32,
...
/// v2.4.0 — IR for the `quant` cognitive block (Hilbert-space projection).
...
pub struct IRQuant {
    ...
    /// v2.23.0 — data re-uploading layers L (`None`/`1` = no re-uploading).
...
/// v2.27.0 — the lowered temporal execution-window guard. The runtime
/// (v2.27.0) evaluates `is_in_window(now, tz, allow)`; the daemon binding +
/// coalesced defer ledger are v2.27.0.
#[derive(Debug, Clone, Serialize)]
pub struct IRWindow {
    pub node_type: &'static str,
    pub source_line: u32,
    ...
    /// v2.27.0 — excluded dates (holidays): ISO `YYYY-MM-DD` literals. A tick
...
/// v2.27.0 — one allowed day/hour span.
#[derive(Debug, Clone, Serialize)]
...
/// v2.28.0 — the `budget { … }` linear-effect rate limit lowered to IR. Each
/// quota gates a declared tool's dispatch on a renewable token bucket (the v2.28.0
...
pub struct IRBudget {
    ...
    pub source_line: u32,
    pub source_column: u32,
    /// v2.69.0 — the name of a **top-level** `budget`. Empty ⇒ the anonymous
    /// daemon-attached form. Skip-if-empty ⇒ every pre-v2.69.0 program serializes
    /// byte-identically (IR-SHA stability).
    #[serde(default, skip_serializing_if = "String::is_empty")]
    pub name: String,
    pub quotas: Vec<IRBudgetQuota>,
    /// `block` (fail-closed) | `defer` (reschedule via the v2.27.0 defer ledger) |
    /// `shed` (skip the call). An omitted policy lowers to `block` (the safe
    /// fail-closed default).
    pub on_exhausted: String,
}

/// v2.28.0 — one quota: `<kind>: <limit> per <period> on Tool(<effect>)`.
#[derive(Debug, Clone, Serialize)]
...
pub struct IRShield {
    ...
    pub severity: String,
    pub quarantine: String,
    /// section 8.2.h.3 — Python emits concrete 0, not null. AST keeps `Option<i64>`
    ...
    // IRShield doesn't emit it. Hidden from JSON output for section 8.2.h parity.
    #[serde(skip)]
    pub taint: String,
    /// ESK — covered regulatory classes for this shield.
    pub compliance: Vec<String>,
    /// v2.34.0 — egress signing algorithm (`hmac_sha256`; empty = the
    /// shield does not sign). Elided from JSON when empty so every pre-v2.34.0
...
/// v2.12.0 — the audit-chain (`ledger`) IR node. Same shape as [`IRPix`]
...
pub struct IRCorpus {
    ...
    /// v2.13.0 — typed weighted edges. Non-empty ⇒ this corpus is an MDN
    /// graph `C = (D, R, τ, ω, σ)`; the runtime builds an `mdn::Corpus` from it.
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub relations: Vec<IRCorpusRelation>,
    /// v2.13.0 — `adaptive: true` enables the memory endofunctor on this
    /// corpus's navigations.
    #[serde(default, skip_serializing_if = "std::ops::Not::not")]
    pub adaptive: bool,
    pub mcp_server: String,
    pub mcp_resource_uri: String,
    /// v2.14.0 — when present, this is a DYNAMIC store-sourced MDN graph: the
    ...
    /// (per-tenant, growing). Absent ⇒ the static v2.13.0 corpus (byte-identical IR).
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub store_source: Option<IRCorpusStoreSource>,
}

/// v2.13.0 — a lowered MDN corpus-graph edge `(from, to, τ, ω)`.
#[derive(Debug, Clone, Serialize)]
...
/// v2.14.0 — the lowered store-mapping of a dynamic, `axonstore`-sourced MDN
...
/// weighted edges. The runtime (v2.14.0) reads these stores tenant-scoped at
...
/// v2.63.0 — one compiled dataspace column. `column_type` is the
/// CANONICAL catalog name (`Text` / `Int` / `Float` / `Bool` /
/// `Timestamp` / `Json`) — aliases are resolved at IR generation, so
/// every downstream consumer (the engine's deploy hook, the v2.63.0 PCC
...
pub struct IRDataspace {
    ...
    pub source_column: u32,
    pub name: String,
    /// v2.63.0 — the typed columnar schema (canonical type names).
...
pub struct IRMandate {
    ...
    /// v2.83.0 — the declared stability hypotheses `(D, L)`, carried to the
...
pub struct IRCompute {
    ...
    pub name: String,
    pub shield_ref: String,
    /// v2.67.0 — typed parameters. ADDITIVE (`skip_serializing_if`), so every
    ...
    #[serde(default, skip_serializing_if = "String::is_empty")]
    pub return_type: String,
    /// v2.67.0 — the lowered v2.26.0 expression the runtime evaluates natively.
...
pub struct IRDaemon {
    ...
    /// v2.28.0 — the `budget { … }` linear-effect rate limit. `None` ⇒ no
    ...
    pub max_time: String,
    pub max_cost: Option<f64>,
    /// v2.4.0 — the daemon's `listen` listeners (channel + alias + handler
    /// body). Pre-v2.4.0 these were DROPPED at lowering (the IR daemon carried no
    /// listeners at all); now they survive so the v2.4.0 runtime can mount + run
    /// them and the v2.4.0 enterprise supervisor can extract them per-tenant.
    /// `skip_serializing_if` keeps a listenerless daemon's JSON unchanged (D8).
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub listeners: Vec<IRListenStep>,
    /// v2.4.0 — the capability scope the daemon's runs are confined to
...
/// v2.42.0 — a compiled `savant` (long-horizon autonomous research
/// primitive). A governed orchestrator: the IR carries the declared surface so
/// the enterprise active-inference engine (v2.42.0+) can drive the FEP loop, and
/// the v2.42.0 checker can bind `memory` to a declared store, `budget` to a v2.28.0
/// linear budget, and the body to a v2.36.0 interruptible session.
#[derive(Debug, Clone, Serialize)]
pub struct IRSavant {
    pub node_type: &'static str,
    pub source_line: u32,
...
/// v2.42.0 — the compiled `cognition { … }` sub-block (active-inference
/// engine parameters).
#[derive(Debug, Clone, Serialize)]
pub struct IRSavantCognition {
    /// `standard | deep | hyper` — HRR dimensionality tier (validated v2.42.0).
    pub depth: String,
    /// Expected-Free-Energy convergence bound (`> 0`, v2.42.0). `None` ⇒ default.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub entropic_threshold: Option<f64>,
    /// `low | med | high` — explore/exploit balance (validated v2.42.0).
    pub divergence: String,
}

/// v2.42.0 — the compiled `memory { … }` sub-block (retention binding).
#[derive(Debug, Clone, Serialize)]
pub struct IRSavantMemory {
    /// A declared `memory`/`corpus` name (resolved v2.42.0). Empty ⇒ ephemeral.
    pub backend: String,
...
/// v2.42.0 — the compiled `budget { … }` sub-block (compute ceiling, bound to
/// a v2.28.0 linear budget in v2.42.0).
#[derive(Debug, Clone, Serialize)]
pub struct IRSavantBudget {
    /// Hard ceiling on FEP-loop iterations before the savant pauses.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub max_iterations: Option<i64>,
    /// Hard ceiling on `synth` (v2.42.0) tool-creation events per mandate.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub max_tool_synth: Option<i64>,
}

/// v2.42.0 — a compiled `mandate <Name> { … }` sub-block (one research goal).
#[derive(Debug, Clone, Serialize)]
...
// ── v2.53.0 — Native Document Synthesis IR ─────────────────────────────────

/// v2.53.0 — a compiled document declaration. The runtime `DocumentRenderer`
/// tool serialises this to deterministic OOXML bytes (v2.53.0); the
/// `DocumentProvenanceSoundness` PCC class (v2.53.0) re-derives the barrier from
...
pub struct IRDocument {
    ...
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub effect_row: Vec<String>,
    /// v2.53.0 — the enclosing `epistemic { mode: … }` at compile time
...
/// v2.53.0 — one compiled document block. `fields` preserves declaration
...
/// v2.53.0 — a compiled `(field, value)` pair. `kind` tags the value shape
...
// ── v2.60.0 — Governed CRM Delivery IR ─────────────────────────────────────

/// v2.60.0 — a compiled delivery declaration. The runtime delivery dispatch
...
/// v2.66.0 — the compiled `notify` declaration. `epistemic_mode`
/// records the enclosing vouch (the v2.53.0/v2.60.0 discipline) so T933
...
pub struct IRNotify {
    ...
    pub name: String,
    pub channel: String,
    /// The v2.48.0 secret-class ref (the recipient value NEVER rides the IR).
...
pub struct IRDeliver {
    ...
    /// the boundary — the T920 barrier's subject.
    #[serde(default, skip_serializing_if = "String::is_empty")]
    pub provenance: String,
    /// The per-tenant credential key (v2.48.0 custody — resolved at dispatch, never
    /// in cognition).
    #[serde(default, skip_serializing_if = "String::is_empty")]
    pub secret: String,
...
/// v2.60.0 — one compiled delivery operation. `fields` preserves declaration
...
/// v2.42.0 — a compiled `synth` dynamic tool-synthesis policy. The IR carries
/// the safety envelope so the enterprise Extism/WASM executor (v2.42.0) enforces
...
/// v2.5.0 — one compiled member of an `extension`. For `effects`
/// the `name` is a provenance base; `default_confidence` is a CEILING
/// (v2.5.0 tainted-overriding). Metadata is elided from JSON when absent
...
/// v2.5.0 — a compiled `extension` declaration. Rides in the IR (and,
...
/// type-checker (v2.5.0) enforces the closed category + no-shadowing +
...
pub struct IRAxonStore {
    ...
    /// global-pool fallback. v2.67.0's census established this, and it is why
    /// v2.67.0 is delicate: moving authority to `resource` moves it *away* from
    ...
    ///
    /// v2.67.0: still parsed, but **deprecated in favour of [`Self::resource_ref`]**,
    /// and a store declared this way is INELIGIBLE for `lease` / `observe` /
    /// `reconcile`. *You cannot govern what you did not declare.*
    pub connection: String,
    /// v2.67.0 — the `resource` this store runs on (`axonstore U { resource: Db }`).
    ///
    /// When present, the store DERIVES its DSN (`resource.endpoint`), its pool
    /// size (`resource.capacity` — a knob that did not exist before v2.67.0; the
    ...
    ///
    /// Empty ⇒ the legacy un-resourced form. Skip-if-empty ⇒ every pre-v2.67.0
    /// store serializes byte-identically (the v2.48.0 `class` precedent).
    #[serde(default, skip_serializing_if = "String::is_empty")]
    pub resource_ref: String,
    pub confidence_floor: Option<f64>,
    ...
    /// pre-v2.48.0 store serializes byte-identically (IR-SHA stability).
    #[serde(default, skip_serializing_if = "String::is_empty")]
    pub class: String,
    ...
    /// v1.31.0 / v1.31.0 type-checker proves every store reference against
    /// this when present. v2.48.0: for a `backend: secrets` store the
...
/// v1.31.0 (D1) — IR mirror of [`crate::store_schema::StoreColumnSchema`].
...
/// v1.31.0 (D1) — IR mirror of [`crate::store_schema::StoreColumn`].
...
pub struct IRStoreColumn {
    ...
    #[serde(default, skip_serializing_if = "is_false")]
    pub identity: bool,
    /// v2.26.0 (D1) — `true` iff the column carries the `index`
    ...
    #[serde(default, skip_serializing_if = "is_false")]
    pub indexed: bool,
    /// v2.26.0 (D1) — the OPTIONAL `Json<T>` shape-lens struct name on a
    ...
    /// to a declared struct `type` — the v2.26.0/v2.26.0 lens well-formedness
...
pub struct IRAxonEndpoint {
    ...
    pub output_type: String,
    pub shield_ref: String,
    /// section 8.2.h.3 — Python emits concrete `0`; AST stays `Option<i64>`.
    pub retries: i64,
    pub timeout: String,
    /// ESK — κ regulatory class on the boundary.
    pub compliance: Vec<String>,
    ...
    /// v1.32.0 (D2) — Query parameters from the inline
    ...
    /// v2.4.0 — capability scopes the request bearer must hold
    /// (the `requires: [scope.dotted]` declaration, v1.23.0). Mirror
    ...
    /// pre-v2.4.0 IR-JSON snapshot (no `requires:`) stays byte-identical
    /// (D5 backwards-compat — empty key parses back to empty Vec).
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub requires_capabilities: Vec<String>,
    /// v2.38.0 — the `cors: <Name>` reference, or `""` when absent
    /// (the design decision: no CORS headers, ever). NEW field on an EXISTING struct —
    ...
    /// to pre-v2.38.0 (zero IR-SHA drift — the standing v2.33.0 discipline).
    #[serde(default, skip_serializing_if = "String::is_empty")]
    pub cors_ref: String,
    /// v2.44.0 — the explicit authorization-coverage opt-out lowered into
    /// the IR so the enterprise runtime (v2.44.0) and the PCC
    /// `AuthorizationCoverage` witness (v2.44.0) can read it. `false` (the
    /// default + the common case) elides from JSON via `is_false` so a
    /// pre-v2.44.0 IR-JSON snapshot stays byte-identical (zero IR-SHA drift —
    /// the standing v2.33.0 discipline).
    #[serde(default, skip_serializing_if = "is_false")]
    pub public: bool,
}

// ── v1.6.0 — Mobile Typed Channels IR ───────────────────────────────
...
/// their containing flow/listener (paper section 3 + section 4 — π-calc prefix
...
pub struct IRChannel {
    ...
    pub shield_ref: String,
    /// v2.34.0 — non-empty ⇒ some `publish <this> within <Shield>` site
    ...
    /// JSON when empty (zero IR-SHA drift for pre-v2.34.0 programs).
    #[serde(skip_serializing_if = "String::is_empty")]
    pub egress_sign: String,
}

/// v2.3.0 — compiled typed WebSocket transport. `protocol` names the
...
/// crediting/decrementing the window per section 4.2 of the paper.
#[derive(Debug, Clone, Serialize)]
pub struct IRSocket {
    pub node_type: &'static str,
    pub source_line: u32,
...
/// v2.37.0 — compiled outbound vendor connection (the client dual of
...
pub struct IRUpstream {
    ...
    pub role: String,
    pub resolve: String,
    /// v2.69.0 — the `resource` this upstream's channel rides. When set,
    /// `resolve` above was DERIVED from the resource's `endpoint` at LOWERING
    /// (the v2.69.0 shield-egress discipline: derivation stamped into the
    ...
    /// when empty — every pre-v2.69.0 upstream serializes byte-identically.
    #[serde(default, skip_serializing_if = "String::is_empty")]
    pub resource_ref: String,
    /// v2.69.0 — **max concurrent connection INSTANCES** of this upstream
    ...
    /// the same in-memory/per-process bound v2.69.0 documented for tools.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub capacity: Option<i64>,
    pub secret: String,
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub backpressure_credit: Option<i64>,
    /// v2.37.0 — the `Preset@vN` reference this declaration was expanded from
    /// (provenance for the compliance reviewer); absent for hand-written ones.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub preset: Option<String>,
}

/// v2.37.0 — one compiled `map:` projection rule.
#[derive(Debug, Clone, Serialize)]
...
/// v2.37.0 — compiled reconnect policy (all three fields required by the
...
/// checker re-derives the same closed catalogs at deploy time (v2.38.0,
...
/// v2.40.0 — compiled `cache` policy. The checker (v2.40.0) re-derives the
...
/// with no `cache` never emits a `caches` key (IR-SHA stable, v2.33.0).
#[derive(Debug, Clone, Serialize)]
pub struct IRCache {
    pub node_type: &'static str,
    pub source_line: u32,
...
/// handle (second-order mobility, paper section 3.2); the runtime dispatches
...
pub struct IREmit {
    ...
    /// v2.69.0 (owed) — the σ-shield the target `channel` declares
    ...
    /// pre-v2.69.0 emit: `skip_serializing_if` elides it → zero IR-SHA drift for
    /// programs whose channels declare no shield).
    #[serde(default, skip_serializing_if = "String::is_empty")]
    pub shield_ref: String,
    /// v2.69.0 — the σ-shield's breach policy, resolved at lowering beside
    /// `shield_ref` (same Phase 0 pre-pass). `None` ⇒ no `on_breach:` declared
    /// (halt, the fail-closed default); elided → zero IR-SHA drift.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub breach_policy: Option<IRBreachPolicy>,
    /// v2.89.0 — the σ-shield's declared `scan:` list, resolved at lowering
...
/// v2.46.0 — compiled `credential` contract. The TTL is carried as
...
/// v2.46.0 — compiled `mint <Credential> as <binding>` step. The runtime
...
/// v2.48.0 — compiled `rotate <SecretsStore> [where "…"] with <Tool>
...
pub struct IRRotateStep {
    ...
    /// v2.21.0 metadata filter; empty = the whole class (elided from the wire).
    #[serde(default, skip_serializing_if = "String::is_empty")]
    pub where_expr: String,
    pub tool_ref: String,
    pub binding: String,
...
/// Compiled publish step — capability extrusion (Publish-Ext, paper section 4.3).
#[derive(Debug, Clone, Serialize)]
pub struct IRPublish {
    ...
    pub channel_ref: String,
    pub shield_ref: String,
    /// v2.34.0 — the referenced shield's `sign:` algorithm, RESOLVED at
    ...
    /// Elided from JSON when empty (zero IR-SHA drift for pre-v2.34.0 programs).
    #[serde(skip_serializing_if = "String::is_empty")]
    pub sign: String,
}

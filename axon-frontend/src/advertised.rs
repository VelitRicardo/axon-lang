...
pub const ADVERTISED: &[(&str, RuntimeStatus)] = &[
    ...
    ("stream", Partial { gap: "§119.n made the PUBLISHED surface real: `stream<T> { on_chunk: … on_complete: … }` parses at both the step-body and flow positions (it was a hard parse error at flow level and a SILENT DISCARD in a step body), the `<T>` chunk type reaches the IR instead of being eaten by the argument-skip loop, and each arm is a step body so its `output:` lands. BOTH chunk sources are wired: a streaming TOOL bound with `apply:` (README block 15's own shape — the handler rides `unified_stream_handler`, so it sees the chunks the declared `backpressure:` policy let through, never the raw source), and the step's own generation (fase_23 §3.7's `stream<Token>` case, hooked into `drain_direct`). Neither has a private drain, so a handler cannot drift from the stream it handles. `on_complete` binds the accumulation as `complete` and its output becomes the step's. An `apply:` that is not a registered streaming tool is REFUSED, never demoted to the LLM path — demoting would stream something the author never named (§112). Gates: axon-frontend/tests/fase119_n_stream_handlers.rs (9, source→AST→IR) + axon-rs/tests/fase119_n_stream_dispatch.rs (10, source→dispatch, incl. README block 15 VERBATIM executing); 5/5 mutations killed. THE GAP: `stream<τ>` is NOT desugared to `effect _StreamBuiltin<T>` + `handle … in …`, so fase_23's D8 claim of one-shot delimited continuations under this primitive is still future work and the algebraic `perform Stream.Yield` bridge stays orthogonal. `on_chunk` is also strictly sequential — a handler slower than the producer applies backpressure to the whole step, which is correct but undocumented as a performance property, and there is no `on_error` arm for a source that terminates with a fault" }),

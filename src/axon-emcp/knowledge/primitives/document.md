...
A `document` leaves the program, so it is a trust boundary. If the type named by
...
of those classes. `axon-T1222` refuses it otherwise, the same coverage rule an
`axonendpoint` lives under (`axon-T957`).

```text
axon-T1222 document 'X' carries regulated data (kappa = {HIPAA}) out of the

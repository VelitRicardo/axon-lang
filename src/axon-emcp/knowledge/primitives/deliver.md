...
A `delivery` leaves the program, so it is a trust boundary. If the type named by
...
of those classes. `axon-T1223` refuses it otherwise, the same coverage rule an
`axonendpoint` lives under (`axon-T957`).

```text
axon-T1223 delivery 'X' carries regulated data (kappa = {HIPAA}) out of the

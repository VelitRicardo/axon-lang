//! §Fase 119.m.2 — **the blame RESPONSIBILITY axis, and the third catalog that
//! did not get created.**
//!
...
//!
//! §0.2 framed this as "implement the paper's axis alongside `BlameKind`". Asked
//! the §0.4 question first — *what already reads this?* — and the axis turned out
...
//! party as a PARAMETER and default to `None`. §3 pins that.
...
// ── §1 — ONE axis: `emcp::Blame` projects onto `BlameParty` ──────────────────
...
// ── §2 — the paper's assignments, where it makes them BY NAME ────────────────
...
// ── §3 — the axes are ORTHOGONAL, and here is the evidence ───────────────────
...
// ── §4 — §39's D11 wire contract is UNTOUCHED ───────────────────────────────

/// An envelope with no party must serialise exactly as it did before §119.m.2,
...
fn s4_an_absent_party_is_elided_from_the_wire() {
    ...
    assert!(
        j.get("party").is_none(),
        "the key must be ABSENT, not null — §39's D11 wire stays byte-identical \
...
/// A pre-§119.m.2 envelope — no `party` key at all — must still deserialise.
#[test]

//! v1.31.0 — Cardinality Coverage Complete.
//!
...
//!
//! section 1 — D1 expand: `for x in xs { … }` tail + singular output → T9XX
//! section 2 — D1 expand: `if/else` branches DISAGREE → W003 emitted
//! section 3 — D1 expand: `if/else` branches AGREE on Singular → no warning
//! section 4 — D1 expand: `if/else` branches AGREE on Plural → no warning
//! section 5 — D3 bilateral: Singular tail + `output: List<T>` → T9XX
//! section 6 — D5 Stream: `output: Stream<T>` + retrieve tail → T9YY
//! section 7 — D5 Stream: `output: Stream<T>` + Stream step tail → no error
//! section 8 — D5 bilateral Stream: `output: T` + Stream tail → T9YY
//! section 9 — D6: `output: Any` accepts disagreed branches (degraded surface)
//! v1.4.0 — D2 runtime: BodyValidationError exposes cardinality fields
//! v1.4.0 — D4 OWASP-safe default + verbose opt-in semantics
//! v1.4.2 — STATIC grep S: Cardinality enum + infer_flow_tail_cardinality
...
// ── section 1 — D1 expand: for-tail + singular output ──────────────────────
...
fn s1_d1_for_tail_with_singular_output_emits_t9xx() {
    ...
    assert!(
        !t9xx.is_empty(),
        "v1.31.0 section 1 — a for-loop tail (always plural) + singular \
...
// ── section 2 — D1 expand: if/else branches DISAGREE → W003 ────────────────
...
fn s2_d1_if_else_disagree_emits_w003() {
    ...
    assert!(
        !w003.is_empty(),
        "v1.31.0 section 2 — if/else branches disagreeing on cardinality \
         (one Singular, one Plural) MUST emit `axon-W003 \
         cardinality_disagreement_in_branches`. All errors: {errs:#?}"
    );
}

// ── section 3 — D1 expand: if/else branches AGREE Singular → no warning ────
...
fn s3_d1_if_else_agree_singular_passes() {
    ...
    assert!(
        warns.is_empty(),
        "v1.31.0 section 3 — if/else branches AGREEING on Singular + \
         singular output is well-formed. No cardinality diagnostic \
         should fire. Errors: {warns:#?}"
    );
}

// ── section 4 — D1 expand: if/else branches AGREE Plural → no warning ──────
...
fn s4_d1_if_else_agree_plural_passes() {
    ...
    assert!(
        warns.is_empty(),
        "v1.31.0 section 4 — if/else branches AGREEING on Plural + \
         List<T> output is well-formed. No cardinality diagnostic \
         should fire. Errors: {warns:#?}"
    );
}

// ── section 5 — D3 bilateral: Singular tail + List<T> output → T9XX ────────
...
fn s5_d3_singular_tail_with_list_output_emits_t9xx() {
    ...
    assert!(
        ...
        "v1.31.0 section 5 — singular flow tail + `output: List<T>` MUST \
         emit `axon-T9XX` with the bilateral hint. All errors: \
         {errs:#?}"
    );
}

// ── section 6 — D5 Stream: retrieve tail + Stream<T> output → T9YY ─────────
...
fn s6_d5_stream_output_with_retrieve_tail_emits_t9yy() {
    ...
    assert!(
        !t9yy.is_empty(),
        "v1.31.0 section 6 — `output: Stream<T>` + non-stream flow tail \
...
// ── section 7 — D5 Stream: Stream step tail + Stream output → no error ─────
...
fn s7_d5_stream_output_with_stream_step_passes() {
    ...
    assert!(
        t9yy.is_empty(),
        "v1.31.0 section 7 — `output: Stream<T>` + step with `output: \
         Stream<T>` is well-formed. No T9YY should fire. Errors: \
         {t9yy:#?}"
    );
}

// ── section 8 — D5 bilateral Stream: T output + Stream tail → T9YY ─────────
...
fn s8_d5_singular_output_with_stream_tail_emits_t9yy() {
    ...
    assert!(
        !t9yy.is_empty(),
        "v1.31.0 section 8 — singular output `T` + Stream<T> tail MUST \
         emit `axon-T9YY` (the bilateral arm of D5). All errors: \
         {errs:#?}"
    );
}

// ── section 9 — D6: output: Any accepts disagreed branches ─────────────────
...
fn s9_d6_any_output_accepts_disagreed_branches() {
    ...
    assert!(
        w003.is_empty(),
        "v1.31.0 section 9 — `output: Any` is the documented degraded \
...
fn s11_d4_verbose_d5_hint_env_var_recognized() {
    ...
    assert!(
        ...
        "v1.31.0 v1.4.0 (D4) — `AXON_VERBOSE_D5_HINT` env var MUST be \
    ...
    for token in ["\"1\"", "\"true\"", "\"yes\"", "\"on\""] {
        assert!(
            src.contains(token),
            "v1.31.0 v1.4.0 (D4) — verbose hint truthy alphabet MUST \
...
// ── v1.4.2 — S STATIC grep: surface declarations present ──────────────
...
fn s12_static_grep_cardinality_surface_present() {
    ...
    assert!(
        ...
        "v1.31.0 S — `pub(crate) enum Cardinality` MUST be \
    ...
    for variant in [
        ...
        assert!(
            ...
            "v1.31.0 S — `Cardinality::{variant}` MUST be \

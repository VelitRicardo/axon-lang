...
//! v1.24.0 — Tokenizer-aware fallback chunking (D9, opt-in).
//!
...
// ─── section 1 — D4: ASYNC path is unaffected by the flag ─────────────────
...
// ─── section 2 — Flag CONTROL semantics ───────────────────────────────────
...
// ─── section 3 — Chunker correctness over canonical inputs ───────────────

// v2.87.0 — the two assertions below need the C TOKENISER KERNEL, and say
// so instead of failing as if the chunker were broken.
//
// `bpe_chunk_text` returns `Vec::new()` when `axon_csys::tokens::cl100k_base()`
// is unavailable, and v2.81.0 made `axon-csys`'s native kernels opt-in
...
// from "no kernel" — the v2.67.0 kernel defect shape (*if the evidence is missing,
// substitute the belief*). It is left alone deliberately: v2.83.0 recorded
...
// ─── section 4 — Tokenizer is reachable via the `axon` public re-export ──
...
// ─── section 5 — Repeated flag flips don't leak state ─────────────────────
...
// ─── section 6 — Concurrent reads with serialized writes ─────────────────

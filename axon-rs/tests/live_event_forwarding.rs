...
//! v1.24.0 — Live FlowExecutionEvent forwarding from runner to SSE wire.
//!
...
//! regression for v1.21.0 + v1.22.0+32 adopter contracts).
...
//! - Client disconnect → executor abort < 100ms (v1.24.0 scope).
...
// ─── section 1 — Wire shape preservation (D9) ─────────────────────────────
...
async fn single_step_wire_carries_token_event_and_complete() {
    ...
    // v1.21.0 + v1.22.0+32 adopter contracts depend on.
...
// ─── section 2 — Event-id monotonicity (W3C SSE Last-Event-ID resume) ─────
...
// ─── section 3 — FlowExecutionEvent catalog invariant via receiver path ───
...
// stub_steps_executed_anchor.rs + flow_execution_event_drift.rs.
...
// ─── section 4 — Error path: not-deployed flow ─────────────────────────────
...
// ─── section 5 — Trace recording survives the streaming path (audit parity) ─
...
async fn trace_recorded_under_reserved_id_after_streaming() {
    ...
    // on this — D9 + v1.21.0 audit parity.
    let app = build_router(server_cfg(false));
    deploy(app.clone(), SINGLE_STEP_SOURCE).await;
...
// ─── section 6 — Catalog closure: every variant is reachable from the wire ─
...
// ─── section 7 — Pure FlowExecutionEvent helpers exercised in cross-stack drift ─
// Catalog-shape assertions live in flow_execution_event_drift.rs;

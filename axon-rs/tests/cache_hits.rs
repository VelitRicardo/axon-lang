//! v2.89.0 — **the `cache` primitive memoises something.**
//!
//! # What was wrong
//!
//! v2.40.0 shipped `cache` complete on paper: the grammar, four type-checker laws
...
//! deferred the last step (v2.40.0, *"deferred, not built"*: wire
//! `CacheRuntime::dispatch` into the dispatch path).
//!
//! v2.89.0 measured what that left. `CacheRuntime::dispatch` had **zero
...
//! v2.83.0 bar: a gate is proof of a PATH only when its input is source and its
...
//! that matters most, and it is why v2.89.0 had to land first. The tenant is a
//! component of every cache key precisely so that a
...
//! fails on the pre-v2.89.0 runtime for exactly that reason.
...
/// declaration fingerprint and therefore part of the cache key, so a
...
fn build_upstream() -> Arc<Upstream> {
    ...
    // the the design decision "errors are never memoised" assertion needs.
...
/// 🎯 The whole step in one assertion: a `pure` tool called twice with the
...
async fn a_pure_tool_called_twice_is_computed_once() {
    ...
    // the handler, never the path (v2.83.0).
...
/// 🎯 **the design decision — an error is never memoised.**
///
...
/// 🎯 **The cross-tenant assertion — and the reason v2.89.0 had to land first.**
///
/// Two tenants run the same program with the same arguments. The tenant is a
/// component of the cache key, so the second tenant MUST miss
/// and reach the vendor itself.
///
/// This fails on the pre-v2.89.0 runtime, and not because of the cache: the
...
// ── v2.87.0's two-doors rule, applied ─────────────────────────────────────
...
/// and dead on the other is the defect v2.67.0 named ("real-on-one-path,
/// dead-on-the-other"), v2.69.0 found twice more (`budget`, then `capacity:`/
/// `lease` inert on SSE), and v2.87.0 generalised into the rule that a subsystem's
/// doors must be audited as a pair. `cache` had two doors from the start.
///
/// The tenant half of this assertion is why v2.89.0 exists. This path had NO
...
/// and it is the v2.69.0 lesson transposed: a `BudgetGate` built per request
...
/// so the design decision isolation still holds. Sharing the whole runtime instead would
...
/// call recomputes. v2.40.0 typed the reference (`axon-T864` proves the channel

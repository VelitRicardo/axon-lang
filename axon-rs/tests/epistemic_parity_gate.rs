...
//! v2.7.0 — the epistemic-wire parity drift gate (the v2.4.0 gate,
...
// ── SSE Event data extraction (mirrors b_2's helper) ───────────
...
// ── section 1 — the two derivation entry points agree ─────────────────────
...
// ── section 2 — the derivation is real + non-empty (degradation lock) ─────
    ...
    assert_eq!(
        ...
        vec![
            EpistemicEnvelope {
                ...
                confidence: 0.80,
                output_type: None, // v2.8.0 — these sample tools declare no output_type
            },
            EpistemicEnvelope {
                base: "know".into(),
...
// ── section 3 — the two wire shapes are byte-identical ────────────────────
...
fn sync_and_streaming_wire_arrays_are_byte_identical() {
    ...
    assert_eq!(
        ...
        "PARITY GATE: the `epistemic_envelopes` array on the sync FlowEnvelope and \
         the streaming axon.complete MUST be byte-identical — the v2.4.0 \
         differential, locked"
    );
}

// ── section 4 — a non-epistemic flow surfaces nothing on either path ──────

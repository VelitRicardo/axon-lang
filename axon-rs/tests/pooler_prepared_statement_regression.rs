...
//! v1.31.0 — Pooler-coherent Transactions Contract — diagnostic anchor.
//!
...
//! section 4 grep -assertion enforces this STATICALLY across the source
//!   tree.
//! - **D2** — `PoolOptions::after_release` runs `DEALLOCATE ALL` on
//! every released connection. section 5 inspects the `connect_named_with_namespace`
...
//! section 3 inspects the source for the `tracing::warn!` site count.
//!
...
//! `pgbouncer_integration.rs` (PG-gated, opt-in).
...
// ════════════════════════════════════════════════════════════════════
// section 1 — The kivi smoke 16 corpus: prepared statement collision symptoms
// ════════════════════════════════════════════════════════════════════

/// section 1 — pin that the symptom strings the adopter saw in their deploy
...
// ════════════════════════════════════════════════════════════════════
// section 2 — D3 observability: 5 silent swallows became tracing::warn!
// ════════════════════════════════════════════════════════════════════

/// section 2 — pin that EVERY `Err(_) => (None, &no_types)` swallow in
/// `query` / `persist` / `mutate` / `purge` / `row_stream::drain_stream`
...
fn s2_d3_observability_no_silent_swallows() {
    ...
    // v1.32.0/.12 superseded the original v1.31.0 `d_letter =
    ...
    // step (`37.x.j.11` for the 4 backend ops, `37.x.j.12` for the
    // stream cursor).
...
// ════════════════════════════════════════════════════════════════════
// section 3 — D3 invariant: warnings cite the primary error explicitly
// ════════════════════════════════════════════════════════════════════

/// section 3 — pin that every `tracing::warn!` site emitted by D3 carries the
...
fn s3_d3_invariant_warnings_carry_primary_error() {
    // v1.32.0/.12 — the superseding warn sites bind the primary
...
// ════════════════════════════════════════════════════════════════════
// section 4 — D1 STATIC ENFORCEMENT: every sqlx::query carries .persistent(false)
// ════════════════════════════════════════════════════════════════════

/// section 4 — pin that EVERY `sqlx::query(...)` and `sqlx::query_as(...)` call
...
fn s4_d1_invariant_every_sqlx_query_uses_persistent_false() {
    ...
    for path in walk_store_sources() {
        ...
        // call (excluding doc comments + the DEALLOCATE ALL site which section 5
        // handles explicitly).
        let call_sites: Vec<usize> = lines
            ...
            .filter_map(|(i, line)| {
                ...
                // section 5 owns the DEALLOCATE ALL meta-invariant separately.
...
// ════════════════════════════════════════════════════════════════════
// section 5 — D2 INSTALLATION: after_release hook with DEALLOCATE ALL
// ════════════════════════════════════════════════════════════════════

/// section 5 — pin that `connect_named_with_namespace` installs the
...
// ════════════════════════════════════════════════════════════════════
// section 6 — v1.31.0 D1: every axon-owned table lives in `axon_admin`
// ════════════════════════════════════════════════════════════════════

/// section 6 — pin the kivi-specific regression class structurally: no
...
/// against adopter apps. A future v2.0.0+ may extend this invariant

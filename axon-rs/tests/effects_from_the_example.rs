//! v2.89.0 — `effect` / `handle` / `perform`, driven from the PUBLISHED
...
//!
//! `examples_compile` already proves the example COMPILES. It does
//! not run it. v2.87.0's dispatch gate runs the machine, but from inline source.
//! This joins the two ends: the published program, dispatched.
//!
//! # What v2.87.0 found
//!
//! The Plotkin/Pretnar engine from v1.17.0 — 590 lines, 49 tests — had ZERO lexer
...
async fn the_published_example_runs_its_handler_over_a_performing_flow() {
    ...
    // the `in { … }` body were inert — the shape v2.87.0's the design decision rejected — this

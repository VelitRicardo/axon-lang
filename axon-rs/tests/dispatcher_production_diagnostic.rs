...
//! v1.24.0 diagnostic anchor — capture the v1.26.0 production-path
//! SSE wire shape for the 8 architectural-group representative flow
//! shapes that the v1.24.0 cycle activates end-to-end.
//!
//! # Why a new diagnostic for 33.z
//!
//! `dispatcher_diagnostic.rs` (33.y.a) captured 4 specific
//! non-canonical shapes (Let / Reason / Tool-using Step) plus a
//! canonical Step regression pin. Each 33.y.c–j step graduated
...
//!
//! As each 33.z step lands the production-side wiring, the
...
//! `real_provider.yml` workflow (v1.24.0). This
//! diagnostic uses the in-tree `stub` backend so the test is
//! hermetic + deterministic + fast.
//!
//! Per-handler unit tests already ship in `c_2–j` integration
...
/// Identical to 33.y.a section 1; re-asserted here so the 33.z migration
...
/// for the step-body `apply: tool_name` syntax (v1.24.0 D8).
...
/// overrides wire the supervisor's event dispatcher (v1.11.0).
///
...
/// the v1.10.0 CPS dispatcher via `apply_lambda_data` helper
...
// ────────────────────────────────────────────────────────────────────
// section 1 — D1 canonical Step regression pin (must stay green throughout 33.z)
// ────────────────────────────────────────────────────────────────────
...
async fn d1_canonical_step_regression_pin_post_v1_26_0() {
    ...
    // 33.z MUST preserve this byte-equal on every step (D4).
    assert_eq!(
        events.tokens.len(),
        1,
...
// ────────────────────────────────────────────────────────────────────
// section 2 — Conditional baseline (pre-33.z.c)
// ────────────────────────────────────────────────────────────────────
...
async fn conditional_falls_back_to_legacy_pre_33_z_c() {
    ...
    if dep != StatusCode::OK {
        eprintln!(
            "─── section 2 Conditional baseline ───\n\
    ...
    let warnings = record_baseline("section 2 Conditional", status, &ct, &body);
...
// ────────────────────────────────────────────────────────────────────
// section 3 — ForIn baseline (pre-33.z.c)
// ────────────────────────────────────────────────────────────────────
...
async fn for_in_falls_back_to_legacy_pre_33_z_c() {
    ...
    let warnings = record_baseline("section 3 ForIn", status, &ct, &body);
...
// ────────────────────────────────────────────────────────────────────
// section 4 — Par baseline (pre-33.z.c)
// ────────────────────────────────────────────────────────────────────
...
async fn par_falls_back_to_legacy_pre_33_z_c() {
    ...
    let warnings = record_baseline("section 4 Par", status, &ct, &body);
...
// ────────────────────────────────────────────────────────────────────
// section 5 — Remember baseline (pre-33.z.c)
// ────────────────────────────────────────────────────────────────────
...
async fn remember_falls_back_to_legacy_pre_33_z_c() {
    ...
    if dep != StatusCode::OK {
        eprintln!(
            "─── section 5 Remember baseline ───\n\
             deploy returned {dep} — anchor records this state. \
             Remember is a cognitive primitive (v1.4.0). \
    ...
    let _warnings = record_baseline("section 5 Remember", status, &ct, &body);
}

// ────────────────────────────────────────────────────────────────────
// section 6 — ShieldApply baseline (pre-33.z.c)
// ────────────────────────────────────────────────────────────────────
...
async fn shield_apply_falls_back_to_legacy_pre_33_z_c() {
    ...
    if dep != StatusCode::OK {
        eprintln!(
            "─── section 6 ShieldApply baseline ───\n\
             deploy returned {dep} — anchor records this state. \
             ShieldApply is a v1.13.1 algebraic-effect handler. \
    ...
    let _warnings = record_baseline("section 6 ShieldApply", status, &ct, &body);
}

// ────────────────────────────────────────────────────────────────────
// section 7 — Emit baseline (pre-33.z.c)
// ────────────────────────────────────────────────────────────────────
...
async fn emit_falls_back_to_legacy_pre_33_z_c() {
    ...
    if dep != StatusCode::OK {
        eprintln!(
            "─── section 7 Emit baseline ───\n\
             deploy returned {dep} — anchor records this state. \
             Emit is a v1.6.0 π-calc typed-channel primitive. \
    ...
    let _warnings = record_baseline("section 7 Emit", status, &ct, &body);
}

// ────────────────────────────────────────────────────────────────────
// section 8 — Hibernate baseline (pre-33.z.c)
// ────────────────────────────────────────────────────────────────────
...
async fn hibernate_falls_back_to_legacy_pre_33_z_c() {
    ...
    if dep != StatusCode::OK {
        eprintln!(
            "─── section 8 Hibernate baseline ───\n\
             deploy returned {dep} — anchor records this state. \
             Hibernate is a v1.4.0 + v1.11.0 PIX primitive. \
             ...
             wire the supervisor's event dispatcher (v1.11.0)."
        );
        return;
    }

    let (status, ct, body) = fetch_sse_body(app, "/hibernate", "{}").await;
    let _warnings = record_baseline("section 8 Hibernate", status, &ct, &body);
}

// ────────────────────────────────────────────────────────────────────
// section 9 — LambdaDataApply baseline (pre-33.z.c)
// ────────────────────────────────────────────────────────────────────
...
async fn lambda_data_apply_falls_back_to_legacy_pre_33_z_c() {
    ...
    if dep != StatusCode::OK {
        eprintln!(
            "─── section 9 LambdaDataApply baseline ───\n\
             deploy returned {dep} — anchor records this state. \
             LambdaDataApply is a v1.10.0 CPS-dispatched lambda. \
    ...
    let _warnings = record_baseline("section 9 LambdaDataApply", status, &ct, &body);
}

// ────────────────────────────────────────────────────────────────────
// v1.4.0 — Catalog totality pin (45 IRFlowNode variants)
// ────────────────────────────────────────────────────────────────────
//

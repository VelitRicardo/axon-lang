//! v2.89.0 (group B) — `daemon`, from a program that lives on disk.
//!
//! # Why this file exists
//!
//! `daemon` was attested on `daemon.rs — OTP-style supervision + v2.31.0 durable
//! event delivery`: a module name. That is an ENGINE citation, and v2.83.0 is
...
//! # What is proven here
//!
//! v2.31.0's headline, driven from a fixture: a producer emits on a typed
//! channel, the bus carries it, and the consumer daemon's `listen` body RUNS
//! with the event payload bound to the consumer flow's parameters. v2.4.0's
...
const FIXTURE: &str = "tests/fixtures/daemon/intent_learner.axon";

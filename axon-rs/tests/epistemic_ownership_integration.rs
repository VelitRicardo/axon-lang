//! v2.0.0 — End-to-end integration -assertions for the
...
//! - section 2 — `FlowEnvelope::seal()` delegates to the C23 kernel
//!     (STATIC grep gate on `wire_envelope.rs`).
//! - section 3 — `ServerExecutionResult` carries `provenance_events` +
//!     `blame_attribution` slots that propagate runtime metadata to
//!     the wire envelope.
//! - section 4 — `from_execution_result` interleaves provenance_events
//!     into provenance_chain with the canonical ordering: flow →
//!     events → steps → backend.
//! - section 5 — Closed-catalog `provenance_event_for` taxonomy: 12
//!     event kinds + `None` for non-participating step types.
//! - section 6 — Closed-catalog blame priority: 5 BlameKind variants
//!     with strictly monotone ordinals; merge_blame respects
//!     priority + stable tie-break.
//! - section 7 — End-to-end converter: a flow with retrieve+shield+
...
// ── section 1 — C23 kernel canonical for Theorem 5.1 ────────────────────────

#[test]
fn c23_kernel_canonical_for_theorem_5_1() {
    ...
    assert_eq!(
        ...
        "v2.0.0 section 1 — Rust const ({}) MUST match C kernel export ({}); \
         divergence is a structural bug.",
        axon_csys::THEOREM_5_1_CEILING,
        from_c
    );
    assert_eq!(from_c, 0.99, "v2.0.0 section 1 — the canonical ceiling is 0.99");
}

#[test]
fn c23_clamp_on_derived_state() {
    ...
    assert_eq!(
        ...
        "v2.0.0 section 1 — C23 kernel clamps derived certainty to 0.99"
    );
}

#[test]
fn c23_preserves_clean_certainty() {
    let env =
        axon_csys::EpistemicEnvelope::new(1.0, false, axon_csys::EpistemicKind::Clean);
    let clamped = axon_csys::validate_degradation(env);
    assert_eq!(
        clamped.certainty, 1.0,
        "v2.0.0 section 1 — C23 kernel preserves apodictic clean certainty"
    );
}

#[test]
fn c23_defensive_normalisation() {
...
// ── section 2 — FlowEnvelope::seal delegates to C23 ─────────────────────────

#[test]
fn seal_delegates_to_c23_kernel() {
    ...
    assert!(
        ...
        "v2.0.0 section 2 — FlowEnvelope::seal MUST delegate to the C23 \
    ...
    assert!(
        ...
        "v2.0.0 section 2 — FlowEnvelope::seal MUST build the canonical \
         EpistemicEnvelope wrapper before calling the C23 kernel."
    );
}

// ── section 3 — ServerExecutionResult carries 39.c.y + 39.c.z fields ─────

// v2.81.0 — these two gates follow their SUBJECT, not its old address.
...
// without dragging in 29,734 lines of `axum` router. The v2.0.0 contract is
...
// `v2.0.0`/`v2.0.0` compile-time uses of `ServerExecutionResult` (repointed to
// `axon::execution_result::…`) are what actually prove it.

#[test]
fn server_execution_result_carries_provenance_events() {
    ...
    assert!(
        ...
        "v2.0.0 section 3 — `ServerExecutionResult.provenance_events: Vec<String>` \
         MUST be present so the runtime's provenance walk surfaces on the wire."
    );
}

#[test]
fn server_execution_result_carries_blame_attribution() {
    let src = fs::read_to_string("src/execution_result.rs")
        .expect("read execution_result.rs");
    assert!(
        src.contains(
        ...
        "v2.0.0 section 3 — `ServerExecutionResult.blame_attribution` MUST be \
         present so the runtime's blame surfacing reaches the wire."
    );
}

// ── section 4 — Provenance chain ordering: flow → events → steps → backend ──

#[test]
fn provenance_chain_canonical_ordering() {
    ...
    assert_eq!(
        ...
        "v2.0.0 section 4 — provenance chain ordering MUST be: \
         flow → semantic events → canonical steps → backend"
    );
}

// ── section 5 — Closed-catalog provenance taxonomy ─────────────────────────

#[test]
fn provenance_taxonomy_closed_catalog() {
    // The closed taxonomy at v2.0.0: 12 semantic kinds + None for
    // non-participating step types. Any future addition requires
    // an explicit plan-vivo step.
    let cases = [
        ...
        // discipline — future cycles must extend explicitly).
        ("future_kind", "x", None),
    ];
    for (stype, sname, expected) in cases {
        ...
        match expected {
            Some(want) => assert_eq!(
                got.as_deref(),
                Some(want),
                "v2.0.0 section 5 — step_type={stype:?} name={sname:?} \
                 must emit {want:?}"
            ),
            None => assert_eq!(
                got, None,
                "v2.0.0 section 5 — step_type={stype:?} must NOT emit a provenance entry"
            ),
        }
    }
}

// ── section 6 — Closed-catalog blame priority ──────────────────────────────

#[test]
fn blame_priority_is_strictly_monotone() {
    ...
    // steps extend the catalog (each new variant MUST insert
...
#[test]
fn merge_blame_respects_priority_chain() {
    ...
    assert_eq!(
        ...
        "v2.0.0 section 6 — chained merges MUST end with the highest-priority \
         (anchor breach) blame"
    );
}

// ── section 7 — End-to-end converter integration ───────────────────────────

#[test]
fn e2e_converter_with_anchor_breach() {
    let exec = ServerExecutionResult {
        success: true,
    ...
    assert!(
        env.certainty <= 0.99,
        "v2.0.0 section 7 — derived state MUST be clamped by C23 kernel; got {}",
        env.certainty
    );
...
fn e2e_clean_path_no_blame() {
    ...
    assert_eq!(env.certainty, 1.0, "v2.0.0 section 7 — clean path → 1.0");
    assert!(env.blame_attribution.is_none(), "v2.0.0 section 7 — clean path → no blame");
}

// ── section 8 — wire_envelope_producers module surface present ──

#[test]
fn producers_module_surface_complete() {
    ...
    for producer in required_producers {
        assert!(
            ...
            "v2.0.0 section 8 — `pub fn {producer}` MUST be declared (closed \
             catalog of 5 BlameKind producers)"
        );
    }
    assert!(
        src.contains("pub fn merge_blame"),
        "v2.0.0 section 8 — `merge_blame` MUST be declared (priority-aware coalesce)"
    );
    assert!(
        src.contains("pub fn blame_priority"),
        "v2.0.0 section 8 — `blame_priority` MUST be declared (closed-catalog priority)"
    );
    assert!(
        src.contains("pub fn provenance_event_for"),
        "v2.0.0 section 8 — `provenance_event_for` MUST be declared (taxonomy entry)"
    );
    assert!(
        src.contains("pub fn collect_provenance_events_from"),
        "v2.0.0 section 8 — `collect_provenance_events_from` MUST be declared (walk helper)"
    );
    assert!(
        src.contains("pub fn derive_blame_from_report"),
        "v2.0.0 section 8 — `derive_blame_from_report` MUST be declared (report walk)"
    );
}

// ── section 9 — C23 kernel source files present ──

#[test]
fn c23_kernel_source_files_present() {
    assert!(
        std::path::Path::new("../axon-csys/c-src/effects/envelope.c").exists(),
        "v2.0.0 section 9 — C23 kernel implementation MUST exist at \
         axon-csys/c-src/effects/envelope.c"
    );
    assert!(
        std::path::Path::new("../axon-csys/c-src/effects/envelope.h").exists(),
        "v2.0.0 section 9 — C23 kernel header MUST exist at \
         axon-csys/c-src/effects/envelope.h"
    );
    assert!(
        std::path::Path::new("../axon-csys/src/envelope.rs").exists(),
        "v2.0.0 section 9 — Rust shim MUST exist at axon-csys/src/envelope.rs"
    );
}

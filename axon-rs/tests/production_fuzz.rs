//! v1.24.0 — D12 production-grade fuzz over the post-33.z.e
...
//!
//! 1. **section 1 — Production hot path totality** (~4 500 LCG iters across
...
//!
//! 2. **section 2 — Cancel-through-orchestration-depth** (100 LCG iters).
...
//!
//! 3. **section 3 — Tool-call SSE emission** (250 LCG iters). Random
...
//!
//! 4. **section 4 — Sync↔async parity determinism stress** (250 LCG iters:
...
//! # D-letter coverage
//!
//! - **D1** — Single hot path through dispatcher (33.z.e). section 1 fuzz
...
//! invariant). section 2 fuzz confirms cancel propagation under random
//!   nesting + timing.
//! - **D4** — Wire byte-compat anchored on canonical Step. section 1's
//!   canonical-Step cluster pins this (1 token "(stub)" + terminal
//!   FlowComplete per iter).
//! - **D5** — `axon.tool_call` SSE emission. section 3 fuzz confirms zero
...
//!   emits ToolCall").
//! - **D7** — Sync↔async parity. section 4 fuzz confirms the 33.z.d corpus
...
async fn drive_production_path(
    ...
    axon::streaming_via_dispatcher::run_streaming_via_dispatcher(
        ...
        None, // v2.15.0 — api_key
        None, // v2.69.0 — channel_semaphores
        None, // v2.69.0 — tool_leases
        String::new(), // v2.89.0 — unscoped tenant, verbatim (v2.49.0)
    )
    .await;
...
// ────────────────────────────────────────────────────────────────────
// Source-template builders for section 1 production hot path totality.
...
// ────────────────────────────────────────────────────────────────────
// section 1 — Production hot path totality (~4 500 LCG iters across 12
...
// ────────────────────────────────────────────────────────────────────
// section 2 — Cancel-through-orchestration-depth fuzz (100 LCG iters).
//
...
// streaming_via_dispatcher section 1: "On cancel OR consumer-drop, the
...
async fn fuzz_s2_cancel_through_orchestration_depth() {
    ...
    for iter in 0..100 {
        ...
        assert!(
            elapsed.as_secs() < 5,
            "section 2 iter={iter} depth={depth} pre_cancel={pre_cancel}: \
        ...
        if !pre_cancel {
            assert!(
                env.saw_flow_start,
                "section 2 iter={iter} depth={depth} pre_cancel=false: \
...
// ────────────────────────────────────────────────────────────────────
// section 3 — Tool-call SSE emission fuzz (250 LCG iters).
//
...
async fn fuzz_s3_tool_call_zero_emission_under_stub() {
    ...
    for iter in 0..250 {
        ...
        .await;
        assert_wire_envelope_valid(&env, &format!("section 3 iter={iter}"));
        assert_eq!(
            env.tool_call_count, 0,
            "section 3 iter={iter}: D5 invariant — stub backend MUST NOT emit ToolCall. \
...
// ────────────────────────────────────────────────────────────────────
// section 4 — Sync↔async parity determinism stress (250 LCG iters:
...
async fn run_async_snapshot(
    ...
    while let Some(ev) = rx.recv().await {
        match ev {
            ...
            // v2.89.0 — attribute by STEP NAME, not by a positional cursor.
            //
            ...
            // stable for this corpus; v2.89.0 added one fixture and two
            // unrelated `par` fixtures started diverging, deterministically.
            //
            // `StepToken` has carried `step_name` (and `branch_path`, the v2.15.0
...
async fn fuzz_s4_parity_determinism_stress() {
    ...
    assert!(
        !fixtures.is_empty(),
        "section 4: corpus must exist; 33.z.d fixtures are the deterministic-stress source-of-truth"
    );
    assert!(
        fixtures.len() >= 50,
        "section 4: expected ≥50 corpus fixtures; got {}",
        fixtures.len()
    );
    ...
    assert!(
        ...
        "section 4 determinism stress: {} divergence(s) detected:\n - {}",
...
// ────────────────────────────────────────────────────────────────────
// section 5 — Cardinality pin (the fuzz pack's grand total).
// ────────────────────────────────────────────────────────────────────
...
fn fuzz_pack_cardinality_pinned() {
    ...
    assert_eq!(S1, 4_500, "section 1 totality: 12 clusters × 375 = 4 500");
    assert_eq!(S2, 100, "section 2 cancel depth: 100");
    assert_eq!(S3, 250, "section 3 tool-call: 250");
    assert_eq!(S4, 250, "section 4 parity determinism: 50 × 5 = 250");
    assert_eq!(
        GRAND_TOTAL, 5_100,

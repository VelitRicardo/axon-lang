//! §Fase 119.f.9 (runtime half) — `weave` synthesises over the source VALUES.
//!
//! The grammar half is `axon-frontend/tests/fase119_f9_weave_statement.rs`.
//!
...
//! `reason`'s `given:` (§119.f.8), and `advertised.rs` attested
...
fn f9r_2_include_is_a_requirement_the_model_sees() {
    // §111's rule: a field decided by nothing is the defect. `include:` is a
...
fn f9r_5_an_unbound_source_contributes_itself_not_a_hole() {
    ...
    // dropped input is the §112 defect ("if the evidence is missing,

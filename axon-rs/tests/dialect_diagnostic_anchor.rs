...
//! v1.24.0 — Diagnostic anchor for the wire-format adapter cycle.
//!
//! Captures the **current v1.27.1 axon-named-events wire surface** as
//! a forensic baseline. Every subsequent step (33.z.k.b through
...
//! # What this file pins
//!
//! 1. **section 1 Canonical Step + stub backend → exactly 1 `axon.token` +
...
//!    parallel anchors).
//!
//! 2. **section 2 Tool-streaming flow (algebraic effect) → axon-named
...
//! assertion REVERSES at that step. The capture here is the
//!    BEFORE picture.
//!
//! 3. **section 3 Wire-format event catalog pinned at 4 axon-named events.**
...
//!    dialect anchor.
//!
//! 4. **section 4 D3 explicit opt-out grammar already accepts `sse` value.**
...
//! gate enforcement. The cycle's actual gates land per-step as
//! their handlers ship.
...
// ─── section 1 — Canonical Step + stub → 1 axon.token + 1 axon.complete ────
...
async fn s1_canonical_step_stub_emits_one_token_one_complete() {
    ...
    eprintln!(
        "section 1 anchor (canonical Step + stub, axon dialect baseline):\n\
         Content-Type: {ct}\n\
    ...
    assert_eq!(tokens, 1, "section 1 D4 anchor: exactly 1 axon.token for stub");
    assert_eq!(completes, 1, "section 1 D4 anchor: exactly 1 axon.complete");
}

// ─── section 2 — Algebraic-effect flow → OpenAI wire surface (Q1 ratification) ─────
//
...
// dialect (W3C named events) — see section 1.
//
...
async fn s2_algebraic_effect_emits_openai_wire_post_33_z_k_g_2() {
    ...
    assert!(
        ...
        "section 2 post-33.z.k.1 anchor: algebraic-effect flow gets SSE wire"
    );
    ...
    eprintln!(
        "section 2 anchor (algebraic-effect flow, POST-33.z.k.g.2 OpenAI dialect):\n\
         Content-Type: {ct}\n\
    ...
    assert_eq!(
        axon_tokens, 0,
        "section 2 POST-33.z.k.g.2: axon.token MUST NOT appear on openai-dialect wire"
    );
    assert_eq!(
        axon_completes, 0,
        "section 2 POST-33.z.k.g.2: axon.complete MUST NOT appear on openai-dialect wire"
    );
    assert!(
        has_done_sentinel,
        "section 2 POST-33.z.k.g.2: openai-style `data: [DONE]` sentinel MUST terminate the stream"
    );
    assert!(
        has_chat_completion_chunk,
        "section 2 POST-33.z.k.g.2: every chunk frame MUST carry `object: \"chat.completion.chunk\"` \
         per OpenAI Chat Completions streaming spec"
    );
    assert!(
        has_role_assistant,
        "section 2 POST-33.z.k.g.2: first chunk MUST emit role-marker `delta: {{\"role\": \"assistant\"}}` \
         per OpenAI Chat Completions streaming spec"
    );
    assert!(
        has_finish_reason_stop,
        "section 2 POST-33.z.k.g.2: final chunk MUST carry `finish_reason: \"stop\"` per OpenAI spec"
    );
}

// ─── section 3 — Wire-format event catalog pinned at 4 axon-named events ───
...
async fn s3_axon_named_event_catalog_pinned_at_four_events() {
    ...
    assert_eq!(
        AXON_EVENTS.len(),
        4,
        "section 3 33.z.k.a anchor: axon dialect closed catalog = 4 events; \
         adding a 5th requires a deliberate step + cross-stack \
         drift gate update"
    );
    eprintln!("section 3 anchor: axon dialect catalog = {AXON_EVENTS:?}");
}

// ─── section 4 — D3 grammar accepts bare `transport: sse` (pre-33.z.k.b) ───
...
async fn s4_bare_transport_sse_grammar_accepts_pre_33_z_k_b() {
    ...
    eprintln!(
        "section 4 anchor (bare `transport: sse` grammar, PRE-33.z.k.b):\n\

...
//! v1.28.0 — Anthropic dialect adapter byte-exact tests.
//!
...
/// Same helper as in openai_dialect_adapter.rs.
...
// ─── section 1 — select_adapter("anthropic", ...) returns AnthropicDialectAdapter
...
// ─── section 2 — FlowStart emits message_start event ───────────────────────
...
// ─── section 3 — StepStart is silently consumed (lazy text-block open) ────
...
// ─── section 4 — First StepToken opens text block + emits text_delta ──────
...
// ─── section 5 — Second StepToken (same step) reuses the open block ───────
...
// ─── section 6 — StepComplete closes the open text block ──────────────────
...
// ─── section 7 — ToolCall emits tool_use block (start + delta + stop) ─────
...
// ─── section 8 — ToolCall mid-step closes open text block first ───────────
...
// ─── section 9 — FlowComplete emits message_delta (terminator deferred) ──
...
// ─── v1.4.0 — flush_terminator emits axon.metadata + message_stop ─────
...
// ─── v1.4.0 — Block indices advance monotonically across the stream ───

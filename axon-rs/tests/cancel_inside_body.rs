...
//! v1.24.0 — Cancellation observed INSIDE the reqwest body.
//!
...
//! in `real_provider.yml` (v1.24.0, opt-in via
...
// ── section 1 — Already-cancelled flag → stream yields None immediately ──
...
// ── section 2 — D3 measurable invariant: p95 cancel-to-None ≤ 100ms ──────
...
// ── section 3 — No cancel → all chunks delivered (no regression) ─────────
...
// ── section 4 — Cancel via cloned flag from independent task ─────────────
...
// ── section 5 — Stub backend honors the cancel field uniformly ───────────
...
// ── section 6 — cancel_aware wrapper direct (no backend) ─────────────────
...
// ── section 7 — Multi-step flow: cancel applies per-step uniformly ───────
//
...
// asserted at the HTTP layer by v1.24.0's cancel-safety suite

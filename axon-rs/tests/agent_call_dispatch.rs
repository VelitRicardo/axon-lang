//! v2.83.0 (runtime half) — the agent call DISPATCHES, and fails closed.
//!
//! The grammar half is `axon-frontend/tests/agent_call_grammar.rs`.
...
//! v2.67.0 compute doctrine ("an empty catalog fails closed") applied to the one
//! construct in the language that spends in a loop.
//!
//! It also pins the seam v2.83.0 was about: a catalog the executor reads and
...
//! everybody forgets) and by the v2.83.0 resume path.
...
async fn m3d_2_an_empty_catalog_fails_closed() {
    // The v2.67.0 doctrine, stated as a test: a catalog nobody filled must run
...
// ── section 2 — a declared agent runs ─────────────────────────────────────────────
...
// ── section 3 — arguments carry VALUES ────────────────────────────────────────────
...
async fn m3d_5_a_dotted_argument_is_resolved_to_its_value() {
    ...
    // VALUE. Passing the string `"Gather.output"` would be the v2.83.0 defect
    ...
    // and by v2.83.0's suite. What THIS asserts is that the argument path is
    // wired to it at all.
    let out = dispatch_node(&call("Analyst", &["Gather.output"]), &mut c).await;
    assert!(out.is_ok(), "a dotted argument must dispatch: {out:?}");
}

// ── section 4 — cancellation is honoured at entry (D3) ────────────────────────────

/// Found by v1.24.0's cancel-propagation fuzz on the first run: the handler

...
//! v1.32.0 (D6) — honest streaming failure.
//!
...
//!
//! v1.31.0 made backend-resolution failure honest (a structured 503);
//! v1.32.0 makes flow-execution failure on the streaming wire
//! honest. A stream that dies says why.
//!
//! Deterministic + infra-free: section 1–section 4 fail connecting to a dead
...
//! then errors mid-walk at the `retrieve` node). section 5 is the happy
//! path. (Pre-v1.30.0 these used a `sqlite` store; the closed catalog
...
/// `dialect`. v1.30.0 closed the axonstore catalog to
...
/// v1.32.0 diagnostic NAMES on the wire.
...
// ── section 1 — the openai dialect surfaces the error ──────────────────────
...
async fn s1_openai_dialect_surfaces_the_error_detail() {
    ...
    assert!(
        ...
        "v1.32.0 — the openai stream terminates with a metadata frame + \
         `[DONE]`. Wire:\n{wire}"
    );
    assert!(
        wire.contains("failed at retrieve from 'bad'"),
        "v1.32.0 D6 — the openai `axon_metadata` frame must surface the \
...
// ── section 2 — the axon dialect surfaces the error ────────────────────────
...
async fn s2_axon_dialect_surfaces_the_error_detail() {
    ...
    assert!(
        ...
        "v1.32.0 — the axon dialect emits a dedicated `event: axon.error`. \
         Wire:\n{wire}"
    );
    assert!(
        wire.contains("failed at retrieve from 'bad'"),
        "v1.32.0 D6 — the `axon.error` event carries the `error` \
         diagnostic naming the failing node. Wire:\n{wire}"
    );
}

// ── section 3 — the anthropic dialect surfaces the error ───────────────────
...
async fn s3_anthropic_dialect_surfaces_the_error_detail() {
    ...
    assert!(
        ...
        "v1.32.0 — the anthropic dialect emits an `axon.metadata` frame. \
         Wire:\n{wire}"
    );
    assert!(
        wire.contains("failed at retrieve from 'bad'"),
        "v1.32.0 D6 — the anthropic `axon.metadata` frame must surface \
         the `error` diagnostic naming the failing node. Wire:\n{wire}"
    );
}

// ── section 4 — the failing node is named ──────────────────────────────────
...
async fn s4_the_failing_node_is_named_in_the_diagnostic() {
    ...
    assert!(
        ...
        "v1.32.0 D6 — a mid-walk dispatch failure must NAME the failing \
         node: the diagnostic reads `flow '…' failed at retrieve from \
         'pg': …`, not a generic `dispatcher error`. Wire:\n{wire}"
    );
}

// ── section 5 — D5: a successful streaming flow carries no `error` ─────────
...
async fn s5_d5_successful_flow_wire_has_no_error_field() {
    ...
    assert!(
        ...
        "v1.32.0 D5 — a flow that does NOT error carries NO `error` field \

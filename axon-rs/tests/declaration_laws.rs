//! v2.89.0 — `memory`, `pix` and `axonstore`, from a program on disk.
//!
...
//! `typed_store_integration`, `memory`'s PEM write-through by v2.67.0's gates, and
//! `pix`'s navigation by v2.13.0's. Those stay. Saying so precisely is the
//! point: a citation that claims more than it shows is the defect this cycle
//! exists to close, and `backend: in_memory` here follows v2.67.0's precedent
//! deliberately — the properties under test are the DECLARED ones.

const FIXTURE: &str = "tests/fixtures/declarations/memory_pix_store.axon";
...
fn the_fixture_is_legal_and_all_three_declarations_reach_the_ir() {
    ...
    assert!(
        ...
        "the v1.31.0 inline schema must ride the artifact — a store whose declared \
         shape reaches no consumer is the v2.67.0 shape"
    );
}

// ── `rotate` — the same bounded shape, for the same structural reason ───────

const ROTATE_FIXTURE: &str = "tests/fixtures/credential/rotate_crm_class.axon";
...
/// structural rather than a testing gap: v2.48.0's runtime gate drives a local tool
/// server on an ephemeral port, and a tool endpoint has no config-key
/// indirection the way v2.37.0 requires for `upstream`. The wire stays proven by
/// v2.48.0's own gates; what a file can carry are the two laws that make the verb
/// governable at all.
#[test]

...
//! v1.31.0 (D1, D3) — `dynamic_endpoint_handler` resolves the
...
// ─── section 1 — resolve_route_backend projects onto the ladder ───────────
...
// ─── section 2 — JSON branch: declared backend executes, beats registry ───
...
// ─── section 3 — SSE branch dispatches against the resolved backend ───────
...
// ─── section 4 — undeclared route still dispatches (D9 no-regression) ─────

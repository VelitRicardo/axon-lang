//! §Fase 122.b (group B) — `deliver`, from a program that lives on disk.
//!
...
//!
//! It is NOT group A: `secret:` is a per-tenant key under §94 custody
...
//! egress is proven by §105's own runtime gates, which stay.
//!
//! # The laws
//!
//! §105's rule is that provenance travels or the delivery refuses:
//!
...
fn every_declared_law_refuses_when_the_fixture_breaks_it() {
    ...
    for (what, from, to, expect) in [
        ...
        // D105.6 — binding `sensitive:*` into a system of record is FURTHER

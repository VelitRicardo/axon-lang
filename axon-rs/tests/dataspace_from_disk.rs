//! v2.81.0 — this gate drives the HTTP server surface (`/v1/deploy` +
...
//! green. v2.87.0's lesson is *"run with `--features server,csys-native`, the
...
//! v2.89.0 — the v2.63.0 data plane, from a program that lives on disk.
//!
//! # What v2.63.0 found, and what this keeps closed
//!
...
//! (`dataspace_engine::focus_query`, `cognitive::run_ingest`, …). v2.83.0 is
...
//! v2.63.0 engine is first-party COLUMNAR and in-process, and the server's deploy
//! path builds it. What cannot reach it is the PARITY corpus, which passes
//! `None` for the engine — so v2.63.0's verbs fail closed there by design (the
//! honesty floor, `MissingDependency`). The distinction matters: "the parity
//! harness cannot drive it" is not "it needs Postgres".
//!
//! So this gate uses the v2.67.0 shape — deploy through `/v1/deploy`, execute
...
const FIXTURE: &str = "tests/fixtures/dataspace/lead_pipeline.axon";
...
async fn a_deployed_fixture_ingests_real_rows_and_the_verbs_run_over_them() {
    ...
    // would still pass. v2.63.0's verbs fail CLOSED (`MissingDependency`) when the
    ...
    let store = engine
        .store("Leads")
        .expect("the DECLARED dataspace must exist in the engine (v2.63.0's ground truth)");
    ...
    assert_eq!(
        ...
        "external data is born Untrusted (v2.52.0) — that is not a default, it is the claim"
    );
    assert_eq!(

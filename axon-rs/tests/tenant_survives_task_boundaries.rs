//! v2.89.0 — **the ambient tenant, and the boundaries that drop it.**
//!
//! # The defect class
//!
...
//! v2.49.0 threaded the tenant to the executor explicitly and still shipped two
//! doors that lost it, because the bridge was written on the wrong side of a
//! boundary and no test compared the two sides. v2.89.0 fixed those two, and
...
//! v2.87.0 dark-targets shape, applied to a different silent set.
//!
...
//! which is exactly what was missing while this class accumulated across v2.49.0,
//! v2.81.0 and v2.89.0.
...
/// The measurement that motivated the whole step, kept as a test.
///
/// If a future tokio ever propagated task-locals across these boundaries, this
/// would fail — and the several parameters v2.89.0/v2.89.0 added would become
...
async fn a_task_local_does_not_survive_any_task_boundary() {
    ...
    assert_eq!(
        ...
        "if this ever becomes 'acme', task-locals now cross the blocking pool and the \
         explicit tenant parameters added in v2.89.0 are no longer load-bearing"
    );
    ...
    assert_eq!(
        ...
        "this is the one that caught v2.89.0's second attempt: a spawned TASK inherits \
...
///
/// This is the failure mode by itself, isolated. v2.89.0 shipped it: the
...
/// nobody would notice is the shape v2.89.0 exists to remove, so it gets the gate
/// this codebase already uses for exactly this — the `adapters.rs` v2.4.0
...
fn the_spawned_key_resolutions_pass_a_threaded_tenant() {
    ...
    assert!(
        ...
        "`server_execute_streaming` must pass its `tenant_id` PARAMETER to \
         ...
         Manager cache is consulted under the wrong name (v2.89.0)"
    );
...
///
/// ```text
/// AXON_REGEN_AMBIENT_READS=1 cargo test --test tenant_survives_task_boundaries
/// ```
///
/// The same shape as `dark_targets.pinned`, and for the same reason:
/// v2.83.0's finding that *a gap written down is honoured as often as someone
/// rereads the note*. This class went unnoticed across three cycles while every
...
fn ambient_tenant_reads_are_pinned() {
    const PIN_FILE: &str = "tests/ambient_tenant_reads.pinned";
    ...
    assert_eq!(
        ...
        "\n\nThe population of AMBIENT `current_tenant_id()` reads under `src/` changed.\n\n\
         ...
         tenant_survives_task_boundaries\n"
    );
}

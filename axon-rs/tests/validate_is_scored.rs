//! v2.88.0 — **`validate … against: <Schema>` SCORES, from `.axon` source.**
//!
//! # What it did before
//!
...
//! prompt requested a structure; the runtime threw it away. v2.67.0's shape inside
...
//! `pix_ops` shape (v2.83.0), invisible because `validate` returned prose
//! either way.
//!
...
fn validate_from_source(src: &str) -> IRValidateStep {
    ...
    // `IRStep.pix_ops`, not at flow level — v2.83.0 gave it that position
...
// ── section 1 — the schema REACHES the artifact ────────────────────────────────────
...
/// the same discipline `shield_signs` (v2.34.0) and the v2.87.0 effect catalog need.
#[test]
...
// ── section 2 — an unresolved schema is REFUSED ────────────────────────────────────
...
// ── section 3 — the confidence is BOUND, and it is the CSR ─────────────────────────
...
/// manufacturing one (a default 1.0, a prose heuristic) is the v2.67.0 defect. A
...
// ── section 4 — nothing moved for programs that do not validate ────────────────────

/// Every new IR field is `skip_serializing_if`, so no pre-v2.88.0 artifact's
/// IR-SHA moves.
#[test]

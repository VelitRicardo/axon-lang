...
//! v1.32.0 (D1, D4) — The Request Binding Contract: runtime delivery.
//!
...
//!
//! section 1 — D1: every declared parameter of a multi-parameter flow binds
//!         from its same-named body field and interpolates in a step
//!         `ask:` prompt.
//! section 2 — D1: a bound `${param}` threads through the FULL agent flow
...
//! section 3 — D4: a body field that matches NO declared parameter is NOT
//!         bound — `${undeclared}` survives un-interpolated, so the
//!         compile-time totality check (37.c) stays the single gate.
//! section 4 — D5: a parameter-less flow behind an endpoint with no `body:`
//!         streams byte-clean — the binding machinery is inert when
//!         there is nothing to bind.
//! section 5 — D1: the binding holds on the JSON transport too — a
...
// ── section 1 — D1: every declared parameter binds by name ─────────────────
...
async fn s1_every_declared_parameter_binds_from_its_body_field() {
    ...
    assert!(
        ...
        "v1.32.0 D1 — every declared flow parameter must bind from its \
         same-named body field and interpolate in the step `ask:`. \
         Wire:\n{wire}"
    );
    assert!(
        !wire.contains("${message}") && !wire.contains("${tenant_id}"),
        "v1.32.0 D1 — no parameter token may survive un-interpolated. \
         Wire:\n{wire}"
    );
}

// ── section 2 — D1: the body param threads through the full agent flow ─────
...
async fn s2_body_param_threads_through_the_full_agent_flow() {
    ...
    assert!(
        ...
        "v1.32.0 D1 — in a MIXED agent flow (retrieve → step → persist) \
    ...
    assert!(
        !wire.contains("${query}"),
        "v1.32.0 D1 — no `${{query}}` token survives un-interpolated. \
         Wire:\n{wire}"
    );
}

// ── section 3 — D4: an undeclared body field is NOT bound ──────────────────
...
// ── section 5 — D1: the contract holds on the JSON transport ───────────────
...
async fn s5_request_binding_holds_on_the_json_transport() {
    ...
    // section 1–section 3 above (the same `bind_request_body` + `interpolate_vars`).
    ...
    // v2.0.0 — the JSON transport now returns a FlowEnvelope: no
    ...
    assert_eq!(
        ...
        Some(0),
        "v1.32.0 D1 — a parameterised flow behind `transport: json` must \
    ...
    assert_eq!(
        ...
        Some(1),
        "v1.32.0 — the single step executed. Response: {resp}"
    );
}

//! v2.83.0 — `hibernate`: the flow HALTS, the continuation parks, and
//! resume rides `emit`.
//!
//! v2.67.0 F20: the handler returned "(hibernating …)" synchronously and the
...
async fn the_walk_halts_at_hibernate_and_parks_the_continuation() {
    ...
    // hibernate). If the walk kept going — the v2.67.0 F20 behavior — the step
...
async fn emit_wakes_a_parked_continuation_end_to_end() {
    ...
    //
    // v2.83.0 — the event name is UNIQUE to this test. The four cases in
    ...
    // what exposed the cross-tenant defect v2.83.0 fixes.

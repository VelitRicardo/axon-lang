...
//! v2.21.0 — Real-Postgres integration: the kivi brief #35 repro END-TO-END.
...
//!
//! Brief #28/v2.17.0 fixed `for e in ClassifyEdges.output` — a step's `List<T>`
...
//! (`invalid input syntax for type uuid: "${s.tenant_id}"`). v2.21.0 unwraps the
...
const SESSION_STORE: &str = "sessions_2";
const LTM_STORE: &str = "ltm_2";
...
fn refusal(why: &str) -> String {
    format!(
        "v2.21.0.g: {why} — and this suite REFUSES to skip.\n\n\
...
async fn exec(backend: &PostgresStoreBackend, sql: &str) {
    sqlx::query(sql)
        ...
        .await
        .unwrap_or_else(|e| panic!("v2.21.0.g fixture SQL failed:\n  {sql}\n  {e}"));
}
...
/// `retrieve sessions_2 { where: "status == 'ACTIVE'"  as: to_hibernate }`.
...
/// `for s in to_hibernate { persist into ltm_2 { tenant_id: "${s.tenant_id}"
...
async fn loop_over_retrieve_envelope_persists_resolved_row_columns() {
    ...
    // shape that pre-v2.21.0 shredded into garbage.

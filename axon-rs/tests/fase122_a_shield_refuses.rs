//! §Fase 122.a — **a declared `scan:` with no scanner to run it REFUSES.**
//!
//! # What this closes
//!
...
//! nothing had checked. §111 F12 names that shape for `warden`: *a clean-looking
//! result for a target that was never opened*.
//!
//! # Why not simply refuse for every shield
//!
//! §119.c already argued the other half, deliberately, when it DELETED
...
//!
//! That argument is right, and this fase keeps it — for a shield that declares
...
//! 27001 A.8.23. It is also what §98.d's web-taint barrier rests on: `<web>`
//! content is born Untrusted (D98.1) and `prompt_injection` is what it must
...
//! (Phase 0, beside `breach_policy` — §114.w's pattern) because `DispatchCtx`
//! carries no shield catalog and §121 measured that stamping beats threading an
...
// ── §1 — the subject: the declared scan reaches both enforcement nodes ───────
...
/// one that only filters, which is the whole distinction §122.a turns on.
#[test]
...
/// elided from the artifact — zero IR-SHA drift for every pre-§122 program.
#[test]
...
/// 🔴 **The defect this fase closes.** A shield that declares
...
/// uses. Two sites, one decision (`shield_registry::resolve_scan`): §120's
...
// ── §3 — what §122.a deliberately does NOT change ────────────────────────────

/// 🎯 **§119.c's argument, preserved.** A shield that declares no `scan:`
/// asserts nothing, so the OSS identity passthrough stays — and stays SILENT.
/// This is the test that keeps this fase from being a blanket refusal.
#[tokio::test]
...
/// A REGISTERED scanner honours the declared scan exactly as before — §122.a

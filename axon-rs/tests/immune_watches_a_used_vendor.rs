...
//! v2.69.0 — 🎯 **THE PAYOFF: the immune system watches a vendor a tool
//! actually calls.**
//!
//! v2.67.0 built the whole loop — `observe` → `immune` (KL-divergence vs a learned
...
//!
//! v2.69.0 closed the gap on the tool side: a `tool` now names a `resource`. So
//! the resource a tool CALLS can be listed in a manifest and observed — and the
//! v2.67.0 immune system learns its baseline, detects its degradation, quarantines
//! it, and heals.
//!
...
/// - `tool Search { resource: SearchApi }` — the thing that CALLS it (v2.69.0).
/// - `manifest Vendors { resources: [SearchApi] }` + `observe` + `immune` — the
/// thing that GUARDS it (v2.67.0).
...
/// degradation.**
///
/// This is the assertion v2.67.0 could not make about anything real: the resource
...
async fn the_immune_system_fires_on_a_vendor_that_a_tool_actually_calls() {
    ...
    assert!(
        ...
        "the immune system must FIRE on the degradation of a vendor a tool actually calls. This \
         is the payoff of v2.69.0: v2.67.0's guard learned to watch infrastructure you declared and \
         did not use; v2.69.0 pointed it at the thing that breaks in production. If this never \

//! §Fase 118.b.2 — the refusal contract, TESTED rather than assumed.
//!
...
//!
//! The §118 thesis is §111 doctrine applied one level down: §111 proved every
//! advertised *primitive* is REAL; §118 proves the advertised *surface stays
...
//!      A capability that vanishes from `--help` is the §111 defect in a new
...
//!      that took §111 a whole fase to find the first time.
...
/// (§111 F16 — a coverage law that was vacuous because it re-derived its own
/// subject).
const TOP_LEVEL: &[&str] = &[
    "check",
    "desugar",
...
/// subcommands" of D118.1 comes from, and a gate that only looked at the top
...
// ── §1 — `--help` parity under every profile ─────────────────────────────────
...
#[test]
fn fase118_b2_s1_every_subcommand_stays_in_help() {
    let help = axon_help();
    ...
    assert!(
        missing.is_empty(),
        "§118.b.2 §1 — these subcommands vanished from `axon --help`: {missing:?}.\n\
...
#[test]
fn fase118_b2_s1_nested_subcommands_stay_advertised_too() {
    for (parent, child) in NESTED {
        let help = help_of(&[parent]);
        assert!(
            lists_subcommand(&help, child),
            "§118.b.2 §1 — `axon {parent} {child}` vanished from `axon {parent} --help`. \
...
#[test]
fn fase118_b2_s1_the_advertised_surface_is_twenty_eight_commands() {
    // D118.1: "all 28 subcommands stay in `--help` under every profile". Pinning
    ...
    assert_eq!(
        ...
        28,
        "§118.b.2 §1 — the advertised surface is 25 top-level + 3 nested = 28. \
         Update D118.1's wording together with these lists, or the refusal \
         contract starts describing a CLI that no longer exists."
    );
}

#[test]
fn fase118_b2_s1_serve_is_advertised_even_without_the_feature() {
    // The single most important line of §1, called out separately so a failure
    // names the actual regression instead of a list.
    let help = axon_help();
    assert!(
        lists_subcommand(&help, "serve"),
        "§118.b.2 §1 — `axon serve` MUST stay in `--help` even when the `server` \
         feature is absent. It refuses in writing (see §2); it does not \
         disappear.\n--help was:\n{help}"
    );
}

// ── §2 — the refusal names the exact way out ─────────────────────────────────

#[cfg(not(feature = "server"))]
#[test]
fn fase118_b2_s2_serve_refuses_in_writing_without_the_feature() {
    let out = Command::new(env!("CARGO_BIN_EXE_axon"))
        .arg("serve")
        .output()
    ...
    assert_eq!(
        out.status.code(),
        Some(2),
        "§118.b.2 §2 — an absent feature exits 2 (the code §118.b.1 established \
    ...
    for needle in [
        ...
        // names the OTHER BINARY — under D118.1 the server is `axon-server`,
        ...
        assert!(
            ...
            "§118.b.2 §2 — the refusal must contain {needle:?}. \
    ...
    assert!(
        ...
        "§118.b.2 §2 — the refusal is a message and an exit code, never a panic. \
         stderr was:\n{stderr}"
    );
}
...
#[cfg(feature = "server")]
#[test]
fn fase118_b2_s2_serve_is_wired_when_the_feature_is_present() {
    ...
    for flag in ["--host", "--port", "--database-url", "--schemas-dir"] {
        assert!(
            ...
            "§118.b.2 §2 — `axon serve` keeps its full flag surface under the \
             `server` feature; {flag} is missing.\n{stdout}"
        );
    }
}

// ── §3 — the second binary (D118.1, option (c)) ──────────────────────────────

#[cfg(feature = "server")]
#[test]
fn fase118_b2_s3_axon_server_binary_exists_and_mirrors_the_flag_surface() {
    ...
    for flag in [
        ...
        assert!(
            stdout.contains(flag),
            "§118.b.2 §3 — `axon-server` must carry `axon serve`'s full flag \
    ...
    assert!(
        ...
        "§118.b.2 §3 — `axon-server --help` must name the `axon` binary.\n{stdout}"
    );
}

// ── §4 — the declaration cannot drift back behind a cfg ──────────────────────

#[test]
fn fase118_b2_s4_serve_subcommand_declaration_is_not_feature_gated() {
    ...
    // `#[cfg(`. Gating it would compile perfectly and silently delete §1.
    let idx = src
        .find("\n    Serve {")
        .expect("§118.b.2 §4 — the `Serve` variant must exist in `Commands`");
    let preceding = &src[idx.saturating_sub(400)..idx];
    assert!(
        !preceding.contains("#[cfg("),
        "§118.b.2 §4 — the `Serve` subcommand declaration MUST NOT be behind a \
    ...
    assert!(
        ...
        "§118.b.2 §4 — the `server` build must dispatch to the real runtime."
    );
    assert!(
        src.contains("#[cfg(not(feature = \"server\"))]\n#[allow(clippy::too_many_arguments)]\nfn run_serve_dispatch("),
        "§118.b.2 §4 — the lean build must have a REAL refusal function, not an \
         absent arm."
    );
}

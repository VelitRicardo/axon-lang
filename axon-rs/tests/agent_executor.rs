//! v2.83.0 — **`agent` runs, and `strategy:` is a CONTROL policy.**
//!
...
//!
//! **the design decision, ratified 2026-08-08: `strategy:` is a CONTROL policy, not a
//! prompt flavour.** The three strategies are three different loops with three
//! different termination proofs, and section 3 below is what makes that claim
...
//!
//! Every bound is checked BEFORE the spend (the v2.69.0 lesson: a budget
...
// ── section 1 — the termination bound is REQUIRED, and refused before any spend ────
...
// ── section 2 — the loop actually terminates at the declared bound ─────────────────
...
async fn m2_3_the_cost_ceiling_is_priced_and_bites() {
    // v2.28.0's affine discipline applied to an agent run: the ceiling is checked
...
// ── section 3 — the three strategies are three DIFFERENT loops ────────────────────

/// the design decision made falsifiable. If `strategy:` were a prompt flavour, all three
...
// ── section 4 — containment: an agent may only reach the tools it was granted ─────
...
/// happened. That is the shape of gate this cycle spent three steps
...
// ── section 5 — on_stuck: every catalog member decides, or refuses by name ────────
...
async fn m5_4_forge_recovery_runs_the_real_forge() {
    // v2.41.0's forge, seeded with the stuck context — not a placeholder string.
    let (mut c, mut rx) = ctx();
    ...
    assert!(
        saw_forge,
        "`on_stuck: forge` must dispatch the REAL v2.41.0 forge, not a message \
         that says it did"
    );
}

// ── section 6 — custom: the step sequence inside the agent block IS the policy ────
...
// ── section 7 — the agent binds its result under its own name ────────────────────

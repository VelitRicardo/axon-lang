...
//! v1.31.0 — IDENTITY Column Recognition diagnostic anchor.
//!
...
// ════════════════════════════════════════════════════════════════════
// section 1 — The kivi smoke-16 #11 corpus pin
// ════════════════════════════════════════════════════════════════════

/// section 1 — pin the kivi `chat_history.id` collision class structurally.
...
// ════════════════════════════════════════════════════════════════════
// section 2 — D3 BEHAVIOURAL — T803 SKIPS a NOT-NULL identity column
// ════════════════════════════════════════════════════════════════════

/// section 2 — pin that T803 does NOT fire when a `persist` omits a NOT-NULL
...
fn s2_d3_t803_skips_notnull_identity_column() {
    ...
    cols.insert(
        ...
        ManifestColumn {
            ...
            // v1.31.0 — the kivi `chat_history.id` case:
...
/// section 2-companion — the negative control: T803 STILL fires on a plain
...
// ════════════════════════════════════════════════════════════════════
// section 3 — D2 manifest round-trip: `identity: true` survives canonical
// ════════════════════════════════════════════════════════════════════

/// section 3 — pin that `identity: true` round-trips through canonical JSON
/// serialization + parsing. The byte-deterministic emission is what
/// the v1.31.0 content_hash relies on.
#[test]
...
/// section 3-companion — D5 absolute: a v1.38.2 manifest (no `identity`
...
// ════════════════════════════════════════════════════════════════════
// section 4 — D1 STATIC: introspect_cli query mentions attidentity
// ════════════════════════════════════════════════════════════════════

/// section 4 — grep -assertion: the deep introspection query in
...
// ════════════════════════════════════════════════════════════════════
// section 5 — D1 BUILDER: build_manifest_store emits identity:true
// ════════════════════════════════════════════════════════════════════

/// section 5 — pin `build_manifest_store` translates `identity_kind: Some(_)`

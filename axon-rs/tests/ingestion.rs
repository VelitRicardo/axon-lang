//! v2.81.0 — this suite exercises the OOXML surface (reader, editor,
...
//! v2.54.0 — the ingestion tool dispatch path: DocumentReader (bounded,
...
fn document_reader_reads_born_untrusted_parsed() {
    ...
    // Born Untrusted + Parsed — the load-bearing property.
...
fn declared_but_unimplemented_tool_typed_refuses_not_hallucinate() {
    // v2.54.0 / the design decision — `FileReader` is declared in stdlib::TOOLS but has no
    ...
    // (`PDFExtractor` moved to a REAL v2.54.0 dispatch arm — see extraction.)

...
//! v1.28.0 — D12 production-grade fuzz × dialects.
//!
...
//!
//! 1. **section 1 — Adapter totality** (3 dialects × 200 iters = 600 iters):
...
//!    closed catalog).
//!
//! 2. **section 2 — Closed-catalog wire output** (3 dialects × 150 iters =
...
//!
//! 3. **section 3 — Arrival-order signature invariant** (300 iters across all
...
//!
//! 7. **section 7 — Determinism across repeats** (200 iters): same seed →
    ...
    if lcg.boolean() {
        ...
        for i in 0..n {
            step_audit_records.push(StepAuditRecord {
                ...
                timestamp_ms: 1_000_000 + lcg.range(1_000_000) as u64,
                // v1.29.0 — tool-stream provenance fields not
...
// ════════════════════════════════════════════════════════════════════
// section 1 — Adapter totality across all 3 dialects (3 × 200 iters)
// ════════════════════════════════════════════════════════════════════
...
fn assert_frame_is_well_formed(frame: &axum::response::sse::Event, dialect: &str, iter: usize) {
    ...
    if data == "[DONE]" {
        assert_eq!(
            dialect, "openai",
            "33.z.k.j section 1: only openai dialect MUST emit `[DONE]` \
    ...
    assert!(
        parsed.is_ok(),
        "33.z.k.j section 1: frame data MUST be well-formed JSON OR the \
...
// ════════════════════════════════════════════════════════════════════
// section 2 — Closed-catalog wire output (event names + JSON shape)
// ════════════════════════════════════════════════════════════════════
        ...
        for f in &frames {
            ...
            if !name.is_empty() {
                assert!(
                    ...
                    "33.z.k.j section 2 axon: frame `event:` name `{name}` is \
    ...
    for iter in 0..150 {
        ...
        for f in &frames {
            assert_eq!(
                event_name(f),
                "",
                "33.z.k.j section 2 openai: every frame MUST be `data:` only \
        ...
        for f in &frames {
            ...
            if !name.is_empty() {
                assert!(
                    ...
                    "33.z.k.j section 2 anthropic: frame `event:` name `{name}` \
...
// ════════════════════════════════════════════════════════════════════
// section 3 — Arrival-order signature invariant across dialects (300 iters)
// ════════════════════════════════════════════════════════════════════
...
fn s3_arrival_order_invariant_holds_across_all_three_dialects() {
    for iter in 0..300 {
        ...
        assert_eq!(
            axon_sig, expected,
            "33.z.k.j section 3 iter {iter}: axon dialect MUST preserve arrival \
             signature {expected:?}, got {axon_sig:?}"
        );
        assert_eq!(
            openai_sig, expected,
            "33.z.k.j section 3 iter {iter}: openai dialect MUST preserve arrival \
             signature {expected:?}, got {openai_sig:?}"
        );
        assert_eq!(
            anthropic_sig, expected,
            "33.z.k.j section 3 iter {iter}: anthropic dialect MUST preserve arrival \
...
// ════════════════════════════════════════════════════════════════════
// section 4 — Anthropic content_block lifecycle (300 iters)
// ════════════════════════════════════════════════════════════════════
...
fn s4_anthropic_block_lifecycle_is_well_formed() {
    for iter in 0..300 {
            ...
            if name == "content_block_start" || name == "content_block_stop" {
                ...
                if name == "content_block_start" {
                    assert!(
                        ...
                        "33.z.k.j section 4 iter {iter}: block index {idx} MUST be \
                    ...
                    assert!(
                        ...
                        "33.z.k.j section 4 iter {iter}: duplicate \
                         content_block_start for index {idx}"
                    );
                } else {
                    assert!(
                        open_blocks.remove(&idx),
                        "33.z.k.j section 4 iter {iter}: content_block_stop for \
        ...
        assert!(
            ...
            "33.z.k.j section 4 iter {iter}: anthropic dialect MUST close every \
...
// ════════════════════════════════════════════════════════════════════
// section 5 — OpenAI tool_call_id monotonicity (200 iters)
// ════════════════════════════════════════════════════════════════════
    ...
    for iter in 0..200 {
        ...
        for _ in 0..tool_call_count {
            ...
            assert_eq!(
                frames.len(),
                1,
                "33.z.k.j section 5 iter {iter}: openai ToolCall MUST emit \
        ...
        for (i, id) in emitted_ids.iter().enumerate() {
            ...
            assert_eq!(
                id, &expected,
                "33.z.k.j section 5 iter {iter}: openai tool_call_id at \
...
// ════════════════════════════════════════════════════════════════════
// section 6 — CompleteEnvelope round-trip projection (3 × 300 iters)
// ════════════════════════════════════════════════════════════════════
...
fn assert_envelope_projects_onto_axon_metadata(
    ...
    assert_eq!(
        ...
        "33.z.k.j section 6 {dialect} iter {iter}: trace_id MUST round-trip"
    );
    assert_eq!(
        metadata["flow"].as_str(),
        Some(envelope.flow_name.as_str()),
        "33.z.k.j section 6 {dialect} iter {iter}: flow MUST round-trip"
    );
    assert_eq!(
        metadata["backend"].as_str(),
        Some(envelope.backend.as_str()),
        "33.z.k.j section 6 {dialect} iter {iter}: backend MUST round-trip"
    );
    assert_eq!(
        metadata["success"].as_bool(),
        Some(envelope.success),
        "33.z.k.j section 6 {dialect} iter {iter}: success MUST round-trip"
    );
    ...
    if envelope.effect_policies.is_empty() {
        assert!(
            ...
            "33.z.k.j section 6 {dialect} iter {iter}: empty effect_policies \
...
// ════════════════════════════════════════════════════════════════════
// section 7 — Determinism across repeats (same input → same wire bytes)
// ════════════════════════════════════════════════════════════════════
    ...
    for iter in 0..200 {
        ...
        for _repeat in 0..3 {
            ...
            if let Some(ref f) = first {
                assert_eq!(
                    *f, normalized,
                    "33.z.k.j section 7 axon iter {iter}: same input → same wire \
    ...
    for iter in 0..200 {
        ...
        for _repeat in 0..3 {
            ...
            if let Some(ref f) = first {
                assert_eq!(
                    *f, normalized,
                    "33.z.k.j section 7 openai iter {iter}: same input → same \
    ...
    for iter in 0..200 {
        ...
        for _repeat in 0..3 {
            ...
            if let Some(ref f) = first {
                assert_eq!(
                    *f, normalized,
                    "33.z.k.j section 7 anthropic iter {iter}: same input → same \
...
// ════════════════════════════════════════════════════════════════════
// section 9 — Anthropic defensive close on malformed (no-terminator) streams
// ════════════════════════════════════════════════════════════════════
//
...
fn s9_anthropic_flush_terminator_defensively_closes_orphan_text_block() {
    ...
    assert_eq!(
        terminator.len(),
        3,
        "33.z.k.j section 9: anthropic flush_terminator MUST defensively close \
    ...
    assert_eq!(
        ...
        "content_block_stop",
        "33.z.k.j section 9: first defensive frame MUST be content_block_stop"
    );
...
fn s9_anthropic_flush_terminator_well_formed_input_emits_two_frames() {
    // Counterpart of section 9 above: on a well-formed input (terminal event
    ...
    assert_eq!(
        terminator.len(),
        2,
        "33.z.k.j section 9 (no-op pin): well-formed input → flush_terminator \
...
// ════════════════════════════════════════════════════════════════════
// section 8 — Iter-count meta-pin (prevents accidental fuzz shrinkage)
// ════════════════════════════════════════════════════════════════════
...
fn s8_fuzz_iter_counts_pinned_at_target() {
    ...
    assert_eq!(
        total, 3350,
        "33.z.k.j section 8: fuzz iter count drifted from the declared 3350. \

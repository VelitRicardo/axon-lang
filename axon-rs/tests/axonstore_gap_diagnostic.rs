...
//! v1.30.0 — Diagnostic anchor for the
...
//! a declarable-but-not-wired capability (cf. SSE v1.21.0–v1.29.0,
//! webhook-HMAC v1.29.1, Dockerfile-clobber v1.20.2).
//!
//! # The reframe — `axonstore` is a cognitive data plane
//!
//! v1.30.0 does NOT ship a faithful Postgres-ORM port (that would only
...
//! that already exists:
//!
//! | Pillar | Enrichment | Join point | 35 step |
//! |---|---|---|---|
...
//! | **III — Streaming** | `retrieve` is a `Stream<Row>` draining through the v1.29.0 `unified_stream_handler` with a `BackpressurePolicy` | `axon::flow_dispatcher::unified_stream` | 35.i |
...
// ════════════════════════════════════════════════════════════════════
// section 1 — Frontend captures the cognitive-data-plane intent in full
// ════════════════════════════════════════════════════════════════════
...
fn s1_frontend_lowers_postgres_axonstore_into_rich_ir_spec() {
    ...
    assert_eq!(
        ...
        1,
        "section 1: exactly one axonstore lowered into IRProgram.axonstore_specs"
    );
    let store = &ir.axonstore_specs[0];

    eprintln!(
        "section 1 anchor (frontend lowers axonstore → IRAxonStore):\n\
         node_type        = {:?}\n\
         name             = {:?}\n\
    ...
    assert_eq!(
        ...
        "section 1: backend `postgresql` preserved through lexer→parser→IR"
    );
    assert_eq!(store.connection, "env:DATABASE_URL");
    assert_eq!(
        store.confidence_floor,
        Some(0.8),
        "section 1 Pillar I: confidence_floor preserved as Option<f64> — \
         35.g will ENFORCE it at retrieve/persist"
    );
    assert_eq!(store.isolation, "serializable");
    assert_eq!(
        store.on_breach, "rollback",
        "section 1 Pillar II: on_breach preserved — 35.h will HONOR it on \
         audit-chain breach"
    );
}

// ════════════════════════════════════════════════════════════════════
// section 2 — Dispatcher store helpers are KV / backend-blind
// ════════════════════════════════════════════════════════════════════
...
fn s2_store_helpers_route_to_in_memory_kv_not_sql() {
    ...
    assert_eq!(
        persisted, 2,
        "section 2: persist snapshots the 2 user bindings into KV"
    );
    ...
    eprintln!(
        "section 2 anchor (store helpers = in-memory KV, backend-blind):\n\
    ...
    assert!(
        kv_present,
        "section 2: persist writes the `__store_<name>_<key>` KV key — pins \
         the in-memory backing shape that 35.d-f will replace with SQL"
    );
    assert_eq!(
        retrieved, "acme-001",
        "section 2: retrieve reads back from the SAME in-memory KV — no SQL"
    );
    ...
    eprintln!(
        "section 2 anchor (mutate/purge also KV): mutate→{mutated} purge→{purged}"
    );
    assert_eq!(mutated, 1, "section 2: mutate updates the KV entry in place");
    assert_eq!(purged, 1, "section 2: purge removes the KV entry");
}

// ════════════════════════════════════════════════════════════════════
// section 3 — `run_persist` / `run_retrieve` handlers route to KV; the IR
...
async fn s3_run_handlers_route_to_kv_ir_steps_have_no_backend_handle() {
    ...
    eprintln!(
        "section 3 anchor (run_* handlers route to KV, IR steps backend-blind):\n\
    ...
    assert_eq!(
        found, "42",
        "section 3: run_persist→run_retrieve round-trips through in-memory KV; \
         `where_expr` is treated as the entry key, not a SQL predicate"
    );
}

// ════════════════════════════════════════════════════════════════════
// section 4 — The four pillars are ABSENT today (pin the v1.29.x state that
...
async fn s4_four_pillars_absent_in_v1_29_baseline() {
    ...
    // consulted. The IRAxonStore carries `confidence_floor` (section 1)
    ...
    // v1.29.0 `unified_stream_handler`; no `BackpressurePolicy`.
    ...
    eprintln!(
        "section 4 anchor (four pillars ABSENT in v1.29.x):\n\
    ...
    assert!(
        returns_bare_string,
        "section 4 Pillar I: retrieve returns a bare String — no epistemic \
         trust tag. 35.g makes every tuple born Untrusted (⊥)."
    );
    assert_eq!(
        audit_chain_keys, 0,
        "section 4 Pillar II: zero audit-chain keys after a mutation — no \
...
// ════════════════════════════════════════════════════════════════════
// section 5 — Closed-catalog totality pins
// ════════════════════════════════════════════════════════════════════
...
fn s5_store_op_and_pillar_catalogs_are_closed() {
    ...
    // is the block wrapper — a future cycle per the honest-scope D12
    // statement). Adding a 5th store op is a deliberate language-level
    // decision requiring a paper update + a dedicated step.
    const STORE_OPS: &[&str] = &["persist", "retrieve", "mutate", "purge"];
    assert_eq!(
        STORE_OPS.len(),
        4,
        "section 5: the axonstore op catalog is EXACTLY 4 — persist/retrieve/\
    ...
    // a founder-level reframe, not an incremental step.
    const PILLARS: &[&str] = &[
    ...
    assert_eq!(
        PILLARS.len(),
        4,
        "section 5: the cognitive-data-plane reframe is EXACTLY 4 pillars. A \
         5th is a founder-level reframe, not an incremental step."
    );

    eprintln!(
        "section 5 anchor (closed catalogs):\n\
         store ops = {STORE_OPS:?}\n\
         pillars   = {PILLARS:?}"
    );
}

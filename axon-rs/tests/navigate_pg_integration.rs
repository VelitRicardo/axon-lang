...
//! v2.15.0/B — Real-Postgres integration for the STRUCTURAL `navigate`
//! bridge: tenant isolation + real-rows→real-hits, against a real database.
//!
//! This is the load-bearing safeguard the v2.15.0 plan flagged. The v2.15.0/B bridge
...
//! (the v2.0.0 GUC `axon.current_tenant` on the flow's pinned connection) to scope
...
//! connection) never see each other's rows (the v2.15.0/B risk-matrix
//!     load-bearing test).
//!
//! # Harness
//!
//! Connects via `AXON_TEST_DATABASE_URL`; when unset or unreachable every test
//! skips gracefully (mirrors `postgres_integration`). CI provides a
//! `postgres:16` service container.
//!
//! # RLS fidelity
//!
//! v2.83.0 — **no longer single-threaded.** This paragraph used to require
...
//!
//! ```text
//! cargo test --features postgres --test navigate_pg_integration
//! ```
//!
...
// v2.83.0 — the table + role names are PER TEST, not shared constants.
//
// **This was not a latent bug.** The names used to be `const DOC_STORE =
// "ltm_summaries"` etc., shared by all four tests, and the module header
...
// three of the four die with `relation "ltm_summaries" already exists`:
...
fn doc_store(ns: &str) -> String {
    format!("navpg_{ns}_ltm_summaries")
}
fn edge_store(ns: &str) -> String {
    format!("navpg_{ns}_ltm_edges")
}
fn app_role(ns: &str) -> String {
    format!("navpg_{ns}_app")
}
...
fn refusal(why: &str) -> String {
    format!(
        "v2.15.0: {why} — and this suite REFUSES to skip.\n\n\
...
async fn exec(backend: &PostgresStoreBackend, sql: &str) {
    sqlx::query(sql)
        ...
        .await
        .unwrap_or_else(|e| panic!("v2.15.0 fixture SQL failed:\n  {sql}\n  {e}"));
}
...
async fn seed(
    ...
    // Enable + FORCE RLS, scope each table by the v2.0.0 GUC. FORCE makes the
    ...
    // v2.83.0 — the role is namespaced too. `IF NOT EXISTS … CREATE ROLE`
    ...
    // v2.83.0 trap of hiding a defect by removing the concurrency that reveals it.
    {
...
/// carries in production. This is the precise condition the v2.15.0/B bridge
...
async fn ctx_for_tenant(
    ...
    for store in [doc_store(ns), edge_store(ns)] {
        ...
        // Set the v2.0.0 tenant GUC (as the privileged owner), THEN drop to the
        // unprivileged RLS-bound role so the policy actually filters the reads.
        //
        // v2.81.0 / the design decision — both statements go through the port's
...
async fn navigate_for(registry: &Arc<StoreRegistry>, tenant: &str, ns: &str) -> String {
    ...
    // channel, and a dropped receiver closes it → `ChannelClosed`. (The v2.15.0/B
...
/// v2.15.0 — `navigate` returns the REAL documents from real rows, ordered
...
/// v2.15.0 — the anti-hallucination guarantee against a REAL empty corpus:
...
/// v2.14.0 / v2.15.0 — tenant isolation: `navigate` reads with an EMPTY where
...
/// v2.15.0/B — THE load-bearing test: two tenants navigating CONCURRENTLY,

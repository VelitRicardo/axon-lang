//! v2.83.0 — **`compute`'s published body form COMPUTES.**
//!
//! Four README blocks (v2.81.0 ledger rows 48–51) write the same shape:
//!
//! ```text
...
//! word `logic` and reported an expression the author never wrote. v2.67.0 had
...
// ── section 1 — the published declaration parses and carries its parts ──────────────
...
// ── section 2 — and it EVALUATES, natively ─────────────────────────────────────────
...
// ── section 4 — the block is closed, and `return` is required ──────────────────────
...
// ── section 5 — the older form still compiles ──────────────────────────────────────

/// v2.67.0's `compute N(p: T) -> T { <expr> }` is untouched. This landing is

...
//! v1.24.0 — D6 cancel-safety integration tests.
//!
...
//! `Backend::stream()` (v1.24.0) is wired into the per-step
...
// ── section 1 — CancellationFlag primitives wired into the SSE path ────────
...
// ── section 3 — Cancel-on-drop guard semantics during SSE handler lifetime
...
// ── section 4 — Idempotency: cancellation is monotone
...
// ── section 5 — Multi-clone semantics: any clone can fire, all observe
...
// ── section 6 — D9 wire-format invariance: cancel-safety wiring is invisible
//        on the happy path (no observable change beyond what 33.c shipped)

#[tokio::test]
async fn d9_wire_byte_compat_preserved_post_33f() {
    // Existing sse_full_body_diagnostic.rs / c_2 tests

//! v1.24.0 drift gate — assert the `flow_dispatcher` module's
//! exhaustive surface is structurally locked.
//!
//! # v1.24.0 rewrite — ShimReason retired
//!
...
// steps extend each factory with realistic payloads as the
...
fn grad_node() -> IRFlowNode {
    // v2.65.0 — synthetic shape: NO compile-time derivative (a stale
...
fn lambda_data_apply_node() -> IRFlowNode {
    // v2.83.0 — the empty name used to complete via the placeholder
...
fn ots_apply_node() -> IRFlowNode {
    // v2.83.0 — same story as the lambda fixture: the identity
...
fn mandate_apply_node() -> IRFlowNode {
    // v2.83.0 — this fixture used to carry an EMPTY mandate name and
...
fn all_48_pairs() -> Vec<(IRFlowNode, &'static str)> {
    vec![
        ...
        // v2.4.0 — the 46th variant.
        (quant_node(), "quant"),
        // v2.4.0 — the 47th variant.
        (yield_node(), "yield"),
        // v2.43.0 — the 48th variant.
        (warden_node(), "warden"),
    ]
}
...
fn fresh_ctx() -> (DispatchCtx, mpsc::UnboundedReceiver<axon::flow_execution_event::FlowExecutionEvent>) {
    ...
    // v2.83.0 — the mandate the routing fixture applies. Its constraint
    ...
    // v2.83.0 — the ΛD + ots the routing fixtures resolve.
...
// ────────────────────────────────────────────────────────────────────
// section 1 — Cardinality + arm coverage
// ────────────────────────────────────────────────────────────────────

#[test]
fn cartesian_product_has_exactly_48_entries() {
    // (v2.65.0 grew the catalog to 49 — the fn name is historical.)
    assert_eq!(
        all_48_pairs().len(),
        49,
        "33.y.b drift gate: the IRFlowNode catalog must cover all 49 \
         variants (v2.4.0 `quant` + v2.4.0 `yield`). Adding a 48th \
...
// ────────────────────────────────────────────────────────────────────
// section 2 — Every variant routes to its labeled handler
// ────────────────────────────────────────────────────────────────────
...
const GRADUATED_VARIANTS: &[&str] = &[
    ...
    // 33.y.j — Lambda + UseTool (45/45 at the close of v1.24.0)
    "lambda_data_apply", "use_tool",
    // v2.4.0 — the `quant` cognitive block (46th).
    "quant",
    // v2.65.0 — `grad`, the proof-carrying derivative (49th).
    "grad",
    // v2.4.0 — the `yield` measurement point (47th).
    "yield",
    // v2.43.0 — the `warden` adversarial-analysis block (48th).
    "warden",
];
...
// v2.41.0 — `forge` stays in the graduated set (it routes to a real handler),
...
/// v2.63.0 — the five DATA-PLANE verbs. They are relational
...
/// in v2.63.0 (ingest) / v2.63.0 (the query verbs).
const DATA_PLANE_GRADUATED: &[&str] =
    &["focus", "associate", "aggregate", "explore", "ingest"];

/// v2.65.0 — `grad` evaluates a COMPILE-TIME derivative. The synthetic
...
/// routing proof (the mint/rotate/v2.63.0 posture).
...
/// v2.67.0 — `compute_apply` REFUSES without a declared compute. It used to
...
/// The refusal IS the routing proof (the v2.63.0 / mint / rotate posture).
...
/// LambdaDataApply uses public helper `apply_lambda_data` (v1.10.0
/// CPS dispatcher); UseTool uses `invoke_tool` (v1.16.0 tool
...
/// v2.4.0 — the `quant` cognitive block. SURFACE only: `run_quant`
...
/// v2.4.0 adds `yield` (the measurement point) to the same surface-only
/// bucket — `run_yield` likewise returns `Completed { tokens_emitted: 0 }`.
/// v2.67.0 — `quant` and `yield` are no longer surface-only either. They
...
/// v2.67.0 — `quant` + `yield`, now fail-closed. Kept as their own counted
...
/// v2.67.0 — `warden` is no longer surface-only. It used to sit in
/// QUANT_GRADUATED and "complete" with an empty output — which is exactly the
/// defect v2.67.0 found: an analysis that never ran was indistinguishable from an
/// analysis that found nothing.
///
/// It now REFUSES without its engine port, and the refusal IS the routing proof
/// (the v2.63.0 / mint / rotate posture). A security primitive must never report
...
async fn every_ir_flow_node_routes_to_its_labeled_handler() {
    ...
    for (node, expected_kind) in pairs {
        ...
        let algebraic_handler = ALGEBRAIC_HANDLERS_GRADUATED.contains(&expected_kind)
            ...
            // v2.4.0 — quant routes here (Completed, 0 tokens).
        ...
        // v2.41.0 — `forge` runs the real creative pipeline; with the stub's
        ...
        // v2.65.0 — grad with no compile-time derivative (the synthetic
        ...
        // v2.67.0 — `compute_apply` refuses without a declared compute.
        if COMPUTE_FAILCLOSED.contains(&expected_kind) {
            match outcome {
                Err(axon::flow_dispatcher::DispatchError::BackendError { ref name, .. })
                    if name == "compute" => {}
                other => panic!(
                    "compute_apply must fail CLOSED with no declared compute — binding a placeholder string is what v2.67.0 F10 was, got {other:?}",
                ),
            }
            continue;
        }

        // v2.67.0 — `quant` refuses without its simulator; `yield` refuses
        ...
        // v2.67.0 — `warden` REFUSES without its engine port. Pre-111 it
        ...
        // v2.63.0 — the data-plane verbs REFUSE without an engine port
        ...
        match (outcome, pure_shape_like, orchestration, strict_empty_completed, algebraic_handler) {
            // v2.83.0 — hibernate routes to its handler and yields the
...
fn graduated_variants_set_size_pinned_48_of_48() {
    assert_eq!(
        ...
        "48 / 48 graduated. All IRFlowNode variants have a NAMED \
         ...
         (33.y.j) + 3 quant+yield+warden (v2.4.0/v2.4.0/v2.43.0) + 1 grad (v2.65.0) = 49 variants total."
    );
    ...
    // v2.63.0 — the five data-plane verbs left the cognitive-framing
...
// ────────────────────────────────────────────────────────────────────
// section 3 — Cancel propagation through dispatch_node entry
// ────────────────────────────────────────────────────────────────────
...
// ────────────────────────────────────────────────────────────────────
// section 4 — flow_plan::ir_flow_node_kind covers every variant
// ────────────────────────────────────────────────────────────────────
//
...
// ────────────────────────────────────────────────────────────────────
// section 5 — DispatchCtx surface usable across crate boundary
// ────────────────────────────────────────────────────────────────────
...
// ────────────────────────────────────────────────────────────────────
// section 6 — DispatchError variants are reachable from the public surface
// ────────────────────────────────────────────────────────────────────
...
// ────────────────────────────────────────────────────────────────────
// section 7 — D7 invariant: zero `unimplemented!()` / `todo!()` markers
// ────────────────────────────────────────────────────────────────────
//
...
async fn dispatch_node_does_not_panic_for_any_variant() {
    ...
    for (node, kind) in pairs {
        ...
        // shape is exercised by section 2 above.
        assert!(

...
//! v2.67.0 — the Cognitive-I/O graph, live through the REAL deploy path.
//!
//! Every test here goes through `POST /v1/deploy` — the v2.49.0 discipline v2.67.0 spent
...
//! (v2.67.0 F14). The kernels took the compiled IR *directly*; nobody had ever handed
...
async fn deploying_a_declared_immune_system_produces_one() {
    ...
    assert_eq!(
        status["active"], true,
        "a declared observe/immune graph must be INSTANTIATED at deploy — before v2.67.0 it was \
         carried into the IR and consumed by nothing (v2.67.0 F14). Got: {status}"
    );

    // And it runs. v2.67.0: `window: 8` ⇒ the first eight ticks LEARN the baseline
    ...
    assert_eq!(
        ...
        "the envelope must carry what the source ACTUALLY reported — the only Handler that \
         existed before v2.67.0 returned c: 1.0 unconditionally, without going anywhere. Got: {t}"
    );
...
// ── v2.89.0 — the SATISFIABLE ensemble, from a program on disk ──────────
//
...
async fn a_deployed_fixture_aggregates_a_satisfiable_ensemble() {
    const FIXTURE: &str = "tests/fixtures/cognitive_io/ensemble_quorum.axon";
...
/// **All-or-nothing** (the v2.63.0 deploy discipline). An `ensemble` naming a quorum

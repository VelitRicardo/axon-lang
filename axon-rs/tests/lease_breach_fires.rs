//! v2.67.0 — **`lease`'s CT-2 Anchor Breach finally has a moment to fire in.**
//!
//! # The finding this closes, and why it is not like the others
//!
//! Every other v2.67.0 defect was *"the piece that makes them touch was missing"* —
...
//!
//! v2.67.0 created the subject. A flow uses an `axonstore`; the store runs on a
...
// ── v2.89.0 — the same breach, from a program on disk ──────────────────
//
...
fn a_compiled_lease_permits_the_use_then_makes_it_a_breach() {
    const FIXTURE: &str = "tests/fixtures/resource/leased_store.axon";
    ...
    *now.lock().unwrap() += chrono::Duration::seconds(3601);
    reg.resolve("Users").expect_err(
        "post-expiry USE is the CT-2 Anchor Breach the README promises — and before \
         v2.67.0 it could not even be expressed, because a flow could not USE a resource",
    );
}

// ── The breach ───────────────────────────────────────────────────────────────

/// **The assertion this entire cycle exists to make possible.**
///
/// A live lease permits the use. Past τ, the same use is a **CT-2 Anchor Breach**.
///
/// Before v2.67.0 this test could not have been written at all — not because the
...
/// **The un-resourced store is INELIGIBLE for lease governance** — v2.67.0's

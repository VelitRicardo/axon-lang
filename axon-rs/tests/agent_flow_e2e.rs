...
//! v1.32.0 (D1, D3, D5) — the Request Binding Contract end-to-end
...
//!
//! section 1 — the founder's `ChatFlow` shape (retrieve ×3 → step → persist),
//!        parameterised, behind `transport: sse`: every body parameter
//!        reaches the `step` deliberation; the mixed flow streams clean.
//! section 2 — the data round-trips: a body parameter is `persist`ed into an
//!        `in_memory` store and `retrieve`d back into a downstream step.
//! section 3 — the error path stays honest: an agent flow that errors names
//!        WHY on the wire (ties 37.e), even with the body bound.
//! section 4 — the JSON transport mirror — the agent flow runs on
//!        `transport: json` with the body threaded + bound.
//! section 5 — D3: an adversarial body value (SQL metacharacters + a nested
//!        `${...}` token) flows through the agent pattern INERT — it is
//!        data, never re-interpreted, never an injection.
//! section 6 — D5: the pre-37 shape (a parameter-less flow, no `body:`)
//!        streams byte-clean — the contract is inert when unused.
//!
//! All `in_memory` — zero external infrastructure (v1.31.0). The
...
// ── section 1 — the founder's ChatFlow shape, parameterised, E2E ───────────
...
async fn s1_founder_chatflow_shape_threads_every_body_param() {
    ...
    assert!(
        ...
        "v1.32.0 D1 — every body parameter must thread to the `step` \
         deliberation through the mixed retrieve→step→persist flow. \
         Wire:\n{wire}"
    );
    assert!(
        wire.contains("axon.complete") && !wire.contains("axon.error"),
        "v1.32.0 D1 — the founder's mixed agent flow streams to a clean \
         terminator. Wire:\n{wire}"
    );
}

// ── section 2 — the data round-trips through an in_memory store ────────────
...
async fn s2_body_param_round_trips_through_the_store() {
    ...
    assert!(
        ...
        "v1.32.0 D1 — a body parameter bound by the Request Binding \
...
// ── section 3 — the error path stays honest ────────────────────────────────
...
async fn s3_errored_agent_flow_names_why_on_the_wire() {
    ...
    // postgresql}`; `sqlite` — the pre-v1.30.0 vehicle — is now rejected
    ...
    assert!(
        ...
        "v1.32.0 D6 — an agent flow that errors emits an honest \
         `axon.error` naming the cause (the failing node), not a \
         hollow terminator. Wire:\n{wire}"
    );
    assert!(
        !wire.contains("axon.complete"),
        "v1.32.0 D1 — exactly one terminator: an errored flow emits \
         `axon.error`, never also `axon.complete`. Wire:\n{wire}"
    );
}

// ── section 4 — the JSON transport mirror ──────────────────────────────────
...
async fn s4_agent_flow_runs_on_the_json_transport() {
    ...
    // v2.0.0 — the JSON transport returns a FlowEnvelope: no
    ...
    assert_eq!(
        ...
        Some(0),
        "v1.32.0 D1 — the agent flow runs through the synchronous JSON \
    ...
    assert_eq!(
        ...
        Some(3),
        "v1.32.0 — all three nodes (retrieve, step, persist) executed. \
         Response: {resp}"
    );
}

// ── section 5 — D3: an adversarial body value is inert data ────────────────
...
async fn s5_d3_adversarial_body_value_is_inert_data() {
    ...
    assert!(
        ...
        "v1.32.0 D3 — an adversarial body value is INERT DATA: \
         interpolated ONCE into the step argument, echoed verbatim, \
         the nested `${{nested}}` NOT re-interpreted. Wire:\n{wire}"
    );
    assert!(
        wire.contains("axon.complete") && !wire.contains("axon.error"),
        "v1.32.0 D3 — the adversarial value does not break the flow. \
         Wire:\n{wire}"
    );
}

// ── section 6 — D5: the pre-37 shape streams byte-clean ────────────────────
...
async fn s6_d5_parameterless_flow_unaffected() {
    ...
    assert!(
        wire.contains("axon.complete") && !wire.contains("axon.error"),
        "v1.32.0 D5 — a parameter-less flow behind an endpoint with no \

...
//! v2.7.0 — the captured epistemic envelope reaches the WIRE on both
//! transports.
//!
//! v2.7.0 derived the `(base, scope, confidence)` triple; v2.7.0 surfaces it:
...
//! cross-transport parity gate is v2.7.0; this test proves each path
...
fn sample_envelope() -> EpistemicEnvelope {
    EpistemicEnvelope {
        ...
        confidence: 0.80,
        output_type: None, // v2.8.0 — no declared output type on this sample
    }
}

// ─── SSE Event data extraction (mirrors h's helper) ───────
...
// ─── section 1 — sync FlowEnvelope surfaces the triple ────────────────────
...
// ─── section 2 — streaming axon.complete surfaces the triple ──────────────
...
// ─── section 3 — cross-transport agreement (preview of the v2.7.0 gate) ────
...
fn both_transports_emit_byte_identical_epistemic_elements() {
    ...
    assert_eq!(
        ...
        "the (base, scope, confidence) elements MUST be identical across transports — \
         the v2.7.0 parity invariant (sync FlowEnvelope vs streaming axon.complete)"
    );
}

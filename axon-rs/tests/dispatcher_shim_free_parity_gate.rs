//! v1.24.0 parity gate — assert the dispatcher is shim-free and
//! D7-compliant.
//!
...
// ────────────────────────────────────────────────────────────────────
// section 1 — No `unimplemented!()` / `todo!()` markers
// ────────────────────────────────────────────────────────────────────
...
// ────────────────────────────────────────────────────────────────────
// section 2 — `panic!()` only inside `#[cfg(test)]` blocks
// ────────────────────────────────────────────────────────────────────
//
...
// ────────────────────────────────────────────────────────────────────
// section 3 — No `legacy_shim` / `ShimReason` / `LegacyShimHandled` symbols
// ────────────────────────────────────────────────────────────────────
//
...
// ────────────────────────────────────────────────────────────────────
// section 4 — All 10 expected dispatcher module files present
// ────────────────────────────────────────────────────────────────────
//
...
fn dispatcher_module_files_pinned_to_expected_set() {
    ...
    let expected: Vec<&str> = vec![
        // v2.83.0 — the `agent` executor. `strategy:` is a CONTROL
        // policy: three loops, three termination proofs. Composed
        ...
        // v2.69.0 — the budget gate, extracted to ONE place so every tool path
        ...
        "pix.rs",
        "pure_shape.rs",
        // v1.29.0 — added with the 4-disjunction convergence:
    ...
    assert_eq!(
        ...
        "33.y.l parity gate: flow_dispatcher module file set drifted. \
         Expected 12 files (10 per step 33.y.c–j plus mod.rs, \
         plus unified_stream.rs added by v1.29.0 for the \
         4-disjunction convergence, plus budget_gate.rs added by \
         v2.69.0, plus agent_loop.rs added by v2.83.0, plus \
         effect_handlers.rs added by v2.87.0); got {:?}. Adding a new sub-module requires \

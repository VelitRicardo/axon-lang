//! §Fase 120.g — **the ratchet on test targets that run NOTHING.**
//!
...
//!
//! §118.b.2 (`bf59ae22`, *"the server is a feature"*) moved the server behind
//! the flag. From that commit forward, breaking a server test could not turn a
//! build red. The §120.f audit turned the feature on and found FIVE breakages
//! that had accumulated in silence, from three different fases:
//!
//! | broken by | what |
//! |---|---|
//! | §119.e | `ToolEntry` gained `substrate`; a fixture stopped compiling |
//! | §119.e | `axon-E042` landed; two compliance fixtures stopped type-checking |
//! | §119.m.3 | `IRFlowNode::AgentCall` left a match non-exhaustive |
//! | §120 | five new `IRFlowNode` variants, same match |
//! | §118.c | `csys-native` became opt-in; two BPE assertions read as a broken chunker |
//!
//! Every one was proven pre-existing with `git stash --include-untracked`, per
//! §119's rule about claiming pre-existence rather than assuming it.
//!
//! # Why a ratchet and not "make `server` default"
//!
//! §118 measured what `server` costs — `axum` + `tower` + `tower-http` +
...
//! weight for every adopter is the trade §118 explicitly refused.
//!
//! And why not just write it down: §119.i.2 wrote down that five
//! `*_pg_integration` suites needed a flag, and **nobody ran them for entire
//! fases**. Its own conclusion: *a gate that depends on a flag the default
//! omits is honoured as often as someone reads the header.* So this is the
//! §119.i.2 remedy — REFUSE rather than skip — applied to features instead of a
...
///
/// ```text
/// AXON_REGEN_DARK_TARGETS=1 cargo test --test fase120_g_no_test_target_runs_nothing
/// ```
///
/// The same shape `fase1_5_rust_ir_regression_snapshot` uses for the IR golden.
/// Regenerating is a diff a reviewer sees; growing the file silently is what
/// this exists to prevent.
const PIN_FILE: &str = "fase120_g_dark_targets.pinned";
...
fn test_targets_that_run_nothing_by_default_are_pinned() {
    ...
    assert!(
        ...
        "these test targets now compile to ZERO tests under the default feature \
         ...
         that ran and succeeded. §118.b.2 created this blind spot and FIVE \
         breakages accumulated in it across three fases before anyone looked.\n\n\
...
fn the_size_of_the_blind_spot_is_stated_not_implied() {
    ...
    assert!(
        ...
        "the `postgres` suites belong in this inventory too — §119.i.2 found \
...
/// §119.h.2's doc-link gate had to defend against in the same words.
#[test]

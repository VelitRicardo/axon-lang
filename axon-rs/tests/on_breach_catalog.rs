...
//! and dead. v2.69.0 gives each policy its documented meaning, at BOTH
...
//! v2.69.0 multi-path discipline).
//!
//! `section 1` — lowering: the shield's policy resolves onto `IREmit` and
//!        `IRShieldApplyStep`; a policy-less shield stamps nothing (IR-SHA
//!        stable); axon-T952 refuses an operand-less policy.
//! `section 2` — runtime, per policy, through the REAL `run_emit`:
...
/// (IR-SHA stability for every pre-v2.69.0 program).
#[test]
...
// ── section 2 — the runtime honors each policy (through the REAL run_emit) ─────────

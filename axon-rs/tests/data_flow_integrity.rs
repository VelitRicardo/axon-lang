...
//! v1.31.0 (D4) — data-flow integrity on the streaming
//! dispatcher path.
//!
...
//! interpolation contract (v1.30.0) on two legs:
//!
...
// ─── unit-test scaffolding (mirrors dispatcher_arm.rs) ─────
...
// ─── section 1 — Leg A: `run_step` interpolates `${alias}` in `ask` ───────
...
// ─── section 2 — Leg B: a step's output is bound under its name ───────────
...
// ─── section 3 — composition: a step reads a prior step's output ──────────
...
// ─── section 4 — no-regression: the canonical agent flow still streams ────

...
//! v1.28.0 — End-to-end Kimi + GLM dispatch.
//!
...
//! # What this pack pins
//!
//! 1. **section 1 — `transport: sse(kimi)` emits canonical OpenAI wire**.
//! 2. **section 2 — `transport: sse(glm)` emits canonical OpenAI wire**.
//! 3. **section 3 — kimi + glm + openai all dispatch to the same adapter**.
...
/// - v1.24.0 `axon_metadata`: `latency_ms` (wall-clock
///     accumulator from request start).
/// - v1.24.0 `step_audit[*]`: `timestamp_ms` (per-step
///     wall-clock).
...
fn strip_volatile_in_place(v: &mut serde_json::Value) {
    if let Some(obj) = v.as_object_mut() {
        obj.remove("created");
        obj.remove("id");
        // v1.24.0 — axon_metadata.latency_ms varies per request.
...
// ─── section 1 — `transport: sse(kimi)` emits canonical OpenAI wire ───────
...
// ─── section 2 — `transport: sse(glm)` emits canonical OpenAI wire ────────
...
// ─── section 3 — kimi + glm + openai share the same adapter (byte-equiv) ─

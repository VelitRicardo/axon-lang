...
//! v1.29.0 — Diagnostic anchor for the tools-as-stream-
//! producers cycle.
//!
//! Captures the **current v1.28.0 baseline** for each of the four
//! disjunctions of `produces_stream(F)` defined by the paper section 3. Every
//! subsequent step (34.b-m) inverts a specific aspect of this
...
//! |---|---|---|
//! | **(a)** Type-level | `step S { output: Stream<T> }` | ✅ live per-chunk via `Backend::stream()` (v1.24.0) |
...
//! | **(d)** Imperative | `perform Stream.Yield(x)` | ✅ static-scan emits one event per static Yield (`bridge_effect_stream_yield`, v1.24.0) |
//!
//! v1.29.0's contract: post-34, all 4 disjunctions invoke the SAME
...
//!
//! Each subsequent step's contract is "invert THIS specific
...
//!
//! section 5 asserts the 4 disjunctions are the CLOSED set. The paper section 3
...
// ════════════════════════════════════════════════════════════════════
// section 1 — Disjunct (a) Type-level — `step S { output: Stream<Token> }`
...
async fn s1_disjunct_a_type_level_stream_emits_axon_dialect_baseline() {
    ...
    eprintln!(
        "section 1 disjunct (a) anchor (type-level Stream<T> + stub baseline):\n\
         Content-Type: {ct}\n\
    ...
    assert_eq!(
        tokens, 1,
        "section 1 disjunct (a) v1.28.0: type-level Stream<T> + stub → exactly 1 axon.token"
    );
    assert_eq!(
        completes, 1,
        "section 1 disjunct (a) v1.28.0: terminator is axon.complete (axon dialect; type-annotation only → Q1 Rule 3 axon default)"
    );
}

// ════════════════════════════════════════════════════════════════════
// section 2 — Disjunct (b) Effect-level apply syntax —
...
async fn s2_disjunct_b_apply_syntax_with_stream_effect_emits_openai_baseline() {
    ...
    // → openai dialect default (v1.24.0). Stub backend signals
    ...
    eprintln!(
        "section 2 disjunct (b) anchor (apply: stream-tool + stub baseline):\n\
         Content-Type: {ct}\n\
    ...
    // v1.31.0 — POST-34.d / 36.i: the tool registry is wired,
    ...
    assert!(
        content_chunks > 1,
        "section 2 disjunct (b): the streaming tool body produces a real \
         stream — more than one content-delta reaches the wire \
         (POST-34.d / 36.i). Got {content_chunks}."
    );
    assert!(
        has_done,
        "section 2 disjunct (b) v1.28.0: openai dialect terminator [DONE] present"
    );
    assert!(
        has_role_marker,
        "section 2 disjunct (b) v1.28.0: openai dialect role marker on first chunk"
    );
    assert_eq!(
        axon_tokens, 0,
        "section 2 disjunct (b) v1.28.0: Q1 Rule 2 (algebraic-effect → openai) — \
         no W3C `event: axon.token` lines on the openai-dialect wire"
    );
}

// ════════════════════════════════════════════════════════════════════
// section 3 — Disjunct (c) Effect-level use_tool syntax —
...
async fn s3_disjunct_c_use_tool_syntax_collapses_to_apply_at_runtime() {
    ...
    // `use_tool_step` handler (v1.24.0) treats this as a synchronous
    // tool invocation regardless of whether the tool declares a
    // stream effect.
    //
    // section 3 documents that disjunct (c) is, at the dispatcher layer,
    ...
    // one tracked by v1.24.0's algebraic-effect override).
    //
    ...
    eprintln!(
        "section 3 disjunct (c) anchor (use_tool semantic equivalence to apply baseline):\n\
         Content-Type: {ct}\n\
    ...
    // v1.31.0 — `use_tool` collapses to `apply:` at runtime;
    ...
    assert!(
        content_chunks > 1,
        "section 3 disjunct (c): `use_tool` collapses to the streaming-tool \
...
// algebraic-effects surface in the source grammar (v1.17.0) which
// is parsed but rarely exercised by adopter shapes.
//
// For the diagnostic anchor, section 4 documents the PRE-34 baseline at
...
fn s4_disjunct_d_imperative_perform_yield_baseline_pin() {
    ...
    // section 4 is a STATIC pin — the runtime integration of disjunct (d)
    ...
    // of produces_stream(F) per the paper section 3. section 5 below pins this
    // cardinality.
    eprintln!(
        "section 4 disjunct (d) anchor (perform Stream.Yield static-scan baseline):\n\
         Pre-34: bridge_effect_stream_yield static-scan emits one wire event \
         per static Yield call site (v1.24.0).\n\
...
// ════════════════════════════════════════════════════════════════════
// section 5 — Closed-catalog totality pin: 4 disjunctions, no 5th
// ════════════════════════════════════════════════════════════════════

#[test]
fn s5_produces_stream_disjunction_catalog_is_exactly_four() {
    // The paper section 3 defines `produces_stream(F)` as the disjunction
    // of exactly 4 rules. Adding a 5th (e.g. "perform Channel.Recv
    // produces a stream") is a deliberate language-level decision
    // that requires a paper update + an explicit step. Pinning
    ...
    assert_eq!(
        ...
        4,
        "section 5 paper section 3 totality: produces_stream(F) is the disjunction of \
         EXACTLY 4 rules. Adding a 5th requires a paper update + a \
         deliberate step + cross-stack drift gate update + adopter \
         docs update."
    );
    eprintln!(
        "section 5 anchor: produces_stream disjunctions = {PRODUCES_STREAM_DISJUNCTIONS:?}"
    );
}

// ════════════════════════════════════════════════════════════════════
// section 6 — Tool registry baseline — `is_streaming` does NOT exist yet
// ════════════════════════════════════════════════════════════════════
...
fn s6_tool_registry_is_streaming_field_present_post_34_c() {
    ...
    // drift gate `registry_drift.rs` over a synthetic
    // 30-tool corpus.
    ...
    let stream_entry = ToolEntry {
        ...
        // v1.29.0 — Caller sets explicitly OR uses derive helper.
        is_streaming: derive_is_streaming(&["stream:drop_oldest".to_string()]),
        scrape: None,
    };
    assert!(
        stream_entry.is_streaming,
        "section 6 post-34.c: effect_row containing `stream:<policy>` MUST \
    ...
    assert!(
        ...
        "section 6 post-34.c: effect_row WITHOUT any `stream:` prefix MUST \
         derive is_streaming = false"
    );

    eprintln!(
        "section 6 anchor (post-34.c): ToolEntry.is_streaming structural field \
...
// ════════════════════════════════════════════════════════════════════
// section 7 — Dispatcher baseline — Tool::stream() trait does NOT exist
// ════════════════════════════════════════════════════════════════════
...
fn s7_tool_trait_pre_34_has_no_stream_method() {
    ...
    eprintln!(
        "section 7 anchor: ToolResult is the v1.28.0 single-materialized return type. \

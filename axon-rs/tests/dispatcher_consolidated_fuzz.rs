//! v1.24.0 CONSOLIDATED D12 fuzz pack — cycle-level cross-cutting
//! adversarial coverage that the per-step fuzz packs (33.y.c
//! through 33.y.j) intentionally don't provide.
//!
//! # Why a consolidated pack?
//!
//! Each step 33.y.c–j ships its own architectural-group fuzz pack
...
//! the 33.y.c–j step packs use. NO external dep. Reseeds via
...
//! |---|---|---|
//! | section 1 handler totality stress (46 variants × 50 iters each) | 2 300 | 2 300 |
//! | section 2 cancel propagation across all 46 (46 × 30 iters) | 1 380 | 3 680 |
//! | section 3 orchestration composition (deep-nest × 200 iters) | 200 | 3 800 |
//! | section 4 algebraic-semantics parity (D10, 300 iters) | 300 | 4 100 |
//! | section 5 tool-call interleaving (D8 + D3, 250 iters) | 250 | 4 350 |
//! | section 6 kind slug round-trip (D11 anchor, 46 × 20 iters) | 920 | 5 350 |
//! | **Total**                                       | — | **5 350** |
//!
...
// ────────────────────────────────────────────────────────────────────
// section 0 — Deterministic PRNG (linear congruential, no external dep)
// ────────────────────────────────────────────────────────────────────
...
fn random_reason(lcg: &mut Lcg) -> IRFlowNode {
    IRFlowNode::Reason(IRReasonStep {
        ...
        target: lcg.ascii_with_random_len(16),
        // v2.83.0 — the block form's fields are fuzzed too, not pinned
...
fn random_weave(lcg: &mut Lcg) -> IRFlowNode {
    IRFlowNode::Weave(IRWeaveStep {
        ...
        // v2.83.0 — fuzzed, not pinned to empty: `weave_prompt` now
...
fn assert_clean_outcome(
    ...
    match outcome {
        ...
        // v2.83.0 — hibernate yields the SUSPENSION outcome: the walk
        ...
        // placeholder it replaces was the v2.67.0 F20 defect.
        Ok(NodeOutcome::Hibernated { .. }) if kind == "hibernate" => {}
        Ok(other) => panic!(
            "33.y.n fuzz: unexpected NodeOutcome variant for {kind} iter {iter}: {other:?}"
        ),
        // v2.67.0 — the primitives MADE REAL in v2.67.0 fail CLOSED when
        ...
        // grad — a stale artifact carrying no compile-time derivative (v2.65.0)
        // mandate — v2.83.0: the identity passthrough that bound the target
        //             unchanged under a compliance-sounding name; with no
        //             catalog attached, refusing IS the correct total outcome
        // lambda — v2.83.0: the placeholder string "lambda:<name>(<target>)"
        //             a downstream step consumed as if it were ψ
        // ots — v2.83.0: the identity under a transformation's name
        // agent — v2.83.0: the fuzz ctx has an EMPTY agent catalog, and an
        ...
        // v2.67.0 — warden + quant refuse without their engine port.
        Err(DispatchError::MissingDependency {
            name: "warden_backend",
        }) if kind == "warden" => {}
        Err(DispatchError::MissingDependency {
        ...
        // v1.24.0 — `DispatchError` is `#[non_exhaustive]`; a
...
// ────────────────────────────────────────────────────────────────────
// section 1 — Handler totality stress
// ────────────────────────────────────────────────────────────────────
//
...
#[tokio::test]
async fn handler_totality_stress() {
    ...
    for (kind, factory) in &factories {
        for i in 0..ITERS_PER_VARIANT {
            ...
            assert_eq!(
                actual_kind, *kind,
                "section 1 33.y.n: kind slug drift — factory for {kind:?} produced \
...
// ────────────────────────────────────────────────────────────────────
// section 2 — Cancel propagation across all 46
// ────────────────────────────────────────────────────────────────────
//
...
#[tokio::test]
async fn cancel_propagation_across_all_46() {
    ...
    for (kind, factory) in &factories {
        for i in 0..ITERS_PER_VARIANT {
            ...
            match outcome {
                ...
                other => panic!(
                    "section 2 33.y.n: D3 cancel propagation broken for {kind} iter {i}: \
...
// ────────────────────────────────────────────────────────────────────
// section 3 — Orchestration composition (deep-nest stress)
// ────────────────────────────────────────────────────────────────────
//
...
#[tokio::test]
async fn orchestration_deep_nest_composition() {
    ...
    for i in 0..ITERS {
        ...
        assert!(
            ...
            "section 3 33.y.n iter {i}: step_counter must be monotonic non-decreasing \
        ...
        assert!(
            ...
            "section 3 33.y.n iter {i}: branch_path leaked through composition: {:?}",
            ctx.branch_path
        );
    }
}
...
// ────────────────────────────────────────────────────────────────────
// section 4 — Algebraic-semantics parity (D10 anchor)
// ────────────────────────────────────────────────────────────────────
//
...
#[tokio::test]
async fn algebraic_semantics_parity_d10() {
    ...
    for i in 0..ITERS {
        ...
        assert_eq!(
            kind_a, kind_b,
            "section 4 33.y.n iter {i} variant {kind}: D10 parity broken — \
...
// ────────────────────────────────────────────────────────────────────
// section 5 — Tool-call interleaving (D8 + D3 cross-cut)
// ────────────────────────────────────────────────────────────────────
//
...
#[tokio::test]
async fn tool_call_interleaving_d8_d3() {
    ...
    for i in 0..ITERS {
        ...
        if pre_cancel {
            match outcome {
                Err(DispatchError::UpstreamCancelled) => {}
                other => panic!(
                    "section 5 33.y.n iter {i}: pre-cancel + tool plumb broken: \
            ...
            match outcome {
                Ok(NodeOutcome::Completed { tokens_emitted, .. }) => {
                    assert_eq!(
                        tokens_emitted, 1,
                        "section 5 33.y.n iter {i}: D4 byte-compat broken — \
                         stub backend should emit exactly 1 token"
                    );
                }
                Err(DispatchError::ChannelClosed) => {} // acceptable
                other => panic!(
                    "section 5 33.y.n iter {i}: tool plumb broke outcome shape: \
...
// ────────────────────────────────────────────────────────────────────
// section 6 — Kind slug round-trip (D11 cross-stack anchor)
// ────────────────────────────────────────────────────────────────────
//
...
#[test]
fn kind_slug_round_trip_d11_anchor() {
    ...
    for (expected_kind, factory) in &factories {
        for i in 0..ITERS_PER_VARIANT {
            ...
            assert!(
                ...
                "section 6 33.y.n iter {i} variant {expected_kind}: kind slug must be non-empty"
            );
            assert_eq!(
                actual_kind, *expected_kind,
                "section 6 33.y.n iter {i}: kind slug drift — factory for \
                 {expected_kind:?} produced IR returning {actual_kind:?}"
            );
            assert!(
                expected_kinds.contains(&actual_kind),
                "section 6 33.y.n iter {i}: kind slug {actual_kind:?} not in the \
...
// ────────────────────────────────────────────────────────────────────
// section 7 — Cardinality pins
// ────────────────────────────────────────────────────────────────────

#[test]
fn factory_table_has_exactly_46_entries() {
    let factories = factories();
    assert_eq!(
        factories.len(),
        46,

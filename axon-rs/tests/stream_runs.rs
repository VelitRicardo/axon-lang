//! v2.67.0 — the `stream` block's body EXECUTES.
//!
//! The frontend half (`axon-frontend/tests/stream_body.rs`) proves the
//! body now survives parsing and reaches the IR. This proves the runtime runs it.
//!
//! Before v2.67.0, `run_stream` emitted `StepStart` + `StepComplete` and returned
...
fn stream_node(body: Vec<IRFlowNode>) -> IRFlowNode {
    IRFlowNode::Stream(IRStreamBlock {
        ...
        source_column: 0,
        body,
        // v2.83.0 — this file pins v2.67.0's `body` form, which stays real.
        // The PUBLISHED surface (`stream<T> { on_chunk … on_complete … }`) is
        // gated separately, from SOURCE, in `stream_dispatch.rs`:

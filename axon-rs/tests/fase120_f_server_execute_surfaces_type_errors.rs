//! §Fase 120.f — **`server_execute` says WHAT is wrong, not just how many.**
//!
//! # The audit this came out of
//!
//! §120.e found that `TypeChecker` had two entry points that were separate,
//! drifted copies of the pass list, and that `axon check` took the one missing
//! §113's resource laws. Fixing it raised the obvious follow-up: *which OTHER
...
//!
//! The fixture below is deliberately a §120 program (`axon-T966`, an
...
/// in `axon-frontend/tests/fase120_a_effect_grammar.rs`).
///
...
/// compare empty vectors and pass forever — the failure mode §119 named for
...
/// diagnostics it counts. Before §120.f the messages were computed and dropped
/// one line later.
#[test]
...
/// making it the 104th entry in `fase120_g_dark_targets.pinned`, and taking the
...
/// That distinction is the §120.g finding applied to its own author.
#[cfg(feature = "server")]
#[test]

...
//! v1.24.0 diagnostic anchor — capture the v1.24.0 production-path
//! SSE wire shape that the v1.24.0 cycle rewrites end-to-end.
//!
//! # Why a second diagnostic
//!
//! `sse_full_body_diagnostic.rs` (v1.24.0 anchor) captured the
...
//! NON-streaming). The `Backend::stream()` trait surface (cycle
...
//! invariants. As each 33.x step lands the assertions are rewritten
...
//! in CI secrets) are covered by the opt-in `real_provider.yml`
//! workflow (v1.24.0). This diagnostic uses only the in-tree
...
/// is tested by `sse_full_body_diagnostic.rs`).
///   D1: Stream<T> output + transport: sse → SSE handler dispatch
///   D2: tool declares effect_policy → stream_policies wire field
/// D4: wire format from v1.21.0-d + v1.22.0 + v1.23.0 + v1.24.0-i
/// D6: replay binding active when transport=sse (v1.23.0)
// v1.24.0 — Explicit `transport: sse(axon)` retains the W3C
...
// ─── section 1 — D1 anchor: Backend::stream() reached on production path ──
//
...
// `real_backend_e2e.rs` E2E tests at the trait layer
...
async fn d1_v1_24_0_synthetic_three_word_chunking_baseline() {
    ...
    // `real_backend_e2e.rs::backend_stream_emits_one_chunk_per_upstream_event`.
    assert_eq!(
        events.tokens.len(),
        1,
    ...
    for tok in &events.tokens {
        ...
        assert!(tok.get("trace_id").is_some(), "axon.token MUST carry trace_id (v1.24.0 D6 anchor)");
...
// ─── section 2 — D2 anchor: stream_policies wire field for declared effect ─
//
// Pre-33.x.d: `axon.complete.stream_policies` array is populated
// when the flow's tool declares `<stream:<policy>>` (v1.24.0 wired
...
async fn d2_post_33_x_d_stream_policies_and_enforcement_summary_both_present() {
    ...
    // v1.24.0 for declared effects (D4 byte-compat).
    let policies = complete
        .get("stream_policies")
        .expect("stream_policies array MUST be present (v1.24.0 wired this field — D4 byte-compat)");
...
// ─── section 3 — D2 anchor: no effect → no stream_policies (D4 byte-compat) ─
...
async fn d2_no_declared_effect_omits_stream_policies() {
    ...
    // No declared effect → stream_policies elided (per v1.24.0 D9
    ...
    assert!(
        ...
        "axon.complete without declared effects MUST elide \
         stream_policies (D4 byte-compat with v1.21.0/31/32 adopters)"
    );
}

// ─── section 4 — D5 anchor: axon-W002 surface ────────────────────────────
//
...
// ─── section 5 — D6 anchor: per-step replay binding ────────────────────────
//
// Pre-33.x.f: `/v1/replay/<trace_id>` for SSE routes returns ONE row
// (final response, v1.23.0 binding).
//
// Post-33.x.f: returns N rows for an N-step flow, each with
// step_name + tokens_emitted + output_hash + effect_policy_applied +
// enforcement_summary slice (matches section 2's `enforcement_summary`).

/// v1.24.0 — Replay-bound SSE source. Adds `replay: true` to
...
// ─── section 6 — D3 anchor: cancel observed between emissions, not in-body ──
//
...
// `cancel_inside_reqwest_body.rs` (lands with 33.x.e + uses a
// local axum slow-drip backend mock).
//
// This stub-driven anchor verifies the wire shape is well-formed end-
// to-end (precondition for section 1/section 2/section 4/section 5 to hold).
...
async fn d3_v1_24_0_wire_shape_well_formed_on_happy_path() {
    ...
    assert!(
        ...
        "Wire MUST emit content events per v1.24.0 live forwarding"
    );
        ...
        for tid in &trace_ids {
            assert_eq!(
                ...
                "Every event in a flow MUST carry the same trace_id \
                 (v1.24.0 D6 anchor — preserved across 33.x cycle)"
            );
        }
    }
}

// ─── section 7 — Cycle closure summary (commentary, no assertions) ─────────
//
// **POST-33.x.b STATE** (this version): section 1 D1 anchor has been
...
// is asserted by `real_backend_e2e.rs` against the
// trait surface; HTTP-level real-provider verification ships in
// 33.x.j.
//
// **REMAINING REWRITES** as each subsequent step lands:
//
// section 2 (D2) — `complete.get("enforcement_summary").is_some()` (vs
//             `.is_none()` today) — flips in 33.x.d
// section 3 (D2) — unchanged (D4 byte-compat invariant)
// section 4 (D5) — `body.contains("axon-W002")` (vs `!body.contains(...)`
//             today) when adopter custom backend has no stream() —
//             flips in 33.x.g
// section 5 (D6) — `entries.len() == step_count` (vs `<= 1` today) for SSE
//             flows with `replay: true` — flips in 33.x.f
// section 6 (D3) — unchanged structurally; the real cancel-in-body anchor
//             lives in `cancel_inside_reqwest_body.rs` (lands
//             33.x.e) with a local axum slow-drip mock
//
// Each rewrite is part of the corresponding step's commit. The

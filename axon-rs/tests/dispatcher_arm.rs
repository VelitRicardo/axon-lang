//! v1.29.0 — Integration tests for the streaming-tool
//! dispatcher arm.
//!
...
//!
//! section 1 — Happy-path × 4 tool surfaces (stub-stream / stub alias /
//!      native programmatically flagged / http synchronous fallback)
//! section 2 — Cancel mid-stream × 4 (pre-cancel / mid-stream / on
//!      terminator / after-close no-op)
//! section 3 — Backpressure × 4 policies (drop_oldest / degrade_quality /
//!      pause_upstream / fail — captured in audit row's
//!      effect_policy_applied; chunk enforcement at the unified
//! handler level deferred to v1.29.0)
//! section 4 — Audit row × 4 surfaces (tokens_emitted / output_hash_hex /
//!      policy_slug / http-fallback error captured as
//!      success=false)
//! section 5 — Composition + back-compat (4: ForIn-like sequential /
...
// ════════════════════════════════════════════════════════════════════
// section 1 — Happy-path × 4 tool surfaces
// ════════════════════════════════════════════════════════════════════
...
async fn s1_http_provider_unreachable_url_emits_connect_error_terminator() {
    // v1.29.0 shipped HTTP streaming. With a valid-but-unreachable
...
// ════════════════════════════════════════════════════════════════════
// section 3 — Backpressure × 4 policies (captured in audit row)
// ════════════════════════════════════════════════════════════════════
...
async fn assert_policy_captured_in_audit(policy_slug: &str) {
    ...
    assert_eq!(
        ...
        Some(policy_slug),
        "34.d section 3: audit row MUST capture the declared policy slug \
...
// ════════════════════════════════════════════════════════════════════
// section 4 — Audit row × 4 surfaces
// ════════════════════════════════════════════════════════════════════
...
async fn s4_audit_row_for_stub_stream_captures_tokens_emitted() {
    ...
    assert_eq!(
        ...
        "34.d section 4: stub-stream tool emits 3 non-empty deltas — \
...
async fn s4_audit_row_for_stub_stream_captures_concatenated_output_hash() {
    ...
    assert_eq!(
        ...
        "34.d section 4: audit row's output_hash_hex MUST equal SHA-256 of \
...
async fn s4_audit_row_for_stub_stream_captures_no_policy_when_absent() {
    ...
    assert_eq!(
        ...
        "34.d section 4: when no stream:<policy> in effect_row, \
...
async fn s4_audit_row_for_http_unreachable_records_success_false() {
    ...
    assert!(
        !audit[0].success,
        "34.e section 4: HTTP connect failure → audit row success=false"
    );
}

// ════════════════════════════════════════════════════════════════════
// section 5 — Composition + back-compat × 4
// ════════════════════════════════════════════════════════════════════

...
//! v1.24.0 — `StreamPolicyEnforcer` activates in production.
//!
...
//! only summarize the declared ones (D2 + v1.24.0 D8 multi-
//!   vertical safety).
...
fn stream_source_with_policy(policy: &str) -> String {
    // v1.24.0 — `transport: sse(axon)` (Q5) keeps W3C wire
...
// ─── section 1 — D2 anchor: all four policies activate in production ──────
...
async fn d2_drop_oldest_activates_with_canonical_summary_shape() {
    ...
    // v1.31.0 — `apply: tk` routes through the streaming-tool
    // path (v1.31.0 wired the tool registry); the stub stream emits
...
// ─── section 2 — D4 byte-compat: flows without effects omit summary ───────
...
async fn d4_flow_without_declared_effect_omits_enforcement_summary() {
    ...
    // since v1.24.0 D9 elides empty arrays — D4 invariant.
    assert!(
...
// ─── section 3 — Multi-step with mixed effects ────────────────────────────
...
// ─── section 4 — Adversarial load at trait layer (4 policies × N chunks) ──
//
...
// ─── section 5 — Terminal-chunk handling preserves wire shape ────────────

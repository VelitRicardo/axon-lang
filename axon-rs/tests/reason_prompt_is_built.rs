//! v2.83.0 (runtime half) — the `reason` block's fields become the PROMPT.
//!
//! The grammar half lives in `axon-frontend/tests/reason_block.rs`.
...
//! is precisely the defect v2.83.0 repairs. A gate that cannot read the prompt
...
fn f8r_2_given_is_resolved_against_the_flow_bindings() {
    ...
    // the v2.67.0 defect ("if the evidence is missing, substitute the belief")
    // wearing a prompt.
...
fn f8r_4_depth_and_strategy_are_consumed_by_the_framing() {
    // v2.67.0's rule: a field decided by nothing is the defect this cycle exists to

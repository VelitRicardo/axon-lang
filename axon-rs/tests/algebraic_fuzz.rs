...
fn assert_no_panic(
    ...
    match outcome {
        ...
        // v2.67.0 — `compute_apply` REFUSES when the compute is undeclared,
        ...
        // reached a downstream step that expected a number (v2.67.0 F10).
        //
        // A handler that cannot fail is not total. It is mute.
        Err(DispatchError::BackendError { name, .. }) if name == "compute" => {}
        // v2.83.0 — the same doctrine, for the cage: `mandate_apply`
        ...
        // any name at all — the v2.67.0 F18 identity. The refusal is the total,
        // correct outcome.
        Err(DispatchError::BackendError { name, .. }) if name.starts_with("mandate:") => {}
        // v2.83.0 — same doctrine for the other two apply-family members:

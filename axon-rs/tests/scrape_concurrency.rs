...
//! v2.69.0 (owed debt) — **`scrape.concurrency` is a REAL bound: the crawl runs
...
//! A documented behavior the code did not have (the v2.67.0 / v2.67.0 pattern). The
...
fn crawl_entry(concurrency: i64) -> ToolEntry {
    ...
    ToolEntry {
        ...
        // v2.83.0 added `substrate` to `ToolEntry` and this fixture stopped
        // compiling THEN — silently, because v2.81.0 had just moved the server
        // behind an off-by-default feature, so `cargo test` no longer built the
        // file. Found by the v2.87.0 audit. `None` = the resource declares no
        // `within:`, which is what this pre-v2.67.0 fixture means.
        substrate: None,
        capacity: None,
        sandbox: None,
        max_results: None,
...
/// the pre-v2.69.0 crawl exactly (never accidental concurrency).
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]

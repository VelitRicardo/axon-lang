...
//! v1.24.0 — Integration tests for the default-on flip + the
...
/// v2.89.0 — the three fixtures below declare `shield … { scan: [pii_leak]
...
///
/// Before v2.89.0 those flows completed because **nothing scanned anything**:
...
/// fixtures were, unintentionally, a demonstration of the defect v2.89.0 closes
...
/// flows stop completing, which is v2.89.0 working. If a fixture is meant to
/// have no barrier, remove its `scan:` instead (a shield with no `scan:` still
/// passes content through, honestly — v2.83.0).
...
// ────────────────────────────────────────────────────────────────────
// section 1 — Flag-state pins RETIRED (33.z.e closure)
// ────────────────────────────────────────────────────────────────────
//
...
// shapes activate the dispatcher" — is now covered by section 2 below
// without flag manipulation.

// ────────────────────────────────────────────────────────────────────
// section 2 — 8 anchor shapes activate dispatcher path by DEFAULT (no flag set)
// ────────────────────────────────────────────────────────────────────
//
...
// ── v2.89.0 — a program on disk whose routes are actually served ────────
//
...
async fn a_deployed_fixture_serves_both_endpoint_spellings() {
    const FIXTURE: &str = "tests/fixtures/endpoint/served_routes.axon";
...
async fn assert_default_on_no_w002(label: &str, src: &str) {
    ...
    ensure_vertical_scanners(); // v2.89.0 — see the doc at the definition.
...
// ────────────────────────────────────────────────────────────────────
// section 3 — `axon.tool_call` SSE wire emission (D5 milestone)
// ────────────────────────────────────────────────────────────────────
//
...
// ────────────────────────────────────────────────────────────────────
// section 4 — Vertical-grounded canonical patterns
// ────────────────────────────────────────────────────────────────────
//
...
// v1.28.0 — algebraic-effect verticals declare
...
const HEALTHCARE_CDS_FLOW: &str =
    // v1.31.0 — `clinical_reasoning` is applied at TOP LEVEL by
    // `step Differential`; since v1.31.0 wired the tool registry, a

...
//! v1.24.0 — Production SSE path drives `Backend::stream()`
//! per step.
//!
//! Verifies the bridge that takes v1.24.0-i primitives off the
...
//! - **Plan vivo section 7 "no MVP, no partial solutions"** — flows that
...
// v1.24.0 — Algebraic-effect flows declare
...
// ─── section 2 — D2 anchor: stream_policies populated for declared effects ─
...
async fn d2_drop_oldest_policy_surfaces_on_complete_event() {
    ...
    let policies = c["stream_policies"]
        .as_array()
        .expect("stream_policies array present (v1.24.0 wiring preserved through 33.x.b)");
...
// ─── section 3 — D4 anchor: wire byte-compat preserved for v1.24.0 fields ──
...
async fn d4_no_effect_declaration_elides_stream_policies() {
    ...
    let parsed = parse_sse(&body);
    let c = &parsed.completes[0];
    // Empty stream_policies is elided per v1.24.0 D9 byte-compat —
...
// ─── section 4 — Legacy fallback for unsupported flow shapes ──────────────
...
// ─── section 5 — Plan-build error paths emit structured FlowError ─────────
...
// ─── section 6 — Effect policy + multi-step plan composition ──────────────

#[tokio::test]
async fn declared_effect_on_one_step_does_not_pollute_other_steps() {
    // v1.24.0 — `transport: sse(axon)` keeps W3C wire (Q5).
    ...
    assert!(
        ...
        "Plain step (no declared effect) MUST NOT appear in stream_policies — \
         per-step independence (D2 + v1.24.0 D8 multi-vertical safety)"
    );
}

// ─── section 7 — Cancel-safety preserved (v1.24.0 baseline + 33.x.b) ────
...
// ─── section 8 — Trace correlation: token + complete share trace_id ───────
...
// ─── section 9 — Catalog closure: only known event types emitted ──────────
...
// ─── v1.4.0 — Empty body POST is accepted ──────────────────────────────

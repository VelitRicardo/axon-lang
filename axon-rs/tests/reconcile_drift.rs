...
//! v2.67.0 — `reconcile` **actually detects drift**.
//!
...
// ── v2.89.0 — the same drift, from a program that lives on disk ─────────
//
...
async fn a_deployed_fixture_measures_real_drift_against_its_declared_manifest() {
    const FIXTURE: &str = "tests/fixtures/cognitive_io/reconcile_drift.axon";
...
async fn a_world_that_matches_the_manifest_has_no_drift() {
    ...
    assert_eq!(
        ...
        "both declared resources are reachable ⇒ no drift. And this zero is MEASURED — before \
         v2.67.0 it was assumed, because the loop compared the manifest against itself. Got: {t}"
    );
...
///
/// Before v2.67.0 this reported `0.0`, always, for any world. The primitive whose
...
async fn a_declared_resource_that_is_gone_produces_real_drift_and_fires_on_drift() {
    ...
    assert!(
        ...
        "manifest declares {{Db, Cache}}; the world has {{Db}}. The Jaccard symmetric difference \
         is 1/2 = 0.5. Before v2.67.0 this was structurally ALWAYS 0.0 — the loop compared the \
...
/// them). **v2.67.0** makes `resource` the single source of truth; provisioning lands
/// there.
///
    ...
    if let Some(r) = t["reconciles"].get("Keeper") {
        assert_ne!(
            ...
            "a provision that created nothing must NEVER be reported as done — that is the \
             DryRunHandler defect in a fourth place. v2.67.0 makes `resource` govern something; \

//! §Fase 119.m.4 — **`emit` must not wake another tenant's continuation.**
//!
...
//! resumed as B, with A's payload bound under the event name. That is the §66
//! cross-tenant class, inside a primitive §119.d shipped.
//!
//! It surfaced as a flaky test (`fase119_d_hibernate`, whose four cases all
...
//! channel-level signal within a tenant, and §74's durable delivery is
...
// ── §2 — the session is NOT the boundary ───────────────────────────────────
...
// ── §3 — back-compat for the single-tenant (OSS) case ──────────────────────
...
/// pre-§119.m.4. This is what keeps the fix from being a breaking change for
...
// ── §4 — expiry is still enforced, per tenant ──────────────────────────────

...
//! v1.24.0 — Anchor regression test for the stub-backend
...
//! v1.24.0 steps (33.c live forwarding, 33.d real backend
...
// ─── section 1 — /v1/execute (JSON) path reports steps_executed >= 1 ──────
...
async fn v1_execute_stub_backend_reports_nonzero_steps_executed() {
    ...
    assert!(
        steps >= 1,
        "steps_executed must be >= 1 for the stub path (got {steps}); regression of v1.24.0 Layer 1 fix"
    );
    let step_names = step_audit["step_names"].as_array().expect("step_names array");
    assert!(
        !step_names.is_empty(),
        "step_names must be non-empty for the stub path; regression of v1.24.0 Layer 1 fix"
    );
    assert_eq!(
...
// ─── section 2 — SSE wire body now shows non-hollow step + non-zero count ──
...
async fn dynamic_route_sse_wire_body_carries_step_name_and_count() {
    ...
    assert!(
        ...
        "Wire body MUST carry step:'Generate' in at least one axon.token \
         event after the v1.24.0 Layer 1 fix. Body:\n{body}"
    );
    // Anchor #2: the wire body must contain a non-empty token field
    // matching the stub placeholder. Real backends (v1.24.0) will
    ...
    assert!(
        ...
        "Wire body MUST carry token:'(stub)' for stub-backend dispatch \
         after the v1.24.0 Layer 1 fix. Body:\n{body}"
    );
    ...
    assert!(
        ...
        "axon.complete envelope MUST NOT report steps_executed:0 for a \
         flow that has steps; regression of v1.24.0 Layer 1 fix. \
         Body:\n{body}"
    );
    assert!(
...
// ─── section 3 — Multi-step flow: each step shows up in the wire ──────────
...
// ─── section 4 — Audit trail records the trace ────────────────────────────

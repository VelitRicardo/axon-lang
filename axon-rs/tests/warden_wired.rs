//! v2.67.0 — `warden` MADE REAL: the engine is wired to the keyword.
//!
//! # What v2.67.0 found (F12)
//!
...
//! authorises nothing — v2.43.0 fail-closed).
...
/// The pre-v2.67.0 context: no port at all. Everything must fail CLOSED.
...
// ── v2.89.0 — the same flagship, driven from SOURCE ─────────────────────
//
// Every gate in this file hand-builds `IRScope` and `IRWarden` in Rust. That is
// a proof of the HANDLER, and v2.83.0 is the expensive lesson about what such a
...
///
/// v2.89.0: skipping the checker would let a fixture break a compile-time
...
async fn a_compiled_program_reaches_the_warden_and_is_analysed() {
    const FIXTURE: &str = "tests/fixtures/warden/authorized_audit.axon";
...
async fn warden_produces_real_attested_findings_and_runs_its_body() {
    ...
    // (3) The body RAN. This is the line that would have failed before v2.67.0.
    assert_eq!(
...
/// paraconsistent validator (paper section 5.3): an un-witnessed finding is noise and
...
async fn only_witnessed_findings_cross_the_boundary() {
    ...
    assert!(
        !verify(&noise),
        "an unwitnessed finding must not cross the boundary — that is the whole point of section 5.3"
    );
}
...
/// codebase was in before v2.67.0, and it must now be loud.
#[tokio::test]
...
/// The allowlist is enforced at RUNTIME (the check v2.43.0 deferred). A target
...
/// sharpest edge of the whole cycle: analysing an unread target and reporting

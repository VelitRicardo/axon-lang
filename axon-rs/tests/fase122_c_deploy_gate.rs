//! §Fase 118.b.2 — this gate drives the HTTP deploy surface, which lives behind
//! the `server` feature. Gated at the file level so a `cli`-only build still
//! compiles.
#![cfg(feature = "server")]

//! §Fase 122.c — a program the mounted backend cannot honour is refused at
//! DEPLOY, before it acts.
//!
//! # The charge that opened §122
//!
...
//!
//! D122.3 (ratified): the deploy gate is the SOURCE OF TRUTH — not a rung below
...
async fn a_scope_the_backend_cannot_reach_refuses_the_deploy() {
    const FIXTURE: &str = "tests/fixtures/fase122_c_deploy_gate/live_network_scope.axon";
...
/// other primitive: §111.d refuses it with `axon-E0783`, but at the `yield`.
///
...
/// distinction §122 exists to make legible.
#[tokio::test]

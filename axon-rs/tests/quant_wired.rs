//! v2.67.0 — `quant` + `yield` MADE REAL: the simulator is wired to the keyword.
//!
...
// ── v2.89.0 — the same measurement, driven from SOURCE ──────────────────
//
...
async fn a_compiled_program_reaches_the_simulator_and_is_measured() {
    const FIXTURE: &str = "tests/fixtures/quant/ground_state_measurement.axon";
...
/// and a `RotationLayer` needs real angles. v2.4.0 shipped the knob and never
/// shipped its parameter source.
///
/// Running `U(0)` and calling it the adopter's circuit would be fabricating the
/// physics — the exact class of defect v2.67.0 exists to end.
#[tokio::test]

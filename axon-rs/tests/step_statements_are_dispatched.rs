//! v2.83.0 — **the step-body statement is DISPATCHED**, not merely parsed.
//!
//! **What was wrong.** v2.83.0 gave the PIX verbs (`navigate` / `drill` / `trail`
...
//! `step.guards` (the v2.83.0/v2.83.0 elevations) and never looked at
//! `step.pix_ops`.
//!
...
//!
//! That sentence was false for every one of them. Ten README rows left the v2.81.0
//! ledger on the strength of programs that compiled and then did nothing — the
//! v2.67.0 shape (`warden`/`quant` had a badge, a registry entry, a parser
//! production AND a dispatch arm, and were no-ops) reproduced one cycle later,
//! inside the cycle built to end it. It is the [`feedback_ask_where_production_calls_it`]
...
//!      observes the order.
//!
//! Mutation-verified in both directions (delete the dispatch loop → section 1 red;
//! move it after the generation → section 2 red).
...
/// (blocks 31/33) that v2.83.0 made parse.
...
// ── section 2 — it runs BEFORE the step generates ───────────────────────────────────
...
// ── section 2b — the elevation fails CLOSED ─────────────────────────────────────────

/// A statement that cannot run must stop the step, not be stepped over. The
/// v2.67.0 kernel defect was exactly the opposite reflex — "if the evidence is
    ...
    let err = outcome.err().unwrap_or_else(|| {
        panic!(
            "a statement outside the step-body catalog must be REFUSED — executing \
             ...
             catalog nobody decided (v2.67.0)"
        )
    });
    let msg = format!("{err:?}");
    assert!(
...
// ── section 3 — a step with no statements is unchanged ──────────────────────────────

/// The dispatch loop must be invisible to every pre-v2.83.0 program: no extra

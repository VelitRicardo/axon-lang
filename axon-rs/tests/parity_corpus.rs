//! v1.24.0 — Sync ↔ async parity drift gate over a 50-flow corpus.
//!
//! # What this drift gate enforces
//!
//! For every `.axon` fixture in `tests/fixtures/parity_corpus/`,
...
//! # Corpus organization
//!
//! `tests/fixtures/parity_corpus/` is organized by vertical
...
//! Part 11 section 11.10) + a cross-vertical set:
//!
...
//! - Mid-stream tool-result interleaving — v1.30.0 scope.
...
fn run_sync(source: &str, source_file: &str, flow_name: &str) -> Result<ServerRunnerMetrics, String> {
    ...
    // v1.32.0 (D3) — execute_server_flow gained `request_path` +
...
fn project_events(events: &[FlowExecutionEvent]) -> AsyncMetrics {
    ...
    //
    // The sibling projection in `production_fuzz.rs` carried the same
    // assumption, and v2.89.0 fixed both in one pass: two harnesses reading one
    // event stream with one private copy of the same wrong rule is the v2.87.0
    // shape, and patching only the one that happened to go red would leave the
    // other to be rediscovered later. Note that v2.15.0 already patched a
    ...
    // the v2.15.0 multiplexing key added for exactly this).
    ...
    for ev in events {
        match ev {
            ...
            FlowExecutionEvent::StepComplete {
                ...
                // v2.15.0 — capture the step's FULL output. `StepComplete`
                ...
                // that let the v2.15.0 `navigate`-hallucination bug slip past this
...
/// - The **async dispatcher** (v1.24.0) walks the full IRFlowNode
...
fn corpus_size_pin_target_50() {
    ...
    // v1.24.0 initial wave — corpus seeded; expansion to 50
...
// ────────────────────────────────────────────────────────────────────
// section 2 — Parity drift gate over the full corpus
// ────────────────────────────────────────────────────────────────────

#[tokio::test]
async fn parity_drift_gate_full_corpus() {
    // v1.24.0 — `StreamingViaDispatcherGuard` retired (the

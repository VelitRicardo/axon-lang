//! v2.77.0 — the Facebook Pages connector, against a recorded-fixture
...
//! registry — one real HTTP hop to the fixture server — and verify the v2.48.0
...
/// request body). The body lets v2.77.0 assert the `attached_media` album.
    ...
    std::thread::spawn(move || {
        for stream in listener.incoming() {
            ...
            std::thread::spawn(move || {
                ...
                // what was POSTed (e.g. the v2.77.0 `attached_media` album).
...
fn multi_photo_publish_uploads_unpublished_containers_then_attaches_to_feed() {
    // v2.77.0 — three images become one album: three UNPUBLISHED photo
...
fn a_dead_endpoint_is_a_transport_error() {
    // v2.83.0 — a PRIVILEGED port, not bind-then-drop.
    //
...
/// v2.77.0 — the flow-reachability e2e for multi-photo. A flow that binds a
...
/// v2.48.0 precedence, end to end: the custody-injected `axon_secret` becomes the

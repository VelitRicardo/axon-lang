...
//! - section 2 the `axon-rs/src/wire_envelope.rs` module exists with the
//!    canonical FlowEnvelope/StepAuditTrail/ExecutionMetrics/
//!    BlameContext/BlameKind surface declared.
//! - section 3 `ExecuteRequest` carries `declared_output_type` (the field
//!    that propagates the endpoint declaration to the wire wrapper).
//! - section 4 `extract_inner_ontological_type` round-trips correctly for
//!    every form the adopter can write.
//! - section 5 wire-shape contract: a sealed `FlowEnvelope` JSON object
//!    has every canonical field of the ψ-vector ⟨T, V, E⟩ and the
//!    Theorem 5.1 bound is honored.
//! - section 6 audit_chain_hash deterministic + tamper-evident.
//! - section 7 D5 unwrap policy: validation runs against the inner result
//!    slot, not the envelope outer shape.
//!
//! These -assertions form the **structural enforcement** of the
//! v2.0.0 wire contract — any future PR that regresses one of them
...
// ── section 1 — STATIC grep: response path no longer emits flat ServerExecutionResult ──

#[test]
fn no_flat_server_execution_result_in_response_path() {
    ...
    assert!(
        ...
        "v2.0.0 section 1 — the legacy flat-envelope wire path \
    ...
    assert!(
        ...
        "v2.0.0 section 1 — `execute_handler` MUST call \
    ...
    assert!(
        src.contains(".seal()"),
        "v2.0.0 section 1 — the `.seal()` invariant MUST be applied \
...
// ── section 2 — Surface declarations present ───────────────────────────────

#[test]
fn wire_envelope_surface_present() {
    let src = fs::read_to_string("src/wire_envelope.rs")
        .expect("read wire_envelope.rs");
    assert!(
        src.contains("pub struct FlowEnvelope"),
        "v2.0.0 section 2 — `pub struct FlowEnvelope` MUST be declared"
    );
    assert!(
        src.contains("pub struct StepAuditTrail"),
        "v2.0.0 section 2 — `pub struct StepAuditTrail` MUST be declared"
    );
    assert!(
        src.contains("pub struct ExecutionMetrics"),
        "v2.0.0 section 2 — `pub struct ExecutionMetrics` MUST be declared"
    );
    assert!(
        src.contains("pub struct BlameContext"),
        "v2.0.0 section 2 — `pub struct BlameContext` MUST be declared"
    );
    assert!(
        src.contains("pub enum BlameKind"),
        "v2.0.0 section 2 — `pub enum BlameKind` (closed catalog) MUST be \
         declared"
    );
    assert!(
        src.contains("pub fn extract_inner_ontological_type"),
        "v2.0.0 section 2 — `extract_inner_ontological_type` helper MUST be \
         declared (used by the wire wrappers)"
    );
}

// ── section 3 — ExecuteRequest carries declared_output_type ───────────────

#[test]
fn execute_request_carries_declared_output_type() {
    let src = fs::read_to_string("src/axon_server.rs")
        .expect("read axon_server.rs");
    assert!(
        src.contains("pub declared_output_type: String"),
        "v2.0.0 section 3 — `ExecuteRequest.declared_output_type` MUST be \
         present so the dynamic-route dispatcher can propagate the \
         endpoint's `output:` declaration through to the wire wrapper."
    );
}

// ── section 4 — extract_inner_ontological_type round-trips correctly ──────

#[test]
fn extract_inner_unwraps_envelope_singular() {
    assert_eq!(
        extract_inner_ontological_type("FlowEnvelope<TenantRecord>"),
        "TenantRecord",
        "v2.0.0 section 4 — singular T must unwrap cleanly"
    );
}

#[test]
fn extract_inner_unwraps_envelope_list() {
    assert_eq!(
        extract_inner_ontological_type("FlowEnvelope<List<TenantRecord>>"),
        "List<TenantRecord>",
        "v2.0.0 section 4 — List<T> inside FlowEnvelope unwraps preserving \
         the nested generic"
    );
}

#[test]
fn extract_inner_preserves_bare_type_legacy() {
...
#[test]
fn extract_inner_empty_defaults_to_any() {
    assert_eq!(extract_inner_ontological_type(""), "Any");
    assert_eq!(extract_inner_ontological_type("   "), "Any");
}

// ── section 5 — Wire shape contract: ψ-vector serialization ────────────────

#[test]
fn sealed_envelope_carries_full_psi_vector() {
    ...
    for required in [
        ...
        assert!(
            ...
            "v2.0.0 section 5 — sealed FlowEnvelope MUST contain field \
...
#[test]
fn theorem_5_1_bounds_derived_certainty() {
    ...
    assert!(
        ...
        "v2.0.0 section 5 — Theorem 5.1: derived states (anchor_breaches > 0 \
...
fn blame_kind_serializes_snake_case() {
    ...
    assert_eq!(
        ...
        "v2.0.0 section 5 — BlameKind MUST serialize as snake_case (wire \
    ...
    for (kind, slug) in cases {
        let b = BlameContext {
            kind: kind.clone(),
            // v2.83.0 — this case pins the KIND's wire slug; leaving the
            // party absent is also what proves it stays elided, keeping v2.0.0's
        ...
        assert_eq!(
            j["kind"], slug,
            "v2.0.0 section 5 — BlameKind::{kind:?} MUST serialize as `{slug}`"
        );
    }
}

// ── section 6 — Audit chain hash determinism + tamper-evidence ────────────

#[test]
fn audit_chain_hash_deterministic_on_identical_input() {
    ...
    assert_eq!(
        ...
        "v2.0.0 section 6 — identical (provenance_chain, step_audit) MUST \
         produce identical audit_chain_hash"
    );
    assert_eq!(
        env_a.audit_chain_hash.len(),
        64,
        "v2.0.0 section 6 — SHA-256 hex digest is exactly 64 chars"
    );
}

#[test]
fn audit_chain_hash_changes_on_provenance_tamper() {
    ...
    assert_ne!(
        ...
        "v2.0.0 section 6 — any change to provenance_chain MUST change the \
         audit_chain_hash (Pillar II tamper-evidence)"
    );
}

// ── section 7 — D5 unwrap policy declarative check ─────────────────────────
//
// v2.0.0 UPDATE — the s7 contract evolved. Pre-39.d the D5 gate
...
#[test]
fn d5_gate_delegates_to_canonical_validate_body() {
    // v2.0.0 — the D5 gate's wire-shape handling MUST live inside
    ...
    assert!(
        ...
        "v2.0.0 section 7 (39.d update) — the D5 gate MUST call \
...
#[test]
fn d5_gate_no_longer_manually_extracts_inner_t() {
    // v2.0.0 — the manual extract pattern that 39.b shipped is
    ...
    assert!(
        ...
        "v2.0.0 section 7 (39.d update) — the 39.b manual `let inner_t = \
...
// ── section 8 — End-to-end converter from ServerExecutionResult ────────────

#[test]
fn from_execution_result_e2e() {

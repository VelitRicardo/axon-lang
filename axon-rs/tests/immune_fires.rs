...
//! v2.67.0 — the immune system **actually fires**.
//!
//! v2.67.0 proved a health report gets *produced*. That is not the same as proving
...
// ── v2.89.0 — the same arc, from a program that lives on disk ───────────
//
...
// stops compiling, v2.89.0's ratchet says so before any adopter meets it.
...
async fn a_deployed_fixture_learns_then_fires_its_declared_reflex() {
    const FIXTURE: &str = "tests/fixtures/cognitive_io/immune_reflex_heal.axon";
...
///
/// This is the arc v2.67.0 found unreachable, v2.67.0 built the loop for, and v2.67.0
...
async fn a_real_deviation_fires_the_declared_reflex_with_a_signed_trace() {
    ...
    assert_eq!(
        ...
        "the declared reflex must FIRE on the anomaly it declares it triggers on. This is the \
         arc v2.67.0 found unreachable. Got: {t}"
    );
    let r = &t["reflexes"][0];

...
//! v2.63.0 — the dataspace deploy hook: a DECLARED dataspace is
//! INSTANTIATED in the deterministic columnar engine at deploy time.
//!
//! The ground-truth finding this pins against regression: before v2.63.0,
...
//! through the REAL `/v1/deploy` handler (the v2.49.0 real-path discipline)
...
"#;
    ...
    let store = engine
        .store("Leads")
        .expect("the DECLARED dataspace must exist in the engine — the v2.63.0 ground-truth fix");
    ...
    assert_eq!(store.row_count(), 0, "born empty — ingest is v2.63.0");
}
...
/// v2.63.0 — THE end-to-end: deploy a program whose flow `let`s raw CSV
/// and `ingest`s it into a declared dataspace, execute through the REAL
/// `/v1/execute` handler (the v2.49.0 discipline), and observe REAL rows
/// in the engine — typed, bounded, born-Untrusted. This is the moment
/// `ingest` stops being a narration: pre-v2.63.0 this exact program would
...
"#;
    ...
    assert_eq!(
        ...
        "external data is born Untrusted (v2.52.0)"
    );
...
/// v2.63.0 — THE aggregate E2E: ingest then γ in one deployed flow,
/// through the real HTTP path. The number in the result was COMPUTED
/// over the columnar batches — pre-v2.63.0 this exact `aggregate` step
...
// ── v2.63.0 — the re-founded /v1/dataspace REST surface ─────────────
...
async fn rest_surface_is_a_view_over_declared_dataspaces() {
    ...
    // (1) Free-form creation is RETIRED — the refusal teaches.

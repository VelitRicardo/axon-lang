//! v2.83.0 — `fabric` governs: the RUNTIME half.
//!
//! v2.67.0's finding, in full: *"`provider`/`region`/`zones` are consumed by
...
fn the_substrate_reaches_the_channel_that_actually_runs() {
    ...
    assert_eq!(
        ...
        "the fabric's provider/region must reach the running channel — this is \
         the exact field v2.67.0 found consumed by NOTHING"
    );
}
...
fn the_legacy_entry_point_stays_byte_identical() {
    ...
    // did pre-v2.83.0: bind, attribute nothing, refuse nothing extra.
    let mut reg = registry_with_tool("Api");
